...
pub(crate)fn begin_reproduction(parent:&mut Organism,rng:&mut ChaCha8Rng,catalog:&[BaseResource])->bool{
 ...
 // Construction performs one structural transaction for each blueprint
 // connection. Reproduction therefore pays the same ordinary transaction
 // stress mechanism without an arbitrary reproduction-specific multiplier.
 parent.add_transaction_stress(blueprint.connections.len() as f64);
...
mod tests{
 ...
 #[test]fn reproduction_commits_blueprint_material_without_touching_parent_structure(){let catalog=crate::resources::default_catalog();let mut parent=adult_parent();let original_structure=parent.structure.clone();let mut rng=ChaCha8Rng::seed_from_u64(7);assert!(begin_reproduction(&mut parent,&mut rng,&catalog));assert_eq!(parent.structure,original_structure);assert!(parent.stored_material.is_empty());assert_eq!(parent.reproductive_readiness,0.0);assert!(parent.stress>0.0);let construction=parent.reproductive_construction.as_ref().unwrap();assert_eq!(construction.committed_material.total_amount(),183.0);assert_eq!(construction.committed_material.count_structured(),61)}
 ...
 #[test]fn completed_construction_becomes_an_independent_offspring(){let catalog=crate::resources::default_catalog();let mut parent=adult_parent();let mut rng=ChaCha8Rng::seed_from_u64(7);assert!(begin_reproduction(&mut parent,&mut rng,&catalog));let construction=parent.reproductive_construction.as_mut().unwrap();for _ in 0..61{assert!(advance_construction(construction,&catalog));}assert!(!advance_construction(construction,&catalog));let child=finish_reproduction(&mut parent,"2".into()).unwrap();assert_eq!(child.id,"2");assert!(matches!(child.development_stage,DevelopmentStage::Offspring));assert_eq!(child.structure.units.len(),61);assert_eq!(child.structure.bonds.len(),312);assert!(parent.reproductive_construction.is_none())}
