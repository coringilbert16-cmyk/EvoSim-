...
mod tests{
 ...
 fn adult_parent()->Organism{let catalog=crate::resources::default_catalog();let genome=initial_genome();let structure=genome.structural_blueprint.realize(&catalog).unwrap();let mut storage=MaterialStorage::default();for element in &genome.structural_blueprint.elements{assert!(storage.store(element.material.clone()));}Organism{id:"parent".into(),occupied_cells:vec![Position{x:50.0,y:50.0}],genome,resource_sense:ResourceSense{sensed_resources:Vec::new(),direction_x:0.0,direction_y:0.0,direction_strength:0.0},memory:Vec::new(),decision_history:DecisionHistory::default(),usable_energy:10.0,stress:0.0,stored_material:storage,structure,development_stage:DevelopmentStage::Adult,age:10,reproductive_readiness:1.0,active_transformation_id:None,reproductive_construction:None}}
 #[test]fn reproduction_commits_blueprint_material_without_touching_parent_structure(){let catalog=crate::resources::default_catalog();let mut parent=adult_parent();let original_structure=parent.structure.clone();let mut rng=ChaCha8Rng::seed_from_u64(7);assert!(begin_reproduction(&mut parent,&mut rng,&catalog));assert_eq!(parent.structure,original_structure);assert!(parent.stored_material.is_empty());assert_eq!(parent.reproductive_readiness,0.0);let construction=parent.reproductive_construction.as_ref().unwrap();assert_eq!(construction.committed_material.total_amount(),127.0);assert_eq!(construction.committed_material.count_structured(),42);assert_eq!(construction.developing_structure.units.len(),0)}
 #[test]fn construction_uses_inherited_blueprint_geometry_and_intact_composites(){let catalog=crate::resources::default_catalog();let mut parent=adult_parent();let mut rng=ChaCha8Rng::seed_from_u64(7);assert!(begin_reproduction(&mut parent,&mut rng,&catalog));let construction=parent.reproductive_construction.as_mut().unwrap();for _ in 0..20{assert!(advance_construction(construction,&catalog));}assert_eq!(construction.developing_structure.units.len(),20);assert_eq!(construction.developing_structure.units[0].placement,parent.genome.structural_blueprint.elements[0].placement);assert!(construction.developing_structure.units.iter().any(|u|u.material.is_composite()));assert_eq!(construction.committed_material.total_amount(),107.0)}
 #[test]fn completed_construction_becomes_an_independent_offspring(){let catalog=crate::resources::default_catalog();let mut parent=adult_parent();let mut rng=ChaCha8Rng::seed_from_u64(7);assert!(begin_reproduction(&mut parent,&mut rng,&catalog));let construction=parent.reproductive_construction.as_mut().unwrap();for _ in 0..61{assert!(advance_construction(construction,&catalog));}assert!(!advance_construction(construction,&catalog));let child=finish_reproduction(&mut parent,"2".into()).unwrap();assert_eq!(child.id,"2");assert!(matches!(child.development_stage,DevelopmentStage::Offspring));assert_eq!(child.structure.units.len(),61);assert_eq!(child.structure.bonds.len(),156);assert!(parent.reproductive_construction.is_none())}
