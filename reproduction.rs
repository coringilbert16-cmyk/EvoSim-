...
/// The juvenile begins as an approximately 40% connected realization of its
/// inherited adult blueprint. This is a starting-size target, not a universal
/// anatomy or construction-order rule.
...
/// Selects the connected set that defines the juvenile's initial realized
/// boundary. This chooses *membership*, not a construction sequence. Once
/// that set is defined, its elements may be physically assembled in any valid
/// connected order.
fn juvenile_target_set(
    ...
    while current_mass + f64::EPSILON < target_mass {
        let candidates = frontier(blueprint, &selected, &all_indices(blueprint));
        let next = candidates.into_iter().min_by_key(|&i| i)?;
...
/// Add one permitted blueprint element. No placement or topology is invented
/// here: every relationship comes from the inherited blueprint.
fn add_blueprint_element(
    ...
    for connection in &blueprint.connections {
        ...
        let a = candidate_structure.connection_site(
            crate::structure::ConnectionSiteRef {
                unit_index: new_index,
                point_index: new_point,
            },
            catalog,
        )?;
        let b = candidate_structure.connection_site(
            crate::structure::ConnectionSiteRef {
                unit_index: other_structure_index,
                point_index: other_point,
            },
            catalog,
        )?;
        if !crate::contact::connection_points_contact(
            a,
            &candidate_structure.units[new_index],
            b,
            &candidate_structure.units[other_structure_index],
            1e-9,
            1.0 - 1e-9,
        ) {
            return None;
        ...
        let (_, work, _) = crate::structural_combine::required_investment(
            pa,
            pb,
            evaluation,
            0.0,
        ...
        candidate_structure.add_bond(crate::structure::Bond {
            unit_a: new_index,
            point_a: new_point,
            unit_b: other_structure_index,
            point_b: other_point,
            strength,
            bond_energy: 0.0,
        });
...
/// Attempts one element from the allowed frontier. The first successful
/// candidate wins, so material availability and physical validity can change
/// the order in which a valid blueprint is assembled.
fn construct_any_frontier_element(
    ...
    for blueprint_index in candidates {
        ...
        let mut candidate_structure = structure.clone();
        let Some((_, stress)) = add_blueprint_element(
...
pub(crate) fn begin_reproduction(
    ...
    // The genome core is the only mandatory-first construction region. Even
    // here, there is no fixed element order: any physically valid connected
    // core element with available material may be constructed next.
    while realized.len() < core.len() {
        ...
        .or_else(|| {
            // With no realized elements yet, any core element may be the seed.
