...
mod integration_tests {
    ...
    fn fresh_organism_is_realized_from_its_genome_blueprint() {
        ...
        assert_eq!(o.structure.units.len(), o.genome.structural_blueprint.elements.len());
        assert_eq!(o.structure.units.len(), 61);
        assert_eq!(o.structure.bonds.len(), 312);
    ...
    #[test]
    fn juvenile_reaches_adulthood_at_ninety_percent_of_mature_blueprint_size() {
        let mut s = Simulation::new(31, 10.0);
        assert!(matches!(s.organisms[0].development_stage, DevelopmentStage::Juvenile));
        s.step();
        assert!(matches!(s.organisms[0].development_stage, DevelopmentStage::Adult));
    }
    ...
    fn fresh_simulation_conserves_total_material_over_many_ticks() {
        ...
        assert!((before - s.total_material_in_system()).abs() < 1e-3);
    }
    ...
    fn add_test_break_bond(s: &mut Simulation) {
        ...
        s.organisms[0].structure.add_bond(Bond {
            unit_a: a,
            point_a: 0,
            unit_b: b,
            point_b: 0,
    ...
    fn break_resolution_changes_state_on_expected_tick() {
        ...
        let a = s.organisms[0].structure.units[61]
            .properties(&s.environment.catalog)
            .unwrap();
        let b = s.organisms[0].structure.units[62]
            .properties(&s.environment.catalog)
            .unwrap();
        ...
        .into_iter()
        .find(|c| c.point_a == 0 && c.point_b == 0)
        .expect("test bond connection points must remain resolvable");
