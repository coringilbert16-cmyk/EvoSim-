...
mod integration_tests {
    ...
    fn break_action_starts_a_transformation_before_resolution() {
        ...
        sim.step();
        assert_eq!(sim.organisms[0].structure.bonds.len(), 1);
        assert!(sim.organisms[0].active_transformation_id.is_some(), "BREAK should start a transformation before its delayed resolution");
        assert_eq!(sim.active_transformations.len(), 1);
        assert_eq!(sim.active_transformations[0].decision_context_key.as_deref(), Some("bond:0:0:1:0"));
    }
    ...
    fn break_resolution_changes_state_on_expected_tick() {
        ...
        assert!(sim.organisms[0].decision_history.has_knowledge(ActionKind::Break, Some("bond:0:0:1:0")));
    }
    ...
    fn resolved_transformation_cannot_trigger_a_second_action_in_the_same_tick() {
        ...
        assert_eq!(sim.organisms[0].structure.bonds.len(), 0);
        assert_eq!(sim.active_transformations.len(), 0);
        assert_eq!(sim.organisms[0].decision_history.entries.len(), 1, "BREAK resolution must not permit a second action during the same tick");
    ...
    fn break_can_consume_usable_energy_when_work_exceeds_bond_energy() {
        ...
        let a = sim.organisms[0].structure.add_unit(StructuralUnit::new("Carbon", Placement { x: 500.0, y: 500.0, rotation_radians: 0.0 }));
        let b = sim.organisms[0].structure.add_unit(StructuralUnit::new("Hydrogen", Placement { x: 501.0, y: 500.0, rotation_radians: 0.0 }));
        sim.organisms[0].structure.add_bond(Bond { unit_a: a, point_a: 0, unit_b: b, point_b: 0, strength: 0.5, bond_energy: 0.5 });
        ...
        assert!(matches!(sim.organisms[0].decision_history.outcome(ActionKind::Break, Some("bond:0:0:1:0")), Some(OutcomeKind::Harmful)));
    }
    ...
    fn organism_can_break_a_structural_bond_and_records_outcome() {
        ...
        for _ in 0..20 { sim.step(); if sim.organisms[0].structure.bonds.is_empty() { break; } }
        assert!(sim.organisms[0].structure.bonds.is_empty());
        assert!((sim.organisms[0].usable_energy - before - 10.500007453306344).abs() < 1e-12);
        assert!(sim.organisms[0].decision_history.has_knowledge(ActionKind::Break, Some("bond:0:0:1:0")));
        assert!(matches!(sim.organisms[0].decision_history.outcome(ActionKind::Break, Some("bond:0:0:1:0")), Some(OutcomeKind::Beneficial | OutcomeKind::Neutral | OutcomeKind::Harmful)));
    }
