...
impl crate::state::Simulation {
    pub(crate) fn calculate_property_deviations(
        ...
        PropertyDeviations {
            ...
            potential_energy: ((properties.potential_energy - baselines.potential_energy)
                / ranges.potential_energy)
                .clamp(-1.0, 1.0),
            reactivity: ((properties.reactivity - baselines.reactivity) / ranges.reactivity)
                .clamp(-1.0, 1.0),
            cohesion: ((properties.cohesion - baselines.cohesion) / ranges.cohesion)
                .clamp(-1.0, 1.0),
        }
    }
    ...
    pub(crate) fn calculate_desirability(
        ...
        let base_desirability =
            (responses.mass + energy_response + responses.reactivity + responses.cohesion) / 4.0;
        let amount_factor = Self::amount_factor(perceived_amount);
        let desirability =
            (base_desirability * amount_factor).clamp(-DESIRABILITY_MAX, DESIRABILITY_MAX);
        (
            deviations,
            responses,
            base_desirability,
    ...
    pub(crate) fn update_resource_perception(organism: &mut Organism, environment: &Environment) {
