...
impl Simulation {
    pub(crate) fn new(seed: u64, ticks_per_second: f64) -> Self {
        ...
        let organism = Self::create_initial_organism(seed, &environment.catalog);
        Self {
            tick: 0,
            ticks_per_second,
            running: true,
    ...
    pub(crate) fn create_initial_organism(seed: u64, catalog: &[crate::resources::BaseResource]) -> Organism {
        let genome = initial_genome_variant(seed);
        let structure = crate::reproduction::instantiate_blueprint(&genome.structural_blueprint, catalog)
            .expect("initial genome must contain a physically realizable seed blueprint");
        Organism {
            id: "1".into(),
    ...
    pub(crate) fn step(&mut self) -> Snapshot {
        ...
        for transformation in &completed {
            ...
            if let Some(organism) = self.organisms.iter_mut().find(|o| o.id == transformation.organism_id) { Self::resolve_transformation(transformation, organism, &mut self.environment, &mut self.energy_ledger); }
        }
        ...
        for id in reproduction_requests {
            if let Some(organism) = self.organisms.iter_mut().find(|o| o.id == id) { let _ = crate::reproduction::begin_reproduction(organism, &mut self.rng); }
        }

        let catalog = self.environment.catalog.clone();
        for organism in &mut self.organisms {
            if let Some(construction) = organism.reproductive_construction.as_mut() { let _ = crate::reproduction::advance_construction(construction, &catalog); }
        }
