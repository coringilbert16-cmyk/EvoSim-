...
impl Simulation {
    ...
    fn create_environment() -> Environment {
        ...
        let starting_amounts: [(&str, f64); 7] = [
            ("Carbon", 10_000.0), ("Methane", 5_000.0), ("Hydrogen", 5_000.0),
            ("Sulfur", 5_000.0), ("Nitrogen", 5_000.0), ("Phosphorus", 5_000.0), ("Water", 20_000.0),
        ];
    ...
    pub(crate) fn create_initial_organism() -> Organism {
        Organism {
            id: "1".into(), occupied_cells: vec![Position { x:500.0, y:500.0 }], genome: initial_genome(),
            resource_sense: ResourceSense { sensed_resources:Vec::new(), direction_x:0.0, direction_y:0.0, direction_strength:0.0 },
            memory:Vec::new(), decision_history:crate::decision::DecisionHistory::default(), usable_energy:0.0,
            stress:0.0, stored_unbonded:crate::resources::Material { parts:Vec::new(), bonded:false },
            structure:crate::structure::OrganismStructure::new(), development_stage:DevelopmentStage::Juvenile,
            age:0, reproductive_readiness:0.0, active_transformation_id:None,
        }
    }
    ...
    fn structural_mass(organism: &Organism, environment: &Environment) -> f64 {
        organism.structure.units.iter().filter_map(|unit| {
            unit.properties(&environment.catalog).map(|properties| properties.mass)
        }).sum()
    }
    ...
    fn current_needs(organism: &Organism, environment: &Environment, parameters: DecisionParameters) -> CurrentNeeds {
        ...
        CurrentNeeds { survival, reproduction }
    }
    ...
    fn action_eligibility(organism: &Organism, _environment: &Environment) -> ActionEligibility {
        ActionEligibility {
            can_move: organism.active_transformation_id.is_none(), can_acquire:false,
    ...
    fn decision_candidates(organism: &Organism, needs: CurrentNeeds, eligibility: ActionEligibility) -> Vec<ActionCandidate> {
        ...
        if relevant(ActionKind::Break) {
            candidates.extend(organism.structure.bonds.iter().enumerate().map(|(index, _)| ActionCandidate {
                action: ActionKind::Break, context_key:Some(format!("bond:{index}")),
            }));
        }
        if relevant(ActionKind::Combine) {
            candidates.push(ActionCandidate { action:ActionKind::Combine, context_key:None });
        }
        ...
        if relevant(ActionKind::Acquire) {
            candidates.push(ActionCandidate { action:ActionKind::Acquire, context_key:None });
        }
        if relevant(ActionKind::Expel) {
            candidates.push(ActionCandidate { action:ActionKind::Expel, context_key:None });
        }
        candidates
    }
        ...
        for organism in organisms {
            ...
            match selected.action {
                ActionKind::Move => {
                    ...
                    crate::decision_runtime::record_outcome(&mut organism.decision_history, &selected,
                        if moved { crate::decision::OutcomeKind::Neutral } else { crate::decision::OutcomeKind::Harmful });
                }
                ActionKind::Combine => {
                    let combined = crate::combine_runtime::try_combine(organism, environment, &mut compatibility_cache).is_some();
                    crate::decision_runtime::record_outcome(&mut organism.decision_history, &selected,
                        if combined { crate::decision::OutcomeKind::Neutral } else { crate::decision::OutcomeKind::Harmful });
                }
                ActionKind::Break => {
