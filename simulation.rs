...
impl Simulation {
    ...
    fn recycle_dead_organism(environment:&mut Environment,organism:&Organism)->Option<crate::decomposition::DecomposingBody>{let Some(position)=organism.occupied_cells.first().cloned()else{return None};for material in organism.stored_material.materials.iter().cloned(){environment.field.deposit(position.x,position.y,material);}if let Some(construction)=&organism.reproductive_construction{for material in construction.committed_material.materials.iter().cloned(){environment.field.deposit(position.x,position.y,material);}}crate::decomposition::DecomposingBody::new(organism.structure.clone(),organism.usable_energy,position)}
    ...
    pub(crate) fn step(&mut self)->Snapshot{
        ...
        let mut survivors=Vec::with_capacity(self.organisms.len());for mut organism in self.organisms.drain(..){let dead=Self::apply_energy_capacity(&mut organism);if dead{if let Some(body)=Self::recycle_dead_organism(&mut self.environment,&organism){self.decomposing_bodies.push(body);}}else{survivors.push(organism);}}self.organisms=survivors;
