...
impl Simulation {
    ...
    pub(crate) fn create_initial_organism() -> Organism {
        Organism {
            ...
            usable_energy: 0.0,
            stress: 0.0,
            stored_material: crate::resources::Material {
                parts: Vec::new(),
    ...
    fn acquisition_targets(organism: &Organism, environment: &Environment) -> Vec<usize> {
        ...
        for field_index in
            environment
                .field
                .cells_within_radius(px, py, crate::state::PROCESSING_REACH)
        {
        ...
        ActionEligibility {
            ...
            can_combine: organism.active_transformation_id.is_none()
                && (organism.structure.units.len() >= 2
                    || organism.stored_material.total_amount() >= 2.0 - f64::EPSILON),
...
mod tests {
    ...
    fn acquire_eligibility_requires_reachable_free_material() {
        ...
        sim.environment
            .field
            .deposit_at_index(reachable, Material::free_base("Carbon", 5.0));
        assert!(Simulation::action_eligibility(&sim.organisms[0], &sim.environment).can_acquire);

        sim.environment.field.cells[reachable].materials.clear();
        sim.environment.field.deposit_at_index(
            reachable,
            Material {
    ...
    fn acquire_candidates_identify_each_target_and_ignore_unreachable_or_structured_material() {
        ...
        let near_b = sim
            .environment
            .field
            .index_for_position(525.0, 500.0)
            .unwrap();
        let far = sim
            .environment
            .field
        ...
        sim.environment
            .field
            .deposit_at_index(near_a, Material::free_base("Carbon", 5.0));
        sim.environment
            .field
            .deposit_at_index(near_b, Material::free_base("Hydrogen", 5.0));
        sim.environment.field.deposit_at_index(
            near_b,
            Material {
                parts: vec![("Carbon".into(), 1.0), ("Hydrogen".into(), 1.0)],
        ...
        sim.environment
            .field
            .deposit_at_index(far, Material::free_base("Sulfur", 5.0));
        ...
        let candidates = Simulation::decision_candidates(
            &sim.organisms[0],
            &sim.environment,
            needs,
            eligibility,
        );
        let acquire_contexts: Vec<_> = candidates
            .iter()
        ...
        assert!(!acquire_contexts
            .iter()
            .any(|key| key.starts_with(&format!("target:{far}"))));
    }
    ...
    fn target_specific_history_can_change_acquire_target_selection() {
        ...
        let acquire_candidates: Vec<_> = candidates
            .into_iter()
            .filter(|candidate| candidate.action == ActionKind::Acquire)
            .collect();
        ...
        history.record(
            ActionKind::Acquire,
            acquire_candidates[0].context_key.clone(),
            OutcomeKind::Harmful,
        );
        let context = DecisionContext { needs, eligibility };
