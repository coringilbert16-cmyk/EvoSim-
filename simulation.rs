...
impl Simulation {
    ...
    fn action_eligibility(organism:&Organism,_environment:&Environment)->ActionEligibility {
        ActionEligibility{can_move:organism.active_transformation_id.is_none(),can_acquire:false,can_combine:organism.active_transformation_id.is_none()&&organism.structure.units.len()>=2,can_break:organism.active_transformation_id.is_none()&&!organism.structure.bonds.is_empty(),can_expel:false}
    }
    ...
    pub(crate) fn total_material_in_system(&self)->f64 {
        ...
        for organism in &self.organisms { total+=organism.stored_unbonded.total_amount(); total+=organism.structure.units.iter().filter_map(|unit|unit.properties(&self.environment.catalog).map(|p|p.mass)).sum::<f64>(); }
        total
    }
}
