...
impl Simulation {
    ...
    fn acquisition_targets(organism: &Organism, environment: &Environment) -> Vec<usize> {
        let (px, py) = { let p = &organism.occupied_cells[0]; (p.x, p.y) };
        ...
        for field_index in environment.field.cells_within_radius(px, py, crate::state::PROCESSING_REACH) {
            ...
            if distance > crate::state::PROCESSING_REACH + f64::EPSILON { continue; }
            if environment.field.cells[field_index].materials.iter().any(|material| !material.is_empty() && material.is_valid() && !material.has_internal_structure()) {
    ...
    fn action_eligibility(organism: &Organism, environment: &Environment) -> ActionEligibility {
        ActionEligibility {
            ...
            can_combine: organism.active_transformation_id.is_none() && (organism.structure.units.len() >= 2 || organism.stored_material.total_amount() >= 2.0 - f64::EPSILON),
    ...
    fn acquire_target(organism: &mut Organism, environment: &mut Environment, field_index: usize) -> bool {
        ...
        let dy = target_y - py;
        let distance = (dx * dx + dy * dy).sqrt();
        if distance > crate::state::PROCESSING_REACH + f64::EPSILON { return false; }
        let Some(material_index) = environment.field.cells.get(field_index).and_then(|cell| cell.materials.iter().position(|material| !material.is_empty() && material.is_valid() && !material.has_internal_structure())) else { return false; };
        let Some(material) = environment.field.take_at_index(field_index, material_index, crate::state::PROCESSING_RATE) else { return false; };
        if material.has_internal_structure() || material.is_empty() { return false; }
