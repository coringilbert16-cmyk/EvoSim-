...
impl Simulation {
    ...
    fn structural_mass(organism:&Organism,environment:&Environment)->f64{organism.structure.units.iter().filter_map(|unit|unit.properties(&environment.catalog).map(|properties|properties.mass)).sum()}
    ...
    fn growth_fraction(organism:&Organism,environment:&Environment)->f64{let mature_mass=Self::mature_structural_mass(organism,environment);if !mature_mass.is_finite()||mature_mass<=0.0{return 0.0}(Self::structural_mass(organism,environment)/mature_mass).max(0.0)}
    ...
    fn update_reproductive_readiness(organism:&mut Organism,environment:&Environment,parameters:DecisionParameters){if !matches!(organism.development_stage,DevelopmentStage::Adult){return}let mature_mass=Self::mature_structural_mass(organism,environment).max(f64::EPSILON);let maturity=(Self::structural_mass(organism,environment)/mature_mass).clamp(0.0,1.0);let reproduction_reserve=parameters.reproduction_reserve.max(f64::EPSILON);let energy_readiness=(organism.usable_energy/reproduction_reserve).clamp(0.0,1.0);let accumulation=(maturity*energy_readiness*parameters.reproduction_accumulation_rate.max(0.0)).clamp(0.0,1.0);organism.reproductive_readiness=(organism.reproductive_readiness+accumulation).clamp(0.0,1.0)}
    fn acquisition_targets(organism:&Organism,environment:&Environment)->Vec<usize>{let Some(position)=organism.occupied_cells.first()else{return Vec::new()};let Some(field_index)=environment.field.index_for_position(position.x,position.y)else{return Vec::new()};if environment.field.cells[field_index].materials.iter().any(|material|!material.is_empty()&&material.is_valid()){vec![field_index]}else{Vec::new()}}
    fn acquisition_context_key(field_index:usize)->String{format!("target:{field_index}")}
    fn action_eligibility(organism:&Organism,environment:&Environment)->ActionEligibility{let can_build_from_storage=organism.structure.units.len()>=1&&organism.stored_material.count_unstructured()>0;let can_join_existing_structure=organism.structure.units.len()>=2;ActionEligibility{can_move:organism.active_transformation_id.is_none(),can_acquire:organism.active_transformation_id.is_none()&&!Self::acquisition_targets(organism,environment).is_empty(),can_combine:organism.active_transformation_id.is_none()&&(can_build_from_storage||can_join_existing_structure),can_break:organism.active_transformation_id.is_none()&&!organism.structure.bonds.is_empty(),can_expel:false}}
    ...
    fn acquire_target(organism:&mut Organism,environment:&mut Environment,field_index:usize)->bool{let Some(position)=organism.occupied_cells.first()else{return false};let Some(expected_index)=environment.field.index_for_position(position.x,position.y)else{return false};if expected_index!=field_index{return false}let Some(material)=environment.field.take_for_acquisition(field_index)else{return false};organism.store_material(material)}

    fn recycle_dead_organism(environment:&mut Environment,organism:&Organism){let Some(position)=organism.occupied_cells.first()else{return};for material in organism.stored_material.materials.iter().cloned(){environment.field.deposit(position.x,position.y,material);}if let Some(construction)=&organism.reproductive_construction{for material in construction.committed_material.materials.iter().cloned(){environment.field.deposit(position.x,position.y,material);}}for unit in &organism.structure.units{environment.field.deposit(position.x,position.y,unit.material.material().clone());}}
    ...
    pub(crate) fn step(&mut self)->Snapshot{
        ...
        let environment_snapshot=self.environment.clone();let decision_parameters=self.decision_parameters;for organism in &mut self.organisms{organism.age+=1;Self::update_development_stage(organism,&environment_snapshot);Self::update_resource_perception(organism,&environment_snapshot);Self::update_memory_from_sources(organism,&environment_snapshot);Self::update_reproductive_readiness(organism,&environment_snapshot,decision_parameters);}
        let mut reproduction_requests=Vec::new();{let(organisms,environment)=(&mut self.organisms,&mut self.environment);let mut compatibility_cache=crate::contact::ConnectionCompatibilityCache::new();for organism in organisms{if completed_organisms.contains(&organism.id){continue}let needs=Self::current_needs(organism,environment,decision_parameters);let eligibility=Self::action_eligibility(organism,environment);let context=DecisionContext{needs,eligibility};let candidates=Self::decision_candidates(organism,environment,needs,eligibility);let Some(selected)=select_action(context,&organism.decision_history,&candidates)else{continue};match selected.action{ActionKind::Move=>{let moved=Self::update_movement(organism,environment);crate::decision_runtime::record_outcome(&mut organism.decision_history,&selected,if moved{crate::decision::OutcomeKind::Neutral}else{crate::decision::OutcomeKind::Harmful});}ActionKind::Combine=>{let combined=crate::combine_runtime::try_combine(organism,environment,&mut compatibility_cache).is_some();crate::decision_runtime::record_outcome(&mut organism.decision_history,&selected,if combined{crate::decision::OutcomeKind::Neutral}else{crate::decision::OutcomeKind::Harmful});if organism.reproductive_readiness>=1.0-f64::EPSILON{reproduction_requests.push(organism.id.clone());}}ActionKind::Break=>{if let Some(transformation)=Self::try_start_transformation(organism,&environment.catalog,&mut self.next_transformation_id,&selected){self.active_transformations.push(transformation);}}ActionKind::Acquire=>{let success=selected.context_key.as_deref().and_then(|key|key.strip_prefix("target:")).and_then(|index|index.parse::<usize>().ok()).map(|field_index|Self::acquire_target(organism,environment,field_index)).unwrap_or(false);crate::decision_runtime::record_outcome(&mut organism.decision_history,&selected,if success{crate::decision::OutcomeKind::Neutral}else{crate::decision::OutcomeKind::Harmful});}ActionKind::Expel=>{}}}}
        ...
        let mut offspring=Vec::new();let mut next_organism_id=self.next_organism_id;for organism in &mut self.organisms{if organism.reproductive_construction.is_some(){if let Some(construction)=organism.reproductive_construction.as_mut(){let _=crate::reproduction::advance_construction(construction,&catalog);}if organism.reproductive_construction.as_ref().map(|construction|construction.committed_material.is_empty()).unwrap_or(false){let child_id=next_organism_id.to_string();if let Some(child)=crate::reproduction::finish_reproduction(organism,child_id){next_organism_id+=1;offspring.push(child);}}}}self.next_organism_id=next_organism_id;self.organisms.extend(offspring);
        let mut survivors=Vec::with_capacity(self.organisms.len());for mut organism in self.organisms.drain(..){let dead=Self::apply_energy_capacity(&mut organism);if dead{Self::recycle_dead_organism(&mut self.environment,&organism);}else{survivors.push(organism);}}self.organisms=survivors;let live_ids:HashSet<String>=self.organisms.iter().map(|o|o.id.clone()).collect();self.active_transformations.retain(|t|live_ids.contains(&t.organism_id));self.energy_ledger.total_usable_energy_held=self.organisms.iter().map(|o|o.usable_energy).sum();self.snapshot()
    }

    pub(crate) fn apply_energy_capacity(organism:&mut Organism)->bool{organism.stress*=crate::state::STRESS_DECAY_PER_TICK;organism.apply_stress_damage()}
