...
impl StructuralMaterial {
    ...
    pub fn primary_resource_name(&self) -> Option<&str> { self.material.parts.first().map(|(name, _)| name.as_str()) }
...
mod tests {
    ...
    #[test] fn primary_resource_name_is_derived_from_material() { let m=StructuralMaterial::single("Carbon"); assert_eq!(m.primary_resource_name(), Some("Carbon")); }
    ...
    #[test] fn composite_primary_resource_name_is_stable_and_material_derived() { let a=Material {parts:vec![("Carbon".into(),1.0)],bonded:true}; let b=Material {parts:vec![("Methane".into(),1.0)],bonded:true}; let m=StructuralMaterial::combine(&a,&b).unwrap(); assert_eq!(m.primary_resource_name(), Some("Carbon")); }
