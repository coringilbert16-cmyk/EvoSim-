...
/// structural unit. Storage is consumed by this operation; the material itself
/// is not divided or otherwise transformed.
pub(crate) fn instantiate_one_unit(
    organism: &mut Organism,
    catalog: &[BaseResource],
) -> Option<usize> {
    let material = organism.stored_material.take_one_unstructured()?;
...
pub(crate) fn try_combine(
        ...
        for unit_b in (unit_a + 1)..organism.structure.units.len() {
            for evaluation in eligible_candidates(&organism.structure, unit_a, unit_b, catalog, compatibility_cache)
                ...
                .filter_map(|candidate| {
                    ...
                    Some(crate::combine::evaluate_formation(candidate, a.cohesion, b.cohesion))
                })
            {
    ...
    let water_field = environment.field.index_for_position(organism.occupied_cells.first()?.x, organism.occupied_cells.first()?.y)
        .map(|index| environment.field.cells[index].materials.iter().flat_map(|material| material.parts.iter()).filter(|(name, _)| name == "Water").map(|(_, amount)| *amount).sum::<f64>())
        .unwrap_or(0.0);

    let (interaction, work_cost, energy_paid) = crate::structural_combine::required_investment(props_a, props_b, evaluation, water_field).ok()?;
    let surplus = energy_paid - evaluation.threshold;
    if organism.usable_energy + EPSILON < energy_paid { return None; }

    let bond_strength = bond_strength(props_a, props_b);
    let bond_energy = surplus;
    let bond = crate::structure::Bond {
        unit_a, point_a: evaluation.candidate.point_a,
        unit_b, point_b: evaluation.candidate.point_b,
        strength: bond_strength, bond_energy,
    };
    crate::contact::try_add_bond(&mut organism.structure, bond, catalog).ok()?;
    organism.usable_energy -= energy_paid;

    Some(CombineAttempt {
        unit_a, unit_b, point_a: evaluation.candidate.point_a, point_b: evaluation.candidate.point_b,
        work_cost, energy_paid,
        interaction_direction: interaction.direction, interaction_magnitude: interaction.magnitude,
        formation_threshold: evaluation.threshold, surplus, bond_strength, bond_energy,
    })
}
...
fn _raw_material_type_check(raw: &Material, catalog: &[BaseResource]) -> bool {
    !raw.has_internal_structure()
        && raw.parts.iter().all(|(name, amount)| *amount >= 0.0 && catalog.iter().any(|r| r.name == *name))
}
