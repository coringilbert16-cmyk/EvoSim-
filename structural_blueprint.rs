...
mod tests {
    ...
    fn blueprint_represents_material_topology_and_geometry() {
        ...
        let mut b = single_element("Methane", catalog[1].shape.clone(), 0.8);
    ...
    fn blueprint_round_trip_preserves_identity() {
        ...
        assert_eq!(restored.elements.len(), blueprint.elements.len());
        assert_eq!(restored.connections, blueprint.connections);
        assert_eq!(restored.elements[0].material, blueprint.elements[0].material);
        assert_eq!(restored.elements[0].placement, blueprint.elements[0].placement);
        assert_eq!(restored.elements[0].geometry.envelope.form, blueprint.elements[0].geometry.envelope.form);
        assert_eq!(restored.elements[0].geometry.connection_regions.len(), blueprint.elements[0].geometry.connection_regions.len());
        for (left, right) in restored.elements[0].geometry.connection_regions.iter().zip(&blueprint.elements[0].geometry.connection_regions) {
            assert!((left.point.x - right.point.x).abs() < 1e-12);
            assert!((left.point.y - right.point.y).abs() < 1e-12);
            assert!((left.point.direction_radians - right.point.direction_radians).abs() < 1e-12);
        }
    }
