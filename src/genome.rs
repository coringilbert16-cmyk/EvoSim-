...
fn default_structural_blueprint() -> StructuralBlueprint {
    crate::seed_selector::select_seed().structural
}

fn default_atomic_blueprint() -> AtomicBlueprint {
    crate::seed_selector::select_seed().atomic
}

pub fn initial_genome() -> Genome {
    let seed = crate::seed_selector::select_seed();
    Genome {
        traits: vec![
        ...
        structural_blueprint: seed.structural,
        atomic_blueprint: seed.atomic,
    }
}
...
mod tests {
    ...
    #[test]
    fn seed_blueprint_is_small_and_layered() {
        ...
        assert!(b.elements[1..7].iter().all(|e| e.material.parts[0].0 == "Water"));
        assert!(b.elements[7..13].iter().all(|e| e.material.parts[0].0 == "Sulfur"));
    }
