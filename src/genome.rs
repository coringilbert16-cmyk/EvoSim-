...
impl Genome {
    ...
    pub fn mature_construction_target(&self) -> Result<StructuralBlueprint, String> { self.architecture.adult_construction_target() }
    pub fn developmental_construction_target(&self, catalog: &[crate::resources::BaseResource]) -> Result<StructuralBlueprint, String> {
        self.architecture.developmental_target(JUVENILE_LINEAR_SCALE, catalog)
    }

    fn refresh_construction_caches(&mut self) -> Result<(), String> {
        self.structural_blueprint = self.mature_construction_target()?;
        self.juvenile_blueprint = self.developmental_construction_target(&crate::resources::default_catalog())?;
        Ok(())
    }
    ...
    pub fn mutate(&mut self, rng: &mut ChaCha8Rng) {
        ...
        self.mutate_architecture(rng, (probability_sum / count).clamp(1e-6, 0.25), (sigma_sum / count).clamp(1e-6, 1.0));
    }

    fn mutate_architecture(&mut self, rng: &mut ChaCha8Rng, mutation_probability: f64, mutation_sigma: f64) {
        let original = self.architecture.clone(); let original_adult = self.structural_blueprint.clone(); let original_juvenile = self.juvenile_blueprint.clone();
        let probability = mutation_probability.clamp(0.0, 1.0); let sigma = mutation_sigma.max(0.0);
        for region in &mut self.architecture.regions {
            if rng.gen::<f64>() >= probability { continue; }
            region.center_x += rng.gen_range(-1.0..1.0) * sigma; region.center_y += rng.gen_range(-1.0..1.0) * sigma;
            let factor = (1.0 + rng.gen_range(-1.0..1.0) * sigma * 0.1).max(0.01); region.extent_x *= factor; region.extent_y *= factor;
        }
        if self.architecture.validate().is_err() || self.refresh_construction_caches().is_err() { self.architecture = original; self.structural_blueprint = original_adult; self.juvenile_blueprint = original_juvenile; }
    }
}
...
pub fn initial_genome() -> Genome {
    ...
    let mut genome = Genome {
        ...
        juvenile_reserve: default_juvenile_reserve(), juvenile_energy_reserve: default_juvenile_energy_reserve(), architecture, structural_blueprint: empty_blueprint(), juvenile_blueprint: empty_blueprint(),
    };
    genome.refresh_construction_caches().expect("default architecture must produce construction targets"); genome
}

#[cfg(test)]
mod tests {
    use super::*; use crate::resources::default_catalog; use rand::SeedableRng;
    #[test] fn genome_architecture_is_the_serialized_structural_authority() { let genome = initial_genome(); assert!(genome.architecture.validate().is_ok()); assert_eq!(genome.architecture.regions.len(), 3); }
    #[test] fn construction_targets_are_derived_from_architecture() { let genome = initial_genome(); let catalog = default_catalog(); assert!(genome.mature_construction_target().unwrap().is_valid()); assert!(genome.developmental_construction_target(&catalog).unwrap().is_valid()); }
    #[test] fn structural_mutation_rolls_back_architecture_and_derived_targets() { let mut genome = initial_genome(); let before_architecture = genome.architecture.clone(); let before_adult = genome.structural_blueprint.clone(); let before_juvenile = genome.juvenile_blueprint.clone(); let mut rng = ChaCha8Rng::seed_from_u64(7); genome.mutate_architecture(&mut rng, 1.0, f64::INFINITY); assert_eq!(genome.architecture, before_architecture); assert_eq!(genome.structural_blueprint, before_adult); assert_eq!(genome.juvenile_blueprint, before_juvenile); }
    #[test] fn reserves_remain_genome_defined() { let genome = initial_genome(); assert!(genome.juvenile_reserve.is_valid()); assert_eq!(genome.juvenile_reserve.total_amount(), 1.0); assert!(genome.juvenile_energy_reserve.is_finite() && genome.juvenile_energy_reserve > 0.0); }
}
