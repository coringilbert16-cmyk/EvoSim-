...
impl Genome {
    ...
    pub fn mutate(&mut self, rng: &mut ChaCha8Rng) {
        ...
        self.mutate_structural_blueprint(
            rng,
            (mutation_probability_sum / count).clamp(1e-6, 0.25),
            (mutation_sigma_sum / count).clamp(1e-6, 1.0),
        );
    }
...
fn add_interface_connectors(
    ...
    elements.extend([
        BlueprintElement {
            material: seed_interface_material(),
            placement: BlueprintPlacement {
                x: 0.0,
                y: center,
