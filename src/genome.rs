...
pub struct Genome {
    ...
    /// Finite physical material reserve required for a viable juvenile. The
    /// resource used by the default seed is an implementation choice, not a
    /// biological law that every juvenile must use the same resource.
    #[serde(default = "default_juvenile_reserve")]
    pub juvenile_reserve: Material,
    /// Finite initial usable-energy reserve for a viable juvenile.
    ...
    /// Mature developmental target. This is genome intent; realized physical
    /// structure remains the sole physical authority.
...
fn add_core_shell(elements: &mut Vec<BlueprintElement>, connections: &mut Vec<BlueprintConnection>) {
    ...
    connections.extend([
        BlueprintConnection { element_a: 0, element_b: 1 }, BlueprintConnection { element_a: 0, element_b: 2 },
        BlueprintConnection { element_a: 1, element_b: 3 }, BlueprintConnection { element_a: 2, element_b: 3 },
    ]);
}
...
fn add_outer_shell(elements: &mut Vec<BlueprintElement>, connections: &mut Vec<BlueprintConnection>) {
    ...
    connections.extend([
        BlueprintConnection { element_a: start, element_b: start + 1 }, BlueprintConnection { element_a: start + 1, element_b: start + 5 },
        BlueprintConnection { element_a: start + 5, element_b: start + 4 }, BlueprintConnection { element_a: start + 4, element_b: start + 7 },
        BlueprintConnection { element_a: start + 7, element_b: start + 6 }, BlueprintConnection { element_a: start + 6, element_b: start + 2 },
        BlueprintConnection { element_a: start + 2, element_b: start + 3 }, BlueprintConnection { element_a: start + 3, element_b: start },
    ]);
}

fn add_interface_connectors(elements: &mut Vec<BlueprintElement>, connections: &mut Vec<BlueprintConnection>) {
    let inner_outer = 1.086_648; let outer_inner = 1.511_858; let length = 0.797_884;
    let gap = outer_inner - inner_outer; let tangent = (length * length - gap * gap).sqrt(); let center = (inner_outer + outer_inner) / 2.0; let start = elements.len();
    elements.extend([
...
/// Add the first post-juvenile growth step. It is deliberately only a
/// developmental target, not a definition of adulthood. The added Nitrogen
/// segment is connected through a physical Hydrogen constituent whose exact
/// endpoints meet the existing and new rigid geometry.
fn add_mature_growth_step(elements: &mut Vec<BlueprintElement>, connections: &mut Vec<BlueprintConnection>) {
    let half = 1.511_858 / 2.0; let thickness = 0.330_719; let outer_offset = 1.677_2175;
    let old_corner_y = outer_offset + thickness / 2.0;
    let new_center_y = old_corner_y + 0.797_884 + thickness / 2.0;
    let connector_center_y = (old_corner_y + (new_center_y - thickness / 2.0)) / 2.0;
    let nitrogen_index = elements.len();
    elements.push(BlueprintElement { material: seed_wall_material(), placement: BlueprintPlacement { x: half, y: new_center_y, rotation_radians: 0.0 } });
    let hydrogen_index = elements.len();
    elements.push(BlueprintElement { material: seed_interface_material(), placement: BlueprintPlacement { x: half, y: connector_center_y, rotation_radians: std::f64::consts::FRAC_PI_2 } });
    connections.push(BlueprintConnection { element_a: 5, element_b: hydrogen_index });
    connections.push(BlueprintConnection { element_a: nitrogen_index, element_b: hydrogen_index });
}

fn default_juvenile_blueprint() -> StructuralBlueprint {
    let mut elements = Vec::new(); let mut connections = Vec::new();
    add_core_shell(&mut elements, &mut connections); add_outer_shell(&mut elements, &mut connections); add_interface_connectors(&mut elements, &mut connections);
...
fn default_structural_blueprint() -> StructuralBlueprint {
    let mut elements = Vec::new(); let mut connections = Vec::new();
    add_core_shell(&mut elements, &mut connections); add_outer_shell(&mut elements, &mut connections); add_interface_connectors(&mut elements, &mut connections); add_mature_growth_step(&mut elements, &mut connections);
    StructuralBlueprint::with_core_elements(elements, connections, vec![0, 1, 2, 3])
}
...
pub fn initial_genome() -> Genome {
    Genome {
        traits: vec![
            trait_def("memory_strength", 0.5, 0.05), trait_def("perception_radius", 100.0, 1.0), trait_def("sensory_resolution", 0.5, 0.05),
            trait_def("directional_resolution", 1.0, 0.05), trait_def("mass_affinity", 0.0, 0.05), trait_def("potential_energy_affinity", 0.5, 0.05),
            trait_def("reactivity_affinity", 0.0, 0.05), trait_def("cohesion_affinity", 0.0, 0.05), trait_def("processing_efficiency", 0.8, 0.05),
            trait_def("movement_efficiency", 0.8, 0.05), trait_def("reproductive_investment", 0.5, 0.05),
        ],
        juvenile_reserve: default_juvenile_reserve(), juvenile_energy_reserve: default_juvenile_energy_reserve(),
        juvenile_blueprint: default_juvenile_blueprint(), structural_blueprint: default_structural_blueprint(),
    }
}

#[cfg(test)]
mod tests {
    use super::*; use crate::cavity::analyze_genome_cavity; use crate::resources::default_catalog; use rand::SeedableRng;
    #[test] fn juvenile_blueprint_has_sealed_genome_and_extracore_structure() { let g=initial_genome(); let b=&g.juvenile_blueprint; assert_eq!(b.elements.len(),16); assert_eq!(b.core_elements,vec![0,1,2,3]); assert!(b.validate().is_ok()); let s=b.realize(&default_catalog()).unwrap(); assert!(analyze_genome_cavity(&s,&default_catalog(),&b.core_elements).unwrap().is_some_and(|c| c.qualifies())); assert!(s.units.len()>b.core_elements.len()); }
    ...
    #[test] fn structural_mutation_preserves_developmental_blueprint_validity() { let mut g=initial_genome(); let before=g.structural_blueprint.clone(); let mut rng=ChaCha8Rng::seed_from_u64(7); g.mutate_structural_blueprint(&mut rng,1.0,0.05); assert!(g.structural_blueprint.is_valid()); assert_ne!(g.structural_blueprint,before); assert_eq!(g.juvenile_blueprint.core_elements,vec![0,1,2,3]); }
    #[test] fn invalid_structural_mutation_is_rejected_transactionally() { let mut g=initial_genome(); let before=g.structural_blueprint.clone(); let mut rng=ChaCha8Rng::seed_from_u64(7); g.mutate_structural_blueprint(&mut rng,1.0,f64::INFINITY); assert_eq!(g.structural_blueprint,before); }
}
