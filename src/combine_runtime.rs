...
pub(crate) struct CombineAttempt {
    pub unit_a: usize, pub unit_b: usize, pub endpoint_a: ConnectionEndpoint, pub endpoint_b: ConnectionEndpoint,
    pub work_cost: f64, pub energy_invested: f64, pub interaction_direction: f64, pub interaction_magnitude: f64,
    pub interaction_energy: f64, pub formation_threshold: f64, pub net_energy_change: f64, pub bond_strength: f64, pub bond_energy: f64,
}
...
fn water_field_amount(environment: &Environment, organism: &Organism) -> f64 {
    organism.occupied_cells.first().and_then(|p| environment.field.index_for_position(p.x, p.y))
        .map(|i| environment.field.cells[i].materials.iter().flat_map(|m| m.parts.iter()).filter(|(n, _)| n == "Water").map(|(_, a)| *a).sum()).unwrap_or(0.0)
}
fn placement_for_connection(existing: &crate::structure::StructuralUnit, existing_point: crate::resources::ConnectionPoint, new_point: crate::resources::ConnectionPoint) -> Placement {
    let w = crate::contact::world_connection_point(existing_point, existing); let angle = w.normal_y.atan2(w.normal_x);
    let rotation = angle + std::f64::consts::PI - new_point.direction_radians; let (s, c) = rotation.sin_cos();
    let rx = new_point.x * c - new_point.y * s; let ry = new_point.x * s + new_point.y * c;
...
fn placement_for_continuous_new(existing: &crate::structure::StructuralUnit, existing_point: crate::resources::ConnectionPoint, new_shape: &crate::resources::Shape) -> Placement {
    let w = crate::contact::world_connection_point(existing_point, existing); let radius = new_shape.form.bounding_radius();
...
fn energy_requirement(investment: f64, work: f64, interaction: f64) -> Option<f64> {
    ...
    let required = investment + work - interaction; required.is_finite().then_some(required.max(0.0))
}
fn settle_energy(energy: &mut f64, investment: f64, work: f64, interaction: f64) -> Option<f64> {
    let required = energy_requirement(investment, work, interaction)?; if *energy + EPSILON < required { return None; }
    let net = interaction - investment - work; let next = *energy + net;
...
fn evaluate_candidate(structure: &crate::structure::OrganismStructure, ua: usize, ub: usize, candidate: crate::contact::ConnectionPairCandidate, catalog: &[BaseResource], water: f64) -> Option<(FormationEvaluation, ExperimentalInteraction, f64, f64, f64)> {
    ...
    let required = energy_requirement(investment, work, interaction.signed_value)?; Some((evaluation, interaction, work, investment, required))
}
...
fn form_bond(structure: &mut crate::structure::OrganismStructure, request: BondFormationRequest, catalog: &[BaseResource], cache: &mut ConnectionCompatibilityCache, energy: &mut f64) -> Option<CombineAttempt> {
    ...
    let mut trial_energy = *energy; let net = settle_energy(&mut trial_energy, investment, work, interaction.signed_value)?;
    *structure = trial_structure; *energy = trial_energy;
...
pub(crate) fn instantiate_one_unit(organism: &mut Organism, catalog: &[BaseResource]) -> Option<usize> {
    let material = organism.stored_material.peek_one_unstructured()?; let resource_name = material.parts.first().filter(|(_, a)| (*a - 1.0).abs() <= EPSILON).map(|(n, _)| n.clone())?;
    if material.parts.len() != 1 || catalog.iter().all(|b| b.name != resource_name) { return None; }
    let material = organism.stored_material.take_one_unstructured_named(&resource_name)?; let (x, y) = organism.occupied_cells.first().map(|p| (p.x, p.y)).unwrap_or((0.0, 0.0));
...
pub(crate) fn try_combine_stored_unit(organism: &mut Organism, environment: &Environment, cache: &mut ConnectionCompatibilityCache) -> Option<CombineAttempt> {
    ...
    let resource_name = raw.parts[0].0.clone(); let resource = environment.catalog.iter().find(|b| b.name == resource_name)?; let water = water_field_amount(environment, organism);
    ...
    for ua in 0..organism.structure.units.len() {
        ...
        for &ep in &existing {
            ...
            for placement in placements {
                let mut hypothetical = organism.structure.clone(); let ub = hypothetical.add_unit(StructuralUnit::from_material(raw.clone(), placement)?);
    ...
    let (ua, placement, evaluation, _) = best?; let ub = organism.structure.units.len(); let mut hypothetical = organism.structure.clone(); hypothetical.add_unit(StructuralUnit::from_material(raw.clone(), placement)?);
    let mut energy = organism.usable_energy; let attempt = form_bond(&mut hypothetical, BondFormationRequest { unit_a: ua, unit_b: ub, endpoint_a: evaluation.candidate.endpoint_a, endpoint_b: evaluation.candidate.endpoint_b, investment: evaluation.threshold, water }, &environment.catalog, cache, &mut energy)?;
    let material = organism.stored_material.take_one_unstructured_named(&resource_name)?; organism.structure = hypothetical; organism.usable_energy = energy; organism.add_transaction_stress(attempt.work_cost); let _ = material; Some(attempt)
}
...
pub(crate) fn try_combine(organism: &mut Organism, environment: &Environment, cache: &mut ConnectionCompatibilityCache) -> Option<CombineAttempt> {
    if !organism.structure.units.is_empty() && !organism.stored_material.is_empty() { if let Some(attempt) = try_combine_stored_unit(organism, environment, cache) { return Some(attempt); } }
    if organism.structure.units.len() < 2 { return None; }
    let catalog = &environment.catalog; let water = water_field_amount(environment, organism); let mut best: Option<(usize, usize, FormationEvaluation, f64)> = None;
    for ua in 0..organism.structure.units.len() { for ub in ua + 1..organism.structure.units.len() { for candidate in eligible_candidates(&organism.structure, ua, ub, catalog, cache) {
    ...
    let (ua, ub, evaluation, _) = best?; let mut energy = organism.usable_energy;
