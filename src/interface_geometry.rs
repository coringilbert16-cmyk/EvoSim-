...
pub fn boundary_length(part: &PlacedMaterialPart) -> f64 {
    match &part.form {
        ...
        Form::Line { length } if length.is_finite() && *length > 0.0 => *length,
        Form::Circle { .. } | Form::Fluid { .. } => 0.0,
        _ => world_polygon_vertices(part)
            .map(|v| polygon_perimeter(&v))
            .unwrap_or(0.0),
    }
}
pub fn shared_boundary_length(
    ...
    match (&a.form, &b.form) {
        (Form::Circle { radius: ar }, Form::Circle { radius: br }) => {
            ...
            let coincident = (a.placement.x - b.placement.x).hypot(a.placement.y - b.placement.y)
                <= tolerance.max(GEOMETRIC_EPSILON);
            if coincident && (ar - br).abs() <= tolerance.max(GEOMETRIC_EPSILON) {
                std::f64::consts::TAU * ar
            } else {
                0.0
            }
        }
        (Form::Line { .. }, Form::Line { .. }) => line_line_shared_length(a, b, tolerance),
        (Form::Line { .. }, polygon) => line_polygon_shared_length(a, b, polygon, tolerance),
        (polygon, Form::Line { .. }) => line_polygon_shared_length(b, a, polygon, tolerance),
...
fn line_line_shared_length(a: &PlacedMaterialPart, b: &PlacedMaterialPart, tolerance: f64) -> f64 {
    let (Some((a0, a1)), Some((b0, b1))) = (line_segment(a), line_segment(b)) else {
        return 0.0;
    };
    let ax = a1.0 - a0.0;
    let ay = a1.1 - a0.1;
    ...
    if cross(ax, ay, b0.0 - a0.0, b0.1 - a0.1).abs() > tolerance.max(GEOMETRIC_EPSILON) * length
        || cross(ax, ay, b1.0 - a0.0, b1.1 - a0.1).abs() > tolerance.max(GEOMETRIC_EPSILON) * length
    {
        return 0.0;
    }
    let ux = ax / length;
    ...
    (length.min(t0.max(t1)) - 0.0_f64.max(t0.min(t1))).max(0.0)
}
fn line_polygon_shared_length(
    line: &PlacedMaterialPart,
    polygon: &PlacedMaterialPart,
    _form: &Form,
    tolerance: f64,
) -> f64 {
    let Some((start, end)) = line_segment(line) else {
        return 0.0;
    };
    let Some(vertices) = world_polygon_vertices(polygon) else {
        return 0.0;
    };
    let mut total = 0.0;
    for (i, &edge_start) in vertices.iter().enumerate() {
        let edge_end = vertices[(i + 1) % vertices.len()];
        total += collinear_segment_overlap_length(start, end, edge_start, edge_end, tolerance)
    }
    total
}
fn collinear_segment_overlap_length(
    ...
    let eps = tolerance.max(GEOMETRIC_EPSILON) * length;
    if cross(ax, ay, b0.0 - a0.0, b0.1 - a0.1).abs() > eps
        || cross(ax, ay, b1.0 - a0.0, b1.1 - a0.1).abs() > eps
    {
        return 0.0;
    }
    let ux = ax / length;
...
fn polygon_shared_boundary_length(
    ...
    let (Some(av), Some(bv)) = (world_polygon_vertices(a), world_polygon_vertices(b)) else {
        return 0.0;
    };
    if polygons_interpenetrate(&av, &bv, tolerance) {
        return 0.0;
    }
    let mut total = 0.0;
    for (i, &a0) in av.iter().enumerate() {
        let a1 = av[(i + 1) % av.len()];
        if segment_length(a0, a1) <= GEOMETRIC_EPSILON {
            continue;
        }
        for (j, &b0) in bv.iter().enumerate() {
            let b1 = bv[(j + 1) % bv.len()];
            if segment_length(b0, b1) <= GEOMETRIC_EPSILON {
                continue;
            }
            total += collinear_segment_overlap_length(a0, a1, b0, b1, tolerance)
        }
    }
    total
}
...
fn polygons_have_transverse_boundary_crossing(
    ...
    for (i, &a0) in a.iter().enumerate() {
        ...
        for (j, &b0) in b.iter().enumerate() {
            ...
            if segments_cross_transversely(a0, a1, b0, b1, tolerance) {
                return true;
            }
        }
    }
...
fn polygon_has_strictly_interior_boundary_point(
    ...
    subject.iter().enumerate().any(|(i, &start)| {
        let end = subject[(i + 1) % subject.len()];
...
fn point_to_segment_distance(point: (f64, f64), start: (f64, f64), end: (f64, f64)) -> f64 {
    ...
    let ls = dx * dx + dy * dy;
    if ls <= GEOMETRIC_EPSILON * GEOMETRIC_EPSILON {
        return (point.0 - start.0).hypot(point.1 - start.1);
    }
    let t = (((point.0 - start.0) * dx + (point.1 - start.1) * dy) / ls).clamp(0.0, 1.0);
    let p = (start.0 + t * dx, start.1 + t * dy);
    (point.0 - p.0).hypot(point.1 - p.1)
}
...
mod tests {
    ...
    fn circle_boundary_length_is_exact_circumference() {
        let c = part(Form::Circle { radius: 2.0 }, 0.0, 0.0, 0.0);
        assert!((boundary_length(&c) - 4.0 * std::f64::consts::PI).abs() < 1e-12)
    }
    #[test]
    fn line_boundary_length_is_exact_length() {
        let l = part(Form::Line { length: 3.0 }, 0.0, 0.0, 0.0);
        assert_eq!(boundary_length(&l), 3.0)
    }
    #[test]
    fn rectangle_boundary_length_is_exact_perimeter() {
        ...
        assert!((boundary_length(&r) - 12.0).abs() < 1e-12)
    }
    #[test]
    ...
    #[test]
    fn line_segments_share_collinear_overlap() {
        let a = part(Form::Line { length: 4.0 }, 0.0, 0.0, 0.0);
        let b = part(Form::Line { length: 2.0 }, 1.0, 0.0, 0.0);
    ...
    fn fluid_has_no_boundary_length_without_authoritative_geometry() {
        ...
        assert_eq!(boundary_length(&f), 0.0);
        assert_eq!(shared_boundary_length(&f, &f, 0.0), 0.0)
    }
}
