...
/// Transfer unbonded material from one connected endpoint to the other.
///
/// The connection is the physical gate. Direction and amount are supplied by
/// the caller, so establishing a connection never causes material to move by
/// itself. The transfer is atomic: if validation fails, neither material store
/// is changed.
pub fn transfer_unbonded_material(
    connection: &CellConnection,
    sender_site: CellSiteRef,
    receiver_site: CellSiteRef,
    sender: &mut Material,
    receiver: &mut Material,
    amount: f64,
) -> Result<(), MaterialTransferError> {
    if !connection.connects(sender_site, receiver_site) {
        return Err(MaterialTransferError::EndpointsNotConnected);
    }
    if sender.bonded || receiver.bonded {
        return Err(MaterialTransferError::BondedMaterialCannotTransfer);
    }
    if !amount.is_finite() || amount <= 0.0 {
        return Err(MaterialTransferError::InvalidAmount);
    }
    if sender.total_amount() + 1e-12 < amount {
    ...
    let mut parts = std::mem::take(&mut receiver.parts);
    parts.extend(transferred.parts);
    receiver.parts = merge_parts(&parts);
    receiver.bonded = false;
...
mod tests {
    ...
    fn connected_transfer_moves_only_requested_amount() {
        ...
        let mut sender = Material::free_base("Carbon", 10.0);
        let mut receiver = Material::free_base("Nitrogen", 2.0);
        ...
        assert!((sender.total_amount() - 7.0).abs() < 1e-9);
        assert!((receiver.total_amount() - 5.0).abs() < 1e-9);
        assert_eq!(receiver.parts.len(), 2);
    }
    ...
    fn disconnected_endpoints_cannot_transfer() {
        let connection = connection();
        let mut sender = Material::free_base("Carbon", 10.0);
        let mut receiver = Material::free_base("Nitrogen", 0.0);
    ...
    fn bonded_material_cannot_transfer() {
        let connection = connection();
        let mut sender = Material { parts: vec![("Carbon".into(), 10.0)], bonded: true };
        let mut receiver = Material::free_base("Nitrogen", 0.0);
        ...
        assert!((sender.total_amount() - 10.0).abs() < 1e-9);
    ...
    fn insufficient_transfer_is_atomic() {
        let connection = connection();
        let mut sender = Material::free_base("Carbon", 2.0);
        let mut receiver = Material::free_base("Nitrogen", 1.0);
        let sender_before = sender.clone();
        let receiver_before = receiver.clone();
