...
impl StructuralBlueprint {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (i, connection) in self.connections.iter().enumerate() { connection.validate(self).map_err(|error| format!("connection {i}: {error}"))?; }
    ...
    pub fn realize(&self, catalog: &[BaseResource]) -> Result<OrganismStructure, String> {
        ...
        realized.insert(0, crate::construction_realization::realize_material(&mut structure, first, catalog)?);
        let mut frontier = vec![0usize];
        while let Some(current) = frontier.pop() {
            for (connection_index, connection) in self.connections.iter().enumerate().filter(|(_, connection)| connection.element_a == current || connection.element_b == current) {
                let next = if connection.element_a == current { connection.element_b } else { connection.element_a };
                if realized.contains_key(&next) { continue; }
                let element = self.elements.get(next).ok_or_else(|| "blueprint connection references missing element".to_string())?;
                let ids = crate::construction_realization::realize_material(&mut structure, element, catalog)?;
                realized.insert(next, ids);
                if realize_connection_groups(&mut structure, &realized, *connection, catalog).is_ok() {
