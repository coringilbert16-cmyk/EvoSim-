...
impl StructuralBlueprint {
    ...
    pub fn realize(&self, catalog: &[BaseResource]) -> Result<OrganismStructure, String> {
        ...
        for c in &self.connections {
            let a = s.connection_site(crate::structure::ConnectionSiteRef { unit_index: c.element_a, point_index: c.point_a }, catalog)
                .ok_or_else(|| format!("connection {c:?} references an invalid first site"))?;
            let b = s.connection_site(crate::structure::ConnectionSiteRef { unit_index: c.element_b, point_index: c.point_b }, catalog)
            ...
            crate::contact::try_add_bond(&mut s, crate::structure::Bond {
                unit_a: c.element_a,
                point_a: c.point_a,
                unit_b: c.element_b,
                point_b: c.point_b,
                strength,
                bond_energy: 0.0,
            }, catalog).map_err(|e| format!("connection {c:?} rejected by physical bond geometry: {e}"))?;
        }
        Ok(s)
    }
    pub fn is_connected(&self) -> bool {
        ...
        let mut stack = vec![0usize]; v[0] = true;
    ...
    fn core_is_connected(&self) -> bool {
        ...
        let mut stack = vec![self.core_elements[0]]; visited.insert(self.core_elements[0]);
        while let Some(cur) = stack.pop() {
            for c in &self.connections {
                let next = if c.element_a == cur { c.element_b } else if c.element_b == cur { c.element_a } else { continue };
...
mod tests {
    ...
    #[test] fn structured_hydrated_element_is_valid() {
        let b = StructuralBlueprint::new(vec![BlueprintElement { material: hydrated_carbon_water(), placement: Placement { x: 0.0, y: 0.0, rotation_radians: 0.0 } }], Vec::new());
        assert!(b.validate().is_ok()); assert_eq!(b.realize(&default_catalog()).unwrap().units.len(), 1);
    }
    #[test] fn disconnected_internal_composite_is_rejected() {
        let m = Material { parts: vec![("Carbon".into(), 1.0), ("Water".into(), 1.0), ("Nitrogen".into(), 1.0)], internal_bonds: vec![InternalBond { part_a: 0, part_b: 1 }] };
        let b = StructuralBlueprint::new(vec![BlueprintElement { material: m, placement: Placement { x: 0.0, y: 0.0, rotation_radians: 0.0 } }], Vec::new());
        assert!(!b.is_valid());
    }
    #[test] fn genome_core_must_be_connected() {
        let element = BlueprintElement { material: Material::free_base("Carbon", 1.0), placement: Placement { x: 0.0, y: 0.0, rotation_radians: 0.0 } };
        let b = StructuralBlueprint::with_core_elements(vec![element.clone(), element], Vec::new(), vec![0, 1]);
        assert!(!b.is_valid());
    }
}
