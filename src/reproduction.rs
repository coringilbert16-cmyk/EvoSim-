...
pub(crate) fn missing_construction_resources(
    parent: &Organism,
    environment: &Environment,
) -> Vec<String> {
    ...
    let child = developing_organism(construction);
    ...
    for resource in &environment.catalog {
        ...
        let mut candidate = child.clone();
        candidate.usable_energy = f64::MAX;
        ...
        .is_some()
        {
            missing.push(resource.name.clone());
        }
    }
    missing
}

pub(crate) fn environment_contains_material(
    environment: &Environment,
    ...
    environment.field.cells.iter().any(|cell| {
        cell.materials.iter().any(|material| {
            material.parts.iter().any(|(name, amount)| {
                *amount > 0.0 && resource_names.iter().any(|wanted| wanted == name)
            })
        }) || cell.physical_materials.iter().any(|physical| {
            physical.material.parts.iter().any(|(name, amount)| {
                *amount > 0.0 && resource_names.iter().any(|wanted| wanted == name)
            })
        })
    })
