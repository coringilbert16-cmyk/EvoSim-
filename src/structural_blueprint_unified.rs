...
pub struct BlueprintPlacement { pub x: f64, pub y: f64, #[serde(default)] pub rotation_radians: f64 }
...
pub struct BlueprintElement { pub material: Material, pub placement: BlueprintPlacement }
...
impl<'de> Deserialize<'de> for BlueprintElement {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error> where D: Deserializer<'de> {
        ...
        Ok(Self { material: stored.material, placement: BlueprintPlacement { x: stored.placement.x, y: stored.placement.y, rotation_radians: stored.placement.rotation_radians.unwrap_or(0.0) } })
    }
}
...
impl BlueprintElement {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if !self.placement.x.is_finite() || !self.placement.y.is_finite() { return Err("blueprint placement must be finite".into()); }
...
impl BlueprintConnection {
    pub fn canonical(self) -> Self { if self.element_a <= self.element_b { self } else { Self { element_a: self.element_b, element_b: self.element_a } } }
    fn validate(&self, b: &StructuralBlueprint) -> Result<(), String> {
        if self.element_a >= b.elements.len() || self.element_b >= b.elements.len() { return Err("references a missing element".into()); }
...
impl StructuralBlueprint {
    pub fn new(elements: Vec<BlueprintElement>, connections: Vec<BlueprintConnection>) -> Self { Self { elements, connections: Self::canonical_connections(connections), core_elements: default_core_elements() } }
    pub fn with_core_elements(elements: Vec<BlueprintElement>, connections: Vec<BlueprintConnection>, core_elements: Vec<usize>) -> Self { Self { elements, connections: Self::canonical_connections(connections), core_elements } }
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (i, e) in self.elements.iter().enumerate() { e.validate().map_err(|error| format!("element {i}: {error}"))?; }
        for (i, c) in self.connections.iter().enumerate() {
            c.validate(self).map_err(|error| format!("connection {i}: {error}"))?;
            if !connection_seen.insert((c.element_a, c.element_b)) { return Err("blueprint contains duplicate connections".into()); }
        }
    ...
    pub fn realize(&self, catalog: &[BaseResource]) -> Result<OrganismStructure, String> {
        ...
        self.realize_with_context(catalog, &mut ledger, &mut preview_energy).map(|(structure, _)| structure)
    }
    pub fn realize_with_context(&self, catalog: &[BaseResource], ledger: &mut EnergyLedger, energy: &mut f64) -> Result<(OrganismStructure, f64), String> {
        self.validate()?;
        ...
        while let Some(current) = queue.pop() {
            ...
            for connection in &self.connections {
                let neighbor = if connection.element_a == current { connection.element_b } else if connection.element_b == current { connection.element_a } else { continue };
                if !visited[neighbor] { visited[neighbor] = true; queue.push(neighbor); }
            }
        }
        if order.len() != self.elements.len() { return Err("blueprint realization stalled before all elements were constructed".into()); }
        let mut total_heat = 0.0;
        for index in order {
            let external = if index == 0 { Vec::new() } else {
            ...
            let (ids, heat) = crate::construction_runtime::realize_material_with_context(&mut structure, &self.elements[index], catalog, ledger, energy, &external)
    ...
    pub fn is_connected(&self) -> bool {
        ...
        while let Some(current) = stack.pop() { for connection in &self.connections { let next = if connection.element_a == current { connection.element_b } else if connection.element_b == current { connection.element_a } else { continue }; if next < visited.len() && !visited[next] { visited[next] = true; stack.push(next); } } }
    ...
    fn core_is_connected(&self) -> bool {
        ...
        while let Some(current) = stack.pop() { for connection in &self.connections { let next = if connection.element_a == current { connection.element_b } else if connection.element_b == current { connection.element_a } else { continue }; if core.contains(&next) && visited.insert(next) { stack.push(next); } } }
...
fn validate_element_contact(structure: &OrganismStructure, ids: &[usize], neighbors: &[Vec<usize>], catalog: &[BaseResource]) -> Result<(), String> {
    for &id in ids { if structure.units.get(id).is_none() { return Err("realized material references a missing constituent".to_string()); } }
    for group in neighbors {
        let min_distance = ids.iter().flat_map(|&a| group.iter().flat_map(move |&b| crate::contact::connection_pair_candidates(structure, a, b, catalog).into_iter().map(|candidate| candidate.distance))).fold(f64::INFINITY, f64::min);
        if min_distance > 1.0 { return Err(format!("realized material has no physical contact with a prescribed neighbor (minimum endpoint distance: {min_distance})")); }
    }
    Ok(())
}