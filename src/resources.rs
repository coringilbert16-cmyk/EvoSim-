...
impl Material{
 ...
 pub fn is_valid(&self)->bool{if self.composition.iter().any(|c|c.resource.is_empty()||!c.amount.is_finite()||c.amount<=0.0){return false}match &self.structure{None=>true,Some(s)=>s.is_valid()&&!s.constituents.is_empty()}}
...
pub fn combine_materials(_inputs:&[Material])->Material{panic!("combine_materials is retired: COMBINE must resolve physical attachment geometry before constructing structured Material")}
