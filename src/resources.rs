...
impl Form {
 ...
 /// A contact extent derived from fluid area for broad-phase/contact calculations.
 /// This is not a geometric shape and must never be treated as one.
 pub fn fluid_contact_radius(&self)->Option<f64>{match self{Self::Fluid{nominal_area}=>Some((nominal_area/std::f64::consts::PI).sqrt()),_=>None}}
...
#[cfg(test)]mod shape_tests{use super::*;#[test]fn catalog_still_constructs_with_seven_resources(){assert_eq!(default_catalog().len(),7)}#[test]fn every_catalog_resource_has_a_valid_shape(){for r in default_catalog(){assert!(r.shape.is_valid())}}#[test]fn fluid_has_no_rigid_bounding_radius(){let w=default_catalog().into_iter().find(|r|r.name=="Water").unwrap();assert_eq!(w.shape.form.rigid_bounding_radius(),None)}#[test]fn fluid_contact_extent_is_area_derived_not_rigid_geometry(){let w=default_catalog().into_iter().find(|r|r.name=="Water").unwrap();let radius=w.shape.form.fluid_contact_radius().unwrap();assert!(radius.is_finite()&&radius>0.0)}#[test]fn rigid_forms_have_rigid_bounding_radii(){for r in default_catalog(){if !matches!(r.shape.form,Form::Fluid{..}){assert!(r.shape.form.rigid_bounding_radius().unwrap().is_finite())}}}}