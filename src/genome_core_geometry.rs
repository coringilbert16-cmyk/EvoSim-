...
fn point_inside_form(x: f64, y: f64, form: &Form, placement: Placement) -> bool {
    ...
    match form {
        ...
        Form::Rectangle { width, height } => local_x.abs() <= *width / 2.0 && local_y.abs() <= *height / 2.0,
        Form::RegularPolygon { sides, radius } => polygon_contains(local_x, local_y, &regular_polygon_vertices(*sides, *radius)),
...
pub fn measure_largest_cavity(structure: &OrganismStructure, catalog: &[BaseResource]) -> Result<CavityMeasurement, String> {
    let threshold = cavity_threshold_area(catalog)?;
    if structure.units.is_empty() {
        return Ok(CavityMeasurement { largest_enclosed_area: 0.0, threshold_area: threshold, qualifies: false });
    }
    ...
    for iy in 0..ny {
        for ix in 0..nx {
            ...
            occupied[iy * nx + ix] = structure.units.iter().any(|unit| {
                unit.shape(catalog).map(|shape| point_inside_form(x, y, &shape.form, unit.placement)).unwrap_or(false)
            });
        }
    }
    ...
    for iy in 0..ny {
        for ix in 0..nx {
            if ix != 0 && iy != 0 && ix + 1 != nx && iy + 1 != ny { continue; }
            let index = iy * nx + ix;
    ...
    for iy in 0..ny {
        for ix in 0..nx {
            let start = iy * nx + ix;
            if occupied[start] || exterior[start] || visited[start] { continue; }
            visited[start] = true;
            ...
            while let Some((x, y)) = component.pop_front() {
                ...
                for (dx, dy) in [(1_i32, 0_i32), (-1, 0), (0, 1), (0, -1)] {
                    let nxp = x as i32 + dx;
                    let nyp = y as i32 + dy;
                    if nxp < 0 || nyp < 0 || nxp >= nx as i32 || nyp >= ny as i32 { continue; }
    ...
    Ok(CavityMeasurement { largest_enclosed_area: largest_area, threshold_area: threshold, qualifies: largest_area > threshold })
}

fn flood_empty(occupied: &[bool], exterior: &mut [bool], width: usize, height: usize, queue: &mut VecDeque<(usize, usize)>) {
