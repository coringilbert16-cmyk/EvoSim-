...
//! bonds belong together. When a material has already been physically realized,
//! its constituent placements travel with it through storage unchanged.
...
pub(crate) struct MaterialStorage {
    ...
    /// Same-index physical realization for each entry in `materials`.
    /// `None` is retained for legacy/logical material until a physical
    /// realization is explicitly supplied; no geometry is invented here.
    #[serde(default)]
