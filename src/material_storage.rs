...
impl MaterialStorage {
    ...
    pub(crate) fn store_physical(
        ...
        { Some(instance) => instance, None => return false };
        if material.parts.is_empty() || !material.is_valid() || !Self::is_discrete(&material) { return false; }
        self.normalize_legacy_alignment();
        self.materials.push(material);
    ...
    pub(crate) fn peek_one_unstructured(&self) -> Option<Material> {
        self.materials.iter().find(|material| !material.has_internal_structure() && !material.is_empty()).cloned()
    }
    ...
    pub(crate) fn peek_matching_physical(&self, target: &Material) -> Option<PhysicalMaterial> {
        ...
        self.physical_instances.get(index).and_then(Clone::clone)
    ...
    pub(crate) fn take_one_unstructured_named(&mut self, name: &str) -> Option<Material> {
        let index = self.materials.iter().position(|material| {
            !material.has_internal_structure() && !material.is_empty() && material.parts.len() == 1
                && material.parts[0].0 == name && (material.parts[0].1 - 1.0).abs() <= MATERIAL_EPSILON
        })?;
        self.normalize_legacy_alignment();
        self.physical_instances.swap_remove(index);
...
mod tests {
    ...
    fn free_material_is_stored_as_discrete_units() {
        ...
        assert_eq!(storage.materials.len(), 3); assert_eq!(storage.count_unstructured(), 3); assert_eq!(storage.total_amount(), 3.0);
    }
    #[test]
    fn peek_does_not_consume_free_material() {
        let mut storage = MaterialStorage::default(); storage.store(Material::free_base("Carbon", 1.0));
        let peeked = storage.peek_one_unstructured().expect("stored unit");
        assert_eq!(peeked, Material::free_base("Carbon", 1.0)); assert_eq!(storage.count_unstructured(), 1);
    }
    #[test]
    fn structured_material_is_stored_intact() {
        let mut storage = MaterialStorage::default(); let m = compound(); assert!(storage.store(m.clone()));
        assert_eq!(storage.materials, vec![m]); assert_eq!(storage.count_structured(), 1);
    }
    #[test]
    fn structured_material_is_taken_intact() {
        let mut storage = MaterialStorage::default(); let m = compound(); storage.store(m.clone());
        assert_eq!(storage.take_matching(&m), Some(m.clone())); assert!(storage.is_empty());
    }
    #[test]
    fn physical_material_is_stored_in_intrinsic_frame() {
        let mut storage = MaterialStorage::default(); let m = compound();
        let placements = vec![Placement { x: 10.0, y: 20.0, rotation_radians: 0.5 }, Placement { x: 10.838, y: 20.0, rotation_radians: 0.75 }];
        ...
        assert_eq!(restored.material, m); let intrinsic = restored.placements.expect("intrinsic realization");
        assert!(intrinsic[0].x.abs() <= 1e-12); assert!(intrinsic[0].y.abs() <= 1e-12); assert!(intrinsic[0].rotation_radians.abs() <= 1e-12);
        assert!((intrinsic[1].x.hypot(intrinsic[1].y) - 0.838).abs() <= 1e-9); assert!((intrinsic[1].rotation_radians - 0.25).abs() <= 1e-12); assert!(restored.internal_connections.is_some());
    }
    #[test]
    fn physical_single_constituent_is_stored_with_its_realization() {
        let mut storage = MaterialStorage::default(); let m = Material::free_base("Carbon", 1.0);
        let placements = vec![Placement { x: 10.0, y: 20.0, rotation_radians: 0.25 }];
        assert!(storage.store_physical(m.clone(), placements, &catalog())); let restored = storage.take_matching_physical(&m).expect("stored instance");
        assert_eq!(restored.material, m); let intrinsic = restored.placements.expect("intrinsic realization");
        assert!(intrinsic[0].x.abs() <= 1e-12); assert!(intrinsic[0].y.abs() <= 1e-12); assert!(intrinsic[0].rotation_radians.abs() <= 1e-12); assert!(restored.internal_connections.is_some());
    }
    #[test]
    fn storage_never_merges_independent_atoms() { let mut storage = MaterialStorage::default(); storage.store(Material::free_base("Carbon", 1.0)); storage.store(Material::free_base("Carbon", 1.0)); assert_eq!(storage.materials.len(), 2); }
    #[test]
    fn storage_never_opens_a_compound() { let mut storage = MaterialStorage::default(); let m = compound(); storage.store(m.clone()); assert!(storage.take_unstructured(1).is_none()); assert_eq!(storage.materials, vec![m]); }
    #[test]
    fn fractional_material_is_rejected_at_storage_boundary() { let mut storage = MaterialStorage::default(); assert!(!storage.store(Material::free_base("Carbon", 1.5))); assert!(storage.is_empty()); }
}
