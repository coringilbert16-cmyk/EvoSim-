...
impl StructuralUnit {
    ...
    pub fn properties(&self, catalog: &[BaseResource]) -> Option<crate::resources::ResourceProperties> {
        if !self.material.is_valid()
            || !self.material.parts.iter().all(|(name, _)| catalog.iter().any(|base| base.name == *name))
        {
            return None;
        }
        Some(self.material.weighted_properties(catalog))
    }
    fn external_geometry_resource<'a>(&self, catalog: &'a [BaseResource]) -> Option<&'a BaseResource> {
    ...
    pub fn connection_sites(&self, catalog: &[BaseResource]) -> Option<ConnectionSites> {
        ...
        if !self.material.has_internal_structure() {
            ...
            return catalog.iter().find(|base| base.name == *name).map(|base| base.shape.connection_sites());
        }
        self.external_geometry_resource(catalog).map(|base| base.shape.connection_sites())
    }
    pub fn shape<'a>(&'a self, catalog: &'a [BaseResource]) -> Option<&'a crate::resources::Shape> {
        ...
        if !self.material.has_internal_structure() {
            ...
            return catalog.iter().find(|base| base.name == *name).map(|base| &base.shape);
        }
        self.external_geometry_resource(catalog).map(|base| &base.shape)
    }
...
impl<'de> Deserialize<'de> for StructuralUnit {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let m = match s.material {
            ...
            None => Material::free_base(s.resource_name.ok_or_else(|| serde::de::Error::custom("structural unit has no material"))?, 1.0),
        };
...
impl<'de> Deserialize<'de> for BondEndpoint {
    ...
    {
        #[derive(Deserialize)]
        struct Stored { constituent_id: Option<PhysicalConstituentId>, unit_index: Option<usize>, location: ConnectionEndpoint }
        let s = Stored::deserialize(deserializer)?;
        let id = s.constituent_id.unwrap_or_else(|| PhysicalConstituentId(s.unit_index.map(|i| u64::MAX - i as u64).unwrap_or(0)));
        Ok(Self { constituent_id: id, location: s.location })
    }
}
impl ConnectionEndpoint {
    pub fn region(self, unit: &StructuralUnit, catalog: &[BaseResource]) -> Option<ConnectionRegion> {
        match self {
            Self::Corner { point_index } => {
                ...
                crate::connection_geometry::transform_polygon_vertex(shape, point_index, unit.placement.x, unit.placement.y, unit.placement.rotation_radians).map(ConnectionRegion::Corner)
            }
            Self::LineEndpoint { point_index } => {
                let shape = unit.shape(catalog)?;
                crate::connection_geometry::transform_line_endpoint(shape, point_index, unit.placement.x, unit.placement.y, unit.placement.rotation_radians).map(ConnectionRegion::Corner)
            }
    ...
    pub fn world_point(self, unit: &StructuralUnit, catalog: &[BaseResource]) -> Option<WorldConnectionPoint> {
        match self {
            Self::Corner { point_index } => {
                let shape = unit.shape(catalog)?;
                crate::connection_geometry::transform_polygon_vertex(shape, point_index, unit.placement.x, unit.placement.y, unit.placement.rotation_radians)
            }
            Self::LineEndpoint { point_index } => {
                let shape = unit.shape(catalog)?;
                crate::connection_geometry::transform_line_endpoint(shape, point_index, unit.placement.x, unit.placement.y, unit.placement.rotation_radians)
            }
            Self::Boundary { angle_radians } => {
                ...
                Some(crate::connection_geometry::transform_derived_point(radius * nx, radius * ny, nx, ny, unit.placement.x, unit.placement.y, unit.placement.rotation_radians))
            }
            Self::Fluid { x, y } => Some(crate::connection_geometry::transform_derived_point(x, y, 0.0, 0.0, unit.placement.x, unit.placement.y, unit.placement.rotation_radians)),
        }
    }
    pub fn same_location(self, other: Self) -> bool {
        match (self, other) {
            (Self::Corner { point_index: a }, Self::Corner { point_index: b }) => a == b,
            (Self::LineEndpoint { point_index: a }, Self::LineEndpoint { point_index: b }) => a == b,
...
impl Bond {
    pub fn touches(&self, u: PhysicalConstituentId, location: ConnectionEndpoint) -> bool {
        (self.endpoint_a.constituent_id == u && self.endpoint_a.location.same_location(location)) || (self.endpoint_b.constituent_id == u && self.endpoint_b.location.same_location(location))
    }
    pub fn has_same_identity(&self, o: &Bond) -> bool {
        (self.endpoint_a.constituent_id == o.endpoint_a.constituent_id && self.endpoint_a.location.same_location(o.endpoint_a.location) && self.endpoint_b.constituent_id == o.endpoint_b.constituent_id && self.endpoint_b.location.same_location(o.endpoint_b.location)) || (self.endpoint_a.constituent_id == o.endpoint_b.constituent_id && self.endpoint_a.location.same_location(o.endpoint_b.location) && self.endpoint_b.constituent_id == o.endpoint_a.constituent_id && self.endpoint_b.location.same_location(o.endpoint_a.location))
    }
    pub fn is_valid(&self, connection_is_valid: impl Fn(BondEndpoint) -> bool) -> bool {
        self.endpoint_a.constituent_id.0 != 0 && self.endpoint_b.constituent_id.0 != 0 && self.endpoint_a.constituent_id != self.endpoint_b.constituent_id && self.bond_energy.is_finite() && self.bond_energy >= 0.0 && connection_is_valid(self.endpoint_a) && connection_is_valid(self.endpoint_b)
    }
}
fn units_strictly_overlap(a: &StructuralUnit, b: &StructuralUnit, catalog: &[BaseResource]) -> bool {
    let (Some(form_a), Some(form_b)) = (a.shape(catalog), b.shape(catalog)) else { return false; };
    let pa = crate::material_geometry::PlacedMaterialPart { part_index: 0, form: form_a.form.clone(), placement: a.placement };
    let pb = crate::material_geometry::PlacedMaterialPart { part_index: 1, form: form_b.form.clone(), placement: b.placement };
    ...
    let (sx, sy) = if distance > 1e-12 { (dx / distance, dy / distance) } else { (1.0, 0.0) };
    let scale = pa.form.bounding_radius().max(pb.form.bounding_radius()).max(1.0);
    let epsilon = 1e-8 * scale;
    let shifted = crate::material_geometry::PlacedMaterialPart { part_index: 0, form: pa.form.clone(), placement: Placement { x: a.placement.x - sx * epsilon, y: a.placement.y - sy * epsilon, rotation_radians: a.placement.rotation_radians } };
...
impl<'de> Deserialize<'de> for PhysicalConstituentGraph {
    ...
    {
        #[derive(Deserialize)]
        struct Stored { units: Vec<StructuralUnit>, bonds: Vec<Bond>, #[serde(default)] next_constituent_id: u64 }
        ...
        for unit in &mut s.units {
            ...
            if id.0 == 0 || used.contains(&id) {
                while next == 0 || used.contains(&PhysicalConstituentId(next)) {
                    next = next.checked_add(1).ok_or_else(|| serde::de::Error::custom("physical constituent ID space exhausted"))?
                }
        ...
        if let Some(max_id) = used.iter().map(|id| id.0).max() { next = next.max(max_id.saturating_add(1)).max(1) }
        for bond in &mut s.bonds {
            for endpoint in [&mut bond.endpoint_a, &mut bond.endpoint_b] {
                if endpoint.constituent_id.0 >= u64::MAX.saturating_sub(s.units.len() as u64) {
                    ...
                    if let Some(id) = s.units.get(legacy_index).map(|u| u.physical_id) { endpoint.constituent_id = id } else { return Err(serde::de::Error::custom("bond references missing legacy unit")); }
                }
            }
            if !s.units.iter().any(|u| u.physical_id == bond.endpoint_a.constituent_id) || !s.units.iter().any(|u| u.physical_id == bond.endpoint_b.constituent_id) {
                return Err(serde::de::Error::custom("bond references missing physical constituent"));
            }
        }
        Ok(Self { units: s.units, bonds: s.bonds, next_constituent_id: next })
    }
}
impl PhysicalConstituentGraph {
    pub fn new() -> Self { Self { units: Vec::new(), bonds: Vec::new(), next_constituent_id: 1 } }
    ...
    pub fn add_unit(&mut self, mut u: StructuralUnit) -> usize {
        ...
        u.physical_id = id; self.units.push(u); index
    }
    pub fn physical_id(&self, unit_index: usize) -> Option<PhysicalConstituentId> { self.units.get(unit_index).map(|u| u.physical_id) }
    pub fn unit_index(&self, id: PhysicalConstituentId) -> Option<usize> { self.units.iter().position(|u| u.physical_id == id) }
    pub fn physical_constituent_ids(&self) -> Vec<PhysicalConstituentId> { self.units.iter().map(|u| u.physical_id).collect() }
    fn endpoint_index(&self, e: BondEndpoint) -> Option<usize> { self.unit_index(e.constituent_id) }
    pub fn is_valid_bond(&self, b: &Bond, c: &[BaseResource]) -> bool {
        ...
        if !b.is_valid(|e| self.endpoint_index(e).and_then(|i| e.location.world_point(self.units.get(i)?, c)).is_some()) { return false; }
        if units_strictly_overlap(&self.units[a], &self.units[d], c) { return false; }
        let Some(pa) = self.units[a].properties(c) else { return false; };
        let Some(pb) = self.units[d].properties(c) else { return false; };
    ...
    pub fn available_connection_sites(&self, c: &[BaseResource]) -> Vec<ConnectionSiteRef> {
        ...
        for i in 0..self.units.len() {
            match self.units[i].connection_sites(c) {
                Some(ConnectionSites::Corners(points)) | Some(ConnectionSites::Endpoints(points)) => {
    ...
    pub fn connected_components(&self) -> Vec<Vec<usize>> {
        ...
        for b in &self.bonds {
            let (Some(a), Some(d)) = (self.unit_index(b.endpoint_a.constituent_id), self.unit_index(b.endpoint_b.constituent_id)) else { continue; };
            adjacency[a].push(d); adjacency[d].push(a)
        }
        let mut visited = vec![false; self.units.len()]; let mut out = Vec::new();
        for start in 0..self.units.len() {
            if visited[start] { continue; }
            let mut stack = vec![start]; visited[start] = true; let mut component = Vec::new();
            while let Some(u) = stack.pop() { component.push(u); for &next in &adjacency[u] { if !visited[next] { visited[next] = true; stack.push(next) } } }
            component.sort_unstable(); out.push(component)
        }
        out
    }
    pub fn component_connection_sites(&self, component: &[usize], catalog: &[BaseResource]) -> Vec<ConnectionSiteRef> {
        let set: HashSet<usize> = component.iter().copied().collect();
        self.available_connection_sites(catalog).into_iter().filter(|s| set.contains(&s.unit_index)).collect()
    }
    pub fn connection_load(&self, u: usize, location: ConnectionEndpoint, _c: &[BaseResource]) -> f64 {
        let Some(id) = self.physical_id(u) else { return 0.0; };
        self.bonds.iter().filter(|b| b.touches(id, location)).map(|b| crate::combine::experimental_bond_strength(b.bond_energy)).sum()
    }
    pub fn connection_count(&self, u: usize, location: ConnectionEndpoint) -> usize {
        let Some(id) = self.physical_id(u) else { return 0; };
        self.bonds.iter().filter(|b| b.touches(id, location)).count()
    }
    ...
    pub fn disconnect_point(&mut self, u: usize, location: ConnectionEndpoint) -> Vec<Bond> {
        let Some(id) = self.physical_id(u) else { return Vec::new(); };
        let mut out = Vec::new(); let mut i = 0;
        while i < self.bonds.len() { if self.bonds[i].touches(id, location) { out.push(self.bonds.remove(i)) } else { i += 1 } }
        out
    }
        ...
        for b in &self.bonds {
            for e in [b.endpoint_a, b.endpoint_b] {
                if let Some(u) = self.unit_index(e.constituent_id) {
                    if !out.iter().any(|(i, p): &(usize, ConnectionEndpoint)| *i == u && p.same_location(e.location)) { out.push((u, e.location)) }
                }
            }
        }
        out
...
pub fn formation_threshold(a: f64, b: f64, la: f64, lb: f64) -> f64 {
    let la = la.max(0.0); let lb = lb.max(0.0); ((a + b) / 2.0) * (1.0 + la.sqrt() + lb.sqrt())
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn physical_constituents_receive_stable_ids() {
        ...
        let b = s.add_unit(StructuralUnit::new("Hydrogen", Placement { x: 1.0, y: 0.0, rotation_radians: 0.0 }));
        assert_ne!(s.physical_id(a), s.physical_id(b));
    }
    #[test]
    fn bond_identity_uses_physical_ids() {
        let a = PhysicalConstituentId(11); let b = PhysicalConstituentId(22);
        let x = Bond { endpoint_a: BondEndpoint::new(a, ConnectionEndpoint::Corner { point_index: 0 }), endpoint_b: BondEndpoint::new(b, ConnectionEndpoint::Corner { point_index: 1 }), strength: 0.5, bond_energy: 1.0 };
        let y = Bond { endpoint_a: BondEndpoint::new(a, ConnectionEndpoint::Corner { point_index: 0 }), endpoint_b: BondEndpoint::new(b, ConnectionEndpoint::Corner { point_index: 1 }), strength: 0.5, bond_energy: 1.0 };
        assert!(x.has_same_identity(&y));
    }
    #[test]
    fn structural_unit_serialization_uses_material_directly() {
        let unit = StructuralUnit::new("Carbon", Placement { x: 0.0, y: 0.0, rotation_radians: 0.0 });
    ...
    fn composite_material_is_accepted_as_structural_material() {
        let material = Material { parts: vec![("Carbon".into(), 2.0), ("Hydrogen".into(), 3.0)], internal_bonds: vec![] };
        let unit = StructuralUnit::from_material(material.clone(), Placement { x: 0.0, y: 0.0, rotation_radians: 0.0 }).unwrap();
