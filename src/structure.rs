...
impl OrganismStructure{
 ...
 /// A connection endpoint is a physical attachment region, never a bond slot.
 /// Whether another bond fits is decided only from the proposed bond geometry.
 pub fn is_valid_bond(&self,b:&Bond,c:&[BaseResource])->bool{if !b.is_valid(self.units.len(),|e|self.endpoint_is_valid(e,c)){return false}if units_strictly_overlap(&self.units[b.endpoint_a.unit_index],&self.units[b.endpoint_b.unit_index],c){return false}if !crate::contact::bond_geometry_is_valid(self,b,c){return false}let Some(a)=self.units[b.endpoint_a.unit_index].properties(c)else{return false};let Some(d)=self.units[b.endpoint_b.unit_index].properties(c)else{return false};let strength=crate::combine::bond_strength(a,d);strength.is_finite()&&(0.0..=1.0).contains(&strength)}
 ...
 pub fn connection_site(&self,s:ConnectionSiteRef,c:&[BaseResource])->Option<ConnectionPoint>{match self.units.get(s.unit_index)?.connection_sites(c)?{ConnectionSites::Corners(p)=>p.get(s.point_index).copied(),_=>None}}
 /// Returns physical connection regions, not regions with unused numerical capacity.
 pub fn available_connection_sites(&self,c:&[BaseResource])->Vec<ConnectionSiteRef>{let mut out=Vec::new();for i in 0..self.units.len(){let Some(ConnectionSites::Corners(points))=self.units[i].connection_sites(c)else{continue};for j in 0..points.len(){out.push(ConnectionSiteRef{unit_index:i,point_index:j})}}out}
 ...
 pub fn loaded_points(&self)->Vec<(usize,ConnectionEndpoint)>{let mut out:Vec<(usize,ConnectionEndpoint)>=Vec::new();for b in &self.bonds{for e in[b.endpoint_a,b.endpoint_b]{if !out.iter().any(|(u,p)|*u==e.unit_index&&p.same_location(e.location)){out.push((e.unit_index,e.location))}}}out}
}

pub fn formation_threshold(a:f64,b:f64,la:f64,lb:f64)->f64{let la=la.max(0.0);let lb=lb.max(0.0);((a+b)/2.0)*(1.0+la.sqrt()+lb.sqrt())}

#[cfg(test)]
mod tests{use super::*;use crate::resources::default_catalog;
...
#[test]fn composite_identity_is_preserved(){let m=crate::resources::Material{parts:vec![("Carbon".into(),1.0),("Hydrogen".into(),1.0)],internal_bonds:vec![crate::resources::InternalBond{part_a:0,part_b:1}]};let u=StructuralUnit::from_material(m.clone(),Placement{x:0.0,y:0.0,rotation_radians:0.0}).unwrap();assert_eq!(u.material.material(),&m);assert!(matches!(u.connection_sites(&default_catalog()),Some(ConnectionSites::Corners(_))))}
#[test]fn connection_count_is_observation_only(){let catalog=default_catalog();let mut s=OrganismStructure::new();let a=s.add_unit(StructuralUnit::new("Carbon",Placement{x:0.0,y:0.0,rotation_radians:0.0}));let b=s.add_unit(StructuralUnit::new("Carbon",Placement{x:1.0,y:0.0,rotation_radians:0.0}));let bond=Bond{endpoint_a:BondEndpoint{unit_index:a,location:ConnectionEndpoint::Corner{point_index:0}},endpoint_b:BondEndpoint{unit_index:b,location:ConnectionEndpoint::Corner{point_index:0}},strength:0.5,bond_energy:1.0};s.push_bond_unchecked(bond);assert_eq!(s.connection_count(a,ConnectionEndpoint::Corner{point_index:0}),1);s.push_bond_unchecked(bond);assert_eq!(s.connection_count(a,ConnectionEndpoint::Corner{point_index:0}),2)}
}