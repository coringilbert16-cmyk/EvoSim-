...
impl Organism{pub(crate)fn store_material(&mut self,material:Material)->bool{self.stored_material.store(material)}pub(crate)fn structural_mass(&self,catalog:&[BaseResource])->f64{self.structure.units.iter().map(|unit|unit.material.mass(catalog)).sum()}pub(crate)fn add_transaction_stress(&mut self,heat:f64){if heat.is_finite()&&heat>0.0{self.stress+=heat}}pub(crate)fn apply_stress_damage(&mut self,environment:&Environment,ledger:&mut EnergyLedger)->bool{if !self.stress.is_finite()||!self.stress_threshold.is_finite(){return true}while self.stress>=self.stress_threshold.max(MIN_STRESS_THRESHOLD){let threshold=self.stress_threshold.max(MIN_STRESS_THRESHOLD);self.stress-=threshold;self.stress_threshold=(threshold*STRESS_THRESHOLD_DECAY).max(MIN_STRESS_THRESHOLD);if self.structure.bonds.is_empty(){return true}if !crate::transformation::resolve_stress_break(self,environment,ledger){break}}false}}
#[derive(Serialize,Deserialize,Clone)]pub(crate)struct Environment{pub(crate)width:f64,pub(crate)height:f64,pub(crate)catalog:Vec<BaseResource>,pub(crate)field:ActiveMaterialField,pub(crate)vents:Vec<Vent>}
