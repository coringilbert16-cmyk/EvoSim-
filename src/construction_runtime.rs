...
pub(crate) fn candidate_placements(
    ...
    for &target in targets {
        ...
        for te in target_endpoints {
            ...
            match (&resource.shape.form, te) {
                (
                    Form::Rectangle { .. } | Form::RegularPolygon { .. } | Form::Polygon { .. },
                    ConnectionEndpoint::Corner { point_index: target_index },
                ) => {
                    ...
                    for candidate_index in 0..candidate_count {
                        for rotation in crate::rigid_boundary::corner_alignment_rotations(
                            &resource.shape, candidate_index, target_shape, target_index,
                            unit.placement.rotation_radians,
                        ) {
                            let Some(local) = crate::rigid_boundary::world_vertex(
                                &resource.shape, candidate_index,
                                Placement { x: 0.0, y: 0.0, rotation_radians: rotation },
                            ) else { continue; };
                            out.push(Placement {
                                x: tp.x - local.0,
                                y: tp.y - local.1,
                ...
                (
                    ...
                    let Some(target_normal) = crate::rigid_boundary::corner_normal(target_shape, target_index) else { continue; };
                ...
                (
                    Form::Line { length: candidate_length },
                    ConnectionEndpoint::LineEndpoint { point_index: target_index },
                ) => {
                    ...
                    for candidate_index in 0..2 {
                        ...
                        for rotation in crate::rigid_boundary::line_endpoint_alignment_rotations(
                            candidate_index, target_index, unit.placement.rotation_radians,
                        ) {
                            let (s, c) = rotation.sin_cos();
    ...
    out.sort_by(|a, b| {
        (a.x - anchor.x).hypot(a.y - anchor.y)
...
fn neighbors(material: &Material, part: usize, assigned: &[Option<usize>]) -> Vec<usize> {
    material.internal_bonds.iter().filter_map(|b| {
        if b.part_a == part { assigned[b.part_b] }
        else if b.part_b == part { assigned[b.part_a] }
        else { None }
    }).collect()
}
    ...
    for &new_id in assigned.iter().flatten() {
        for &target in group {
            ...
            let Some(attempt) = combine_specific_pair(
                &mut candidate, new_id, target, catalog, 0.0, &mut cache,
                &mut candidate_ledger, &mut candidate_energy,
            ) else { continue; };
            if let Some(result) = solve_external_groups(
                group_index + 1, &candidate, &candidate_ledger, candidate_energy,
                assigned, external, catalog, heat + attempt.work_cost,
            ) { return Some(result); }
        }
    }
    None
    ...
    for candidate_placement in candidate_placements(structure, resource, anchor, &targets, catalog) {
        ...
        for bond in material.internal_bonds.iter().filter(|b| b.part_a == part || b.part_b == part) {
            ...
            match combine_specific_pair(
                &mut candidate, a, b, catalog, 0.0, &mut cache,
                &mut candidate_ledger, &mut candidate_energy,
            ) {
        ...
        if let Some(result) = solve_parts(
            part + 1, &candidate, &candidate_ledger, candidate_energy,
            &candidate_assigned, material, anchor, catalog, external, candidate_heat,
        ) { return Some(result); }
    }
    None
}
