...
fn candidate_placements(
    ...
    if let Some(vertices) = resource.shape.form.polygon_vertices() {
        ...
        if width.is_finite() && width > 0.0 && height.is_finite() && height > 0.0 {
            out.extend([
                Placement { x: anchor.x + width, y: anchor.y, rotation_radians: anchor.rotation_radians },
                Placement { x: anchor.x - width, y: anchor.y, rotation_radians: anchor.rotation_radians },
                Placement { x: anchor.x, y: anchor.y + height, rotation_radians: anchor.rotation_radians },
                Placement { x: anchor.x, y: anchor.y - height, rotation_radians: anchor.rotation_radians },
            ]);
        }
    }
    ...
    out.dedup_by(|a, b| {
        (a.x - b.x).abs() <= 1e-10 && (a.y - b.y).abs() <= 1e-10 && (a.rotation_radians - b.rotation_radians).abs() <= 1e-10
    });
    out
}
