...
impl EnergyLedgerAuthority for EnergyLedger {
    ...
    fn settle_combine_holder(&mut self, holder: &mut f64, interaction: f64, investment: f64, work: f64) -> bool {
        ...
        let net = interaction - investment - work; let next = *holder + net;
        ...
        if !self.settle_transaction(tx) { return false; } *holder = next.max(0.0); true
    }
    fn settle_break_holder(&mut self, holder: &mut f64, bond_energy: f64, interaction: f64, work: f64) -> bool {
        ...
        let net = bond_energy + interaction - work; if *holder + EPSILON < (-net).max(0.0) { return false; }
        let next = *holder + net; if !next.is_finite() || next < -EPSILON { return false; }
    ...
    fn settle_maintenance(&mut self, holder: &mut f64, paid: f64) -> bool {
        ...
        let next = *holder - paid; let tx = EnergyTransaction { reason: EnergyReason::Maintenance, potential_released: 0.0, usable_delta: -paid, structural_delta: 0.0, heat_dissipated: paid };
        if !self.settle_transaction(tx) { return false; } *holder = next.max(0.0); true
    }
    fn settle_decomposition(&mut self, holder: &mut f64, bond_energy: f64, interaction: f64, work: f64) -> bool {
        if !holder.is_finite() || *holder < -EPSILON || !bond_energy.is_finite() || !interaction.is_finite() || !work.is_finite() || bond_energy < 0.0 || work < 0.0 { return false; }
    ...
    fn transfer(&mut self, from: &mut f64, to: &mut f64, amount: f64) -> bool {
        ...
        let next_from = *from - amount; let next_to = *to + amount; if next_from < -EPSILON || !next_to.is_finite() { return false; }
        *from = next_from.max(0.0); *to = next_to; true
    }
}

#[cfg(test)]
mod tests { use super::*; #[test] fn combine_transaction_balances() { let tx = EnergyTransaction { reason: EnergyReason::Combine, potential_released: 5.0, usable_delta: 2.0, structural_delta: 2.0, heat_dissipated: 1.0 }; assert!(tx.balanced()); } #[test] fn transfer_conserves_holder_sum() { let mut ledger = EnergyLedger::default(); let mut a = 10.0; let mut b = 2.0; assert!(ledger.transfer(&mut a, &mut b, 4.0)); assert_eq!(a + b, 12.0); } }
