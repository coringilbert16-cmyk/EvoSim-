...
/// cavity. Cache validity is derived from the authoritative realized structure
/// and the environmental material actually sampled at the realized cavity
/// boundary; no synthetic revision counter is required.
pub(crate) fn update_organism_harmonics(
    organism: &mut crate::state::Organism,
    environment: &crate::state::Environment,
) {
    let structure_key = structure_fingerprint(&organism.structure);
    let boundary_units = if organism
        .harmonic_cache
        .as_ref()
        .is_some_and(|cache| cache.structure_key == structure_key)
    {
        organism
            .harmonic_cache
            .as_ref()
            .map(|cache| cache.boundary_units.clone())
            .unwrap_or_default()
    } else {
    ...
    if organism.harmonic_cache.as_ref().is_some_and(|cache| {
        cache.structure_key == structure_key && cache.environment_key == environment_key
    }) {
    ...
    let spectrum = genome_cavity_spectrum(
        &organism.structure,
        &environment.catalog,
        &environment.field,
        &boundary_units,
    );
    organism.harmonic_spectrum = spectrum.clone();
    organism.harmonic_cache = Some(crate::state::HarmonicCache {
        structure_key,
        environment_key,
        boundary_units,
        spectrum,
...
fn structure_fingerprint(structure: &crate::structure::OrganismStructure) -> u64 {
    use std::hash::{Hash, Hasher};
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    structure.units.len().hash(&mut hasher);
    for unit in &structure.units {
        unit.material.parts.len().hash(&mut hasher);
        for (name, amount) in &unit.material.parts {
            name.hash(&mut hasher);
            amount.to_bits().hash(&mut hasher);
        }
        unit.material.internal_bonds.len().hash(&mut hasher);
        for bond in &unit.material.internal_bonds {
            bond.part_a.hash(&mut hasher);
            bond.part_b.hash(&mut hasher);
        }
        unit.placement.x.to_bits().hash(&mut hasher);
        unit.placement.y.to_bits().hash(&mut hasher);
        unit.placement.rotation_radians.to_bits().hash(&mut hasher);
    }
    structure.bonds.len().hash(&mut hasher);
    for bond in &structure.bonds {
        format!("{bond:?}").hash(&mut hasher);
    }
    hasher.finish()
}
