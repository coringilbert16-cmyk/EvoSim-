...
/// Generate only geometrically determined placements. There is intentionally
/// no rotation sweep: one-point contact has an analytic placement for every
/// endpoint pair, and a two-point constraint is resolved by the contact layer.
    ...
    for &target in targets {
        ...
        for endpoint in &endpoints {
            ...
            if anchored {
                ...
                let candidate = Placement { x: anchor.x, y: anchor.y, rotation_radians: rotation };
                // An anchored element cannot translate merely to satisfy a
                // neighbor. The rotation is useful only if the endpoint really
                // reaches the target at the fixed anchor position.
                let rotated = rotate_point(local, rotation);
                if (anchor.x + rotated.0 - world_target.0).abs() <= 1e-7
                    && (anchor.y + rotated.1 - world_target.1).abs() <= 1e-7
                {
                    add_unique_placement(&mut out, candidate);
                }
            } else {
                add_unique_placement(
                    &mut out,
                    placement_for_point_contact(local, world_target, 0.0),
                );
            }
        }
    }

    // If two known endpoints constrain a rigid non-circular part, derive its
    // unique orientation instead of sampling arbitrary rotations. This is a
    // bounded local calculation; it does not recurse over a global structure.
    if !anchored && targets.len() >= 2 && endpoints.len() >= 2 {
        let local_points = endpoints
            .iter()
            .filter_map(|e| local_endpoint_point(resource, *e, catalog))
            .collect::<Vec<_>>();
            ...
            for target_b in targets.iter().skip(ia + 1) {
                ...
                for la in 0..local_points.len() {
                    for lb in (la + 1)..local_points.len() {
                        let lp_a = local_points[la];
                        let lp_b = local_points[lb];
                        let ldx = lp_b.0 - lp_a.0;
                        let ldy = lp_b.1 - lp_a.1;
                        let wdx = world_b.0 - world_a.0;
                        let wdy = world_b.1 - world_a.1;
                        let llen = ldx.hypot(ldy);
                        let wlen = wdx.hypot(wdy);
                        if llen <= CONTACT_EPSILON || wlen <= CONTACT_EPSILON { continue; }
                        if (llen - wlen).abs() > 1e-7 * llen.max(wlen).max(1.0) { continue; }
                        let rotation = wdy.atan2(wdx) - ldy.atan2(ldx);
                        let rotated = rotate_point(lp_a, rotation);
                        add_unique_placement(&mut out, Placement {
                            x: world_a.0 - rotated.0,
                            y: world_a.1 - rotated.1,
                            rotation_radians: rotation,
                        });
                    }
                }
                if let Form::Circle { radius } = &resource.shape.form {
                    ...
                    if distance > CONTACT_EPSILON && distance <= 2.0 * radius + CONTACT_EPSILON {
                        ...
                        if height_sq >= -CONTACT_EPSILON {
                            ...
                            let py = ux * height;
                            add_unique_placement(&mut out, Placement {
                                x: world_a.0 + ux * along + px,
                                y: world_a.1 + uy * along + py,
...
fn choose_next_part(material: &Material, assigned: &[Option<usize>]) -> Option<usize> {
    ...
    for part in 0..material.parts.len() {
        ...
        let neighbors = material.internal_bonds.iter().filter(|bond| {
...
fn solve_material_placements(
    ...
    fn search(
        ...
        for i in 0..material.parts.len() {
            ...
            if !unit.realize_default_geometry(catalog) { return false; }
            if working.units.iter().any(|old| units_strictly_overlap(&unit, old, catalog)) { return false; }
            working_indices[i] = Some(working.add_unit(unit));
        }

        let mut targets = Vec::new();
        for bond in &material.internal_bonds {
            let other = if bond.part_a == part && assigned[bond.part_b].is_some() { working_indices[bond.part_b] }
                else if bond.part_b == part && assigned[bond.part_a].is_some() { working_indices[bond.part_a] }
                else { None };
            if let Some(index) = other { targets.push(index); }
        }
        targets.sort_unstable();
        targets.dedup();

        let mut external_target_units = Vec::new();
        for constraint in external {
            external_target_units.extend(constraint.iter().copied());
        }
        external_target_units.sort_unstable();
        external_target_units.dedup();

        // External constraints are used only to orient the anchored first part
        // or to give later parts a physical target. They never cause translation
        // of an element whose blueprint anchor is fixed.
        let anchored = part == 0;
        let mut target_units = targets;
        target_units.extend(external_target_units);
        target_units.sort_unstable();
        target_units.dedup();
        let target_endpoints = contact_targets_for_units(&working, &target_units, catalog);
        let candidates = if target_endpoints.is_empty() && anchored {
            vec![anchor]
        } else {
            candidate_placements_for_targets(res, &target_endpoints, &working, anchor, anchored, catalog)
        };
        ...
        for candidate in candidates {
            ...
            let mut valid = true;
            for i in 0..material.parts.len() {
                let Some(p) = if i == part { Some(candidate) } else { placements[i] } else { continue };
                let mut unit = StructuralUnit::new(material.parts[i].0.clone(), p);
                if !unit.realize_default_geometry(catalog) || trial.units.iter().any(|old| units_strictly_overlap(&unit, old, catalog)) {
                    valid = false;
                    break;
                }
    ...
    if !search(structure, material, anchor, &mut placements, &mut assigned, external_constraints, catalog) {
...
fn commit_material(
    ...
    for ((name, _), placement) in material.parts.iter().zip(placements.iter()) {
        ...
        if trial.units.iter().any(|old| units_strictly_overlap(&unit, old, catalog)) {
    ...
    for constraint in external_constraints {
        ...
        'pair: for &a in &indices {
            for &b in constraint {
                if connect_units(&mut trial, a, b, catalog).is_some() { connected = true; break 'pair; }
            }
        }
...
pub(crate) fn realize_material_with_constraints(
    ...
    if material.parts.iter().any(|(_, amount)| (*amount - 1.0).abs() > f64::EPSILON) {
    ...
    if material.internal_bonds.iter().any(|b| b.part_a >= material.parts.len() || b.part_b >= material.parts.len() || b.part_a == b.part_b) {
