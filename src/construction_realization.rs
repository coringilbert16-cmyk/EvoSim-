...
fn circle_center(structure: &OrganismStructure, unit_index: usize, catalog: &[BaseResource]) -> Option<(f64, f64, f64)> {
    let unit = structure.units.get(unit_index)?;
    let Form::Circle { radius } = unit.shape(catalog)?.form else { return None };
    Some((unit.placement.x, unit.placement.y, radius))
}

fn add_circle_contact_candidates(out: &mut Vec<Placement>, resource: &BaseResource, targets: &[ContactTarget], structure: &OrganismStructure, anchor: Placement, catalog: &[BaseResource]) {
    let Form::Circle { radius } = resource.shape.form else { return; };
    let target_units = targets.iter().map(|target| target.unit_index).collect::<std::collections::BTreeSet<_>>().into_iter().collect::<Vec<_>>();
    for &target_index in &target_units {
        let Some((tx, ty, tr)) = circle_center(structure, target_index, catalog) else { continue; };
        let distance = radius + tr;
        if distance <= CONTACT_EPSILON { continue; }
        let preferred = (anchor.y - ty).atan2(anchor.x - tx);
        const SAMPLES: usize = 16;
        for sample in 0..SAMPLES {
            let angle = preferred + std::f64::consts::TAU * sample as f64 / SAMPLES as f64;
            add_unique_placement(out, Placement { x: tx + distance * angle.cos(), y: ty + distance * angle.sin(), rotation_radians: 0.0 });
        }
    }
    for ia in 0..target_units.len() {
        let Some((ax, ay, ar)) = circle_center(structure, target_units[ia], catalog) else { continue; };
        for &b_index in target_units.iter().skip(ia + 1) {
            let Some((bx, by, br)) = circle_center(structure, b_index, catalog) else { continue; };
            let dx = bx - ax; let dy = by - ay; let distance = dx.hypot(dy); let ra = radius + ar; let rb = radius + br;
            if distance <= CONTACT_EPSILON || distance > ra + rb + CONTACT_EPSILON || distance < (ra - rb).abs() - CONTACT_EPSILON { continue; }
            let along = (ra * ra - rb * rb + distance * distance) / (2.0 * distance);
            let height_sq = ra * ra - along * along;
            if height_sq < -CONTACT_EPSILON { continue; }
            let height = height_sq.max(0.0).sqrt(); let ux = dx / distance; let uy = dy / distance; let px = -uy * height; let py = ux * height;
            add_unique_placement(out, Placement { x: ax + ux * along + px, y: ay + uy * along + py, rotation_radians: 0.0 });
            add_unique_placement(out, Placement { x: ax + ux * along - px, y: ay + uy * along - py, rotation_radians: 0.0 });
        }
    }
}
...
fn candidate_placements_for_targets(resource: &BaseResource, targets: &[ContactTarget], structure: &OrganismStructure, anchor: Placement, catalog: &[BaseResource]) -> Vec<Placement> {
    ...
    let mut out = Vec::new();
    add_circle_contact_candidates(&mut out, resource, targets, structure, anchor, catalog);
    for &target in targets {
        ...
        for endpoint in &endpoints {
            ...
            let target_dx = world_target.0 - anchor.x; let target_dy = world_target.1 - anchor.y;
    ...
    if targets.len() >= 2 && endpoints.len() >= 2 {
        ...
        for (ia, target_a) in targets.iter().enumerate() {
            ...
            for target_b in targets.iter().skip(ia + 1) {
                ...
                for la in 0..local_points.len() { for lb in (la + 1)..local_points.len() {
                    let lp_a = local_points[la]; let lp_b = local_points[lb]; let ldx = lp_b.0 - lp_a.0; let ldy = lp_b.1 - lp_a.1; let wdx = world_b.0 - world_a.0; let wdy = world_b.1 - world_a.1; let llen = ldx.hypot(ldy); let wlen = wdx.hypot(wdy);
                    if llen <= CONTACT_EPSILON || wlen <= CONTACT_EPSILON || (llen - wlen).abs() > 1e-7 * llen.max(wlen).max(1.0) { continue; }
                    let rotation = wdy.atan2(wdx) - ldy.atan2(ldx); let rotated = rotate_point(lp_a, rotation);
...
fn partial_configuration_valid(structure: &OrganismStructure, material: &Material, assigned: &[Option<usize>], catalog: &[BaseResource]) -> bool {
    for bond in &material.internal_bonds { let (Some(a), Some(b)) = (assigned[bond.part_a], assigned[bond.part_b]) else { continue; }; if !internal_contact_exists(structure, a, b, catalog) { return false; } } true
}
fn choose_next_part(material: &Material, assigned: &[Option<usize>]) -> Option<usize> {
    ...
    for part in 0..material.parts.len() { if assigned[part].is_some() { continue; } let neighbors = material.internal_bonds.iter().filter(|bond| (bond.part_a == part && assigned[bond.part_b].is_some()) || (bond.part_b == part && assigned[bond.part_a].is_some())).count() as i32; let degree = material.internal_bonds.iter().filter(|bond| bond.part_a == part || bond.part_b == part).count() as i32; let score = neighbors * 100 + degree * 10 - part as i32; if score > best_score { best_score = score; best = Some(part); }} best
}
...
fn solve_material_placements(structure: &OrganismStructure, element: &BlueprintElement, catalog: &[BaseResource], external_constraints: &[Vec<usize>]) -> Result<Vec<Placement>, String> {
    let material = &element.material; let mut placements = vec![None; material.parts.len()]; let mut assigned = vec![None; material.parts.len()]; let anchor = Placement { x: element.placement.x, y: element.placement.y, rotation_radians: 0.0 };
    fn search(base: &OrganismStructure, material: &Material, anchor: Placement, placements: &mut [Option<Placement>], assigned: &mut [Option<usize>], external: &[Vec<usize>], catalog: &[BaseResource]) -> bool {
        ...
        let Some(part) = choose_next_part(material, assigned) else { return false; }; let Some(res) = resource(catalog, &material.parts[part].0) else { return false; };
        let mut working = base.clone(); let mut working_indices = vec![None; material.parts.len()];
        for i in 0..material.parts.len() { let Some(p) = placements[i] else { continue; }; let mut unit = StructuralUnit::new(material.parts[i].0.clone(), p); if !unit.realize_default_geometry(catalog) || working.units.iter().any(|old| units_strictly_overlap(&unit, old, catalog)) { return false; } working_indices[i] = Some(working.add_unit(unit)); }
        let mut target_units = Vec::new();
        for bond in &material.internal_bonds { let other = if bond.part_a == part && assigned[bond.part_b].is_some() { working_indices[bond.part_b] } else if bond.part_b == part && assigned[bond.part_a].is_some() { working_indices[bond.part_a] } else { None }; if let Some(index) = other { target_units.push(index); } }
        for constraint in external { target_units.extend(constraint.iter().copied()); } target_units.sort_unstable(); target_units.dedup();
        let target_endpoints = contact_targets_for_units(&working, &target_units, catalog);
        let candidates = candidate_placements_for_targets(res, &target_endpoints, &working, anchor, catalog);
        for candidate in candidates {
            let mut trial = base.clone(); let mut trial_indices = vec![None; material.parts.len()]; let mut valid = true;
            for i in 0..material.parts.len() { let p = if i == part { Some(candidate) } else { placements[i] }; let Some(p) = p else { continue; }; let mut unit = StructuralUnit::new(material.parts[i].0.clone(), p); if !unit.realize_default_geometry(catalog) || trial.units.iter().any(|old| units_strictly_overlap(&unit, old, catalog)) { valid = false; break; } trial_indices[i] = Some(trial.add_unit(unit)); }
            if !valid || !partial_configuration_valid(&trial, material, &trial_indices, catalog) { continue; }
            placements[part] = Some(candidate); assigned[part] = trial_indices[part]; if search(base, material, anchor, placements, assigned, external, catalog) { return true; } assigned[part] = None; placements[part] = None;
        }
        false
    }
...
fn connect_units(structure: &mut OrganismStructure, a: usize, b: usize, catalog: &[BaseResource]) -> Option<f64> {
    ...
    for candidate in candidates { if candidate.distance > CONTACT_EPSILON { continue; } let evaluation = crate::combine::evaluate_formation(candidate, pa.cohesion, pb.cohesion); let (_, work, _) = match crate::combine::required_investment(pa, pb, evaluation, 0.0) { Ok(value) => value, Err(_) => continue }; let strength = crate::combine::bond_strength(pa, pb); if !strength.is_finite() || !(0.0..=1.0).contains(&strength) { continue; } let bond = Bond { endpoint_a: BondEndpoint::new(id_a, candidate.endpoint_a), endpoint_b: BondEndpoint::new(id_b, candidate.endpoint_b), strength, bond_energy: 0.0 }; let mut trial = structure.clone(); if try_add_bond(&mut trial, bond, catalog).is_ok() { *structure = trial; return Some(work); } } None
}
...
fn commit_material(structure: &mut OrganismStructure, material: &Material, placements: &[Placement], catalog: &[BaseResource]) -> Result<Vec<usize>, String> {
    ...
    for ((name, _), placement) in material.parts.iter().zip(placements.iter()) { let mut unit = StructuralUnit::new(name.clone(), *placement); if !unit.realize_default_geometry(catalog) { return Err(format!("material references invalid resource {name}")); } if trial.units.iter().any(|old| units_strictly_overlap(&unit, old, catalog)) { return Err("material realization contains strict constituent overlap".into()); } indices.push(trial.add_unit(unit)); }
    for relationship in &material.internal_bonds { let a = indices[relationship.part_a]; let b = indices[relationship.part_b]; if !internal_contact_exists(&trial, a, b, catalog) { return Err("material relationship could not be physically realized".into()); } if connect_units(&mut trial, a, b, catalog).is_none() { return Err("material relationship could not be committed as a physical bond".into()); } }
