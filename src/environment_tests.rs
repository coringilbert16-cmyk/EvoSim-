...
fn diffusion_conserves_mass_per_resource_type_not_just_total() {
    ...
    field.deposit(150.0, 150.0, make_raw("Methane", 200.0));
    field.deposit(150.0, 150.0, make_raw("Carbon", 80.0));
    for _ in 0..50 {
...
fn corner_cell_diffusion_conserves_mass_with_only_two_neighbors() {
    let mut field = ActiveMaterialField::new(200.0, 200.0, 25.0);
    field.deposit(0.0, 0.0, make_raw("Sulfur", 40.0));
...
fn vent_draws_only_from_its_own_region_not_a_global_pool() {
    ...
    assert!((field.cells[field_idx_a].total_amount() - region_a_amount).abs() < 1e-9);
    assert!((reservoir.cells[idx_b].amount_of("Methane") - (region_b_amount - 10.0)).abs() < 1e-9);
}
...
#[test]
fn vent_releases_reservoir_stock_as_raw_active_material() {
    let (mut field, mut reservoir) = field_and_reservoir();
    let field_index = field.index_for_position(500.0, 500.0).unwrap();
    let reservoir_index = reservoir.reservoir_index_for_field_index(&field, field_index);
...
#[test]
fn vent_releases_any_unified_reservoir_stock_as_raw_active_material() {
    let (mut field, mut reservoir) = field_and_reservoir();
    let field_index = field.index_for_position(500.0, 500.0).unwrap();
    let reservoir_index = reservoir.reservoir_index_for_field_index(&field, field_index);
...
#[test]
fn settling_moves_raw_active_material_into_the_reservoir() {
    let (mut field, mut reservoir) = field_and_reservoir();
    let idx = field.index_for_position(500.0, 500.0).unwrap();
    field.deposit_at_index(idx, make_raw("Carbon", 100.0));
    field.deposit_at_index(idx, make_raw("Carbon", 40.0));
    for _ in 0..20 {
        apply_settling(&mut field, &mut reservoir, DEFAULT_SETTLING_FRACTION);
    }
    assert!(field.cells[idx].total_amount() < 140.0);
...
fn settling_preserves_structured_material_in_active_field() {
    ...
    assert_eq!(field.cells[idx].materials.len(), 1);
    assert!(field.cells[idx].materials[0].has_internal_structure());
    assert!((field.cells[idx].total_amount() - 100.0).abs() < 1e-9);
    assert_eq!(reservoir.total_amount(), 0.0);
}
