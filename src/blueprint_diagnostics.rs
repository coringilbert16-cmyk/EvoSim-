...
mod tests {
    ...
    fn blueprint(
        ...
        StructuralBlueprint::new(
            ...
            edges
                .iter()
                .map(|&(a, b)| crate::structural_blueprint::BlueprintConnection {
                    element_a: a,
                    element_b: b,
                })
                .collect(),
    ...
    fn realized_result(b: &StructuralBlueprint) -> (usize, usize) {
        ...
        let structure = b
            .realize(&catalog)
            .expect("diagnostic blueprint must be structurally valid");
    ...
    fn legacy_and_explicit_orientation_round_trip() {
        ...
        let legacy: BlueprintElement =
            serde_json::from_str(legacy).expect("legacy blueprint must deserialize");
        let explicit: BlueprintElement =
            serde_json::from_str(explicit).expect("oriented blueprint must deserialize");
    ...
    fn blueprint_orientation_is_realized_as_element_frame() {
        ...
        let structure = StructuralBlueprint::new(vec![element], Vec::new())
            .realize(&default_catalog())
            .expect("oriented element must realize");
    ...
    fn blueprint_orientation_must_be_finite() {
        ...
        assert_eq!(
            blueprint.validate(),
            Err("element 0: blueprint orientation must be finite".into())
        );
    }
    ...
    fn multi_constraint_triangle_is_fully_realized() {
        ...
        let b = blueprint(
            3,
            &[(0, 1), (0, 2), (1, 2)],
            &[(0.0, 0.0), (d, 0.0), (d / 2.0, h)],
        );
        let (units, bonds) = realized_result(&b);
        assert_eq!(
            units, 3,
            "triangle blueprint must realize every constituent"
        );
    ...
    fn k4_is_rejected_by_the_physical_contact_graph_oracle() {
        ...
        let Form::Circle { radius } = water.shape.form else {
            panic!("water diagnostic requires circular default geometry")
        };
        let d = 2.0 * radius;
        let h = d * (3.0_f64).sqrt() / 2.0;
        let units = [
        ...
        for a in 0..units.len() {
            for b in (a + 1)..units.len() {
                let distance = (units[a].placement.x - units[b].placement.x)
                    .hypot(units[a].placement.y - units[b].placement.y);
                if (distance - 2.0 * radius).abs() <= 1e-9 {
                    pair_contacts += 1;
                }
            }
        }
        assert!(
            pair_contacts < 6,
            "K4 geometry cannot contain all six pair contacts"
        );
    }
}
