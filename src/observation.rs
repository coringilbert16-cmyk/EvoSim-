...
impl ObservationContext {
    pub(crate) fn world() -> Self {
        Self { level: ObservationLevel::World, organism_ids: Vec::new(), focused_organism_id: None }
    }

    pub(crate) fn organism(organism_ids: Vec<String>) -> Self {
        Self { level: ObservationLevel::Organism, organism_ids, focused_organism_id: None }
    }

    pub(crate) fn structure(organism_ids: Vec<String>, focused_organism_id: Option<String>) -> Self {
        Self { level: ObservationLevel::Structure, organism_ids, focused_organism_id }
    }
}
...
impl ObservationProjection {
    pub(crate) fn world(payload: WorldObservation) -> Self {
        Self { context: ObservationContext::world(), payload: ObservationPayload::World(payload) }
    }

    pub(crate) fn organism(context: ObservationContext, payload: OrganismObservation) -> Option<Self> {
        if context.level != ObservationLevel::Organism { return None; }
        Some(Self { context, payload: ObservationPayload::Organism(payload) })
    }

    pub(crate) fn structure(context: ObservationContext, payload: StructureObservation) -> Option<Self> {
        if context.level != ObservationLevel::Structure { return None; }
        Some(Self { context, payload: ObservationPayload::Structure(payload) })
    }
}
    ...
    pub(crate) fn from_simulation(simulation: &Simulation) -> Self {
        ...
        let field = simulation
            ...
            .filter_map(|(cell_index, cell)| {
                ...
                if materials.is_empty() { return None; }
                let (x, y) = simulation.environment.field.cell_center(cell_index);
                Some(WorldFieldObservation { cell_index, x, y, materials })
            })
            .collect();

        let vents = simulation.environment.vents.iter().map(|vent| WorldPointObservation { x: vent.x, y: vent.y }).collect();
        let decomposing_bodies = simulation.decomposing_bodies.iter().map(|body| WorldPointObservation { x: body.position.x, y: body.position.y }).collect();
...
impl OrganismObservation {
    pub(crate) fn from_simulation(simulation: &Simulation, organism_id: &str) -> Option<Self> {
        ...
        Some(Self {
            id: organism.id.clone(), x: position.x, y: position.y,
            min_x, max_x, min_y, max_y, silhouette,
...
impl StructureObservation {
    pub(crate) fn from_simulation(simulation: &Simulation, organism_id: &str) -> Option<Self> {
        let organism = simulation.organisms.iter().find(|organism| organism.id == organism_id)?;
        let catalog = &simulation.environment.catalog;
        let units = organism.structure.units.iter().enumerate().map(|(unit_index, unit)| StructureUnitObservation {
            unit_index,
            material: unit.material.material().clone(),
            placement: unit.placement,
            form: unit.shape(catalog).map(|shape| shape.form.clone()),
        }).collect::<Vec<_>>();
        let bonds = organism.structure.bonds.iter().map(|bond| StructureBondObservation {
            unit_a: bond.unit_a,
        ...
        Some(Self { id: organism.id.clone(), units, bonds })
    }
}
...
fn connection_endpoint(
    ...
    let local = structure.connection_site(crate::structure::ConnectionSiteRef { unit_index, point_index }, catalog)?;
...
mod tests {
    ...
    fn structure_context_preserves_stable_ids_and_focus() {
        let context = ObservationContext::structure(vec!["17".into(), "23".into()], Some("23".into()));
    ...
    fn structure_observation_preserves_material_placement_and_bonds() {
        ...
        assert!(observation.bonds.iter().all(|bond| bond.endpoint_a.is_some() && bond.endpoint_b.is_some()));
    }
