...
pub fn corner_alignment_rotations(candidate: &Shape, candidate_vertex: usize, target: &Shape, target_vertex: usize, target_rotation: f64) -> Vec<f64> {
    ...
    let ci = candidate_vertex; let ti = target_vertex; let c_prev = cv[(ci + cv.len() - 1) % cv.len()]; let c_here = cv[ci]; let c_next = cv[(ci + 1) % cv.len()]; let t_prev = tv[(ti + tv.len() - 1) % tv.len()]; let t_here = tv[ti]; let t_next = tv[(ti + 1) % tv.len()];
    let mut out = Vec::new();
    for ca in [edge_angle(c_prev, c_here), edge_angle(c_here, c_next)].into_iter().flatten() { for ta in [edge_angle(t_prev, t_here), edge_angle(t_here, t_next)].into_iter().flatten() { for relative in [ta - ca, ta + std::f64::consts::PI - ca] { let rotation = target_rotation + relative; if !out.iter().any(|r: &f64| (r - rotation).abs() <= 1e-10) { out.push(rotation); } } } }
    out
}
pub fn line_endpoint_alignment_rotations(candidate_endpoint: usize, target_endpoint: usize, target_rotation: f64) -> Vec<f64> { if candidate_endpoint > 1 || target_endpoint > 1 { return Vec::new(); } let candidate_interior = if candidate_endpoint == 0 { 0.0 } else { std::f64::consts::PI }; let target_interior = target_rotation + if target_endpoint == 0 { 0.0 } else { std::f64::consts::PI }; vec![target_interior + std::f64::consts::PI - candidate_interior, target_interior - candidate_interior] }
