...
pub fn line_endpoint_alignment_rotations(candidate_endpoint: usize, target_endpoint: usize, target_rotation: f64) -> Vec<f64> {
    ...
    let mut rotations = vec![target_interior + std::f64::consts::PI - candidate_interior, target_interior - candidate_interior];
...
pub fn corner_normal(shape: &Shape, vertex: usize) -> Option<(f64, f64)> {
    ...
    if vertices.len() < 3 || vertex >= vertices.len() { return None; }
    ...
    let normals = |a: (f64, f64), b: (f64, f64)| {
        ...
        let dy = b.1 - a.1;
        let len = dx.hypot(dy);
        if len <= f64::EPSILON { None } else if ccw { Some((dy / len, -dx / len)) } else { Some((-dy / len, dx / len)) }
    };
    let (nx0, ny0) = normals(prev, here)?;
    let (nx1, ny1) = normals(here, next)?;
    let len = (nx0 + nx1).hypot(ny0 + ny1);
    if len <= f64::EPSILON { None } else { Some(((nx0 + nx1) / len, (ny0 + ny1) / len)) }
}

/// Outward direction of a rigid line endpoint.
pub fn line_endpoint_normal(shape: &Shape, endpoint: usize) -> Option<(f64, f64)> {
    let Form::Line { .. } = shape.form else { return None; };
    match endpoint { 0 => Some((-1.0, 0.0)), 1 => Some((1.0, 0.0)), _ => None }
}
...
mod tests {
    ...
    #[test] fn corner_alignment_uses_incident_edges_not_authored_normals() { let rotations = corner_alignment_rotations(&square(), 0, &square(), 2, 0.0); assert!(rotations.iter().any(|r| (r - PI / 2.0).abs() < 1e-10)); assert!(rotations.iter().any(|r| r.abs() < 1e-10)); }
    #[test] fn line_endpoint_alignment_is_rigid() { let rotations = line_endpoint_alignment_rotations(0, 1, 0.0); assert_eq!(rotations.len(), 2); assert!(rotations.iter().any(|r| r.abs() < 1e-12)); assert!(rotations.iter().any(|r| (r - PI).abs() < 1e-12)); }
    #[test] fn l_shape_preserves_its_concave_boundary() { let v = l_shape().form.polygon_vertices().unwrap(); assert_eq!(v.len(), 6); assert_eq!(v[4], (0.0, 0.0)); }
    #[test] fn l_inner_corner_normal_comes_from_incident_edges() { let (nx, ny) = corner_normal(&l_shape(), 4).unwrap(); assert!((nx + 2.0_f64.sqrt() / 2.0).abs() < 1e-10); assert!((ny + 2.0_f64.sqrt() / 2.0).abs() < 1e-10); }
    #[test] fn catalog_phosphorus_l_has_a_real_interior_corner() { let phosphorus = default_catalog().into_iter().find(|resource| resource.name == "Phosphorus").expect("default catalog must contain Phosphorus"); let vertices = phosphorus.shape.form.polygon_vertices().unwrap(); assert_eq!(vertices.len(), 6); assert_eq!(vertices[3], (0.0, 0.0)); let (nx, ny) = corner_normal(&phosphorus.shape, 3).unwrap(); assert!((nx + 2.0_f64.sqrt() / 2.0).abs() < 1e-10); assert!((ny - 2.0_f64.sqrt() / 2.0).abs() < 1e-10); }
    #[test] fn square_corner_normal_is_physical_bisector() { let (nx, ny) = corner_normal(&square(), 0).unwrap(); assert!((nx + 2.0_f64.sqrt() / 2.0).abs() < 1e-10); assert!((ny + 2.0_f64.sqrt() / 2.0).abs() < 1e-10); }
    #[test] fn world_vertex_applies_only_rigid_transform() { let p = world_vertex(&square(), 0, Placement { x: 10.0, y: 20.0, rotation_radians: PI / 2.0 }).unwrap(); assert!((p.0 - 11.0).abs() < 1e-12); assert!((p.1 - 19.0).abs() < 1e-12); }
}
