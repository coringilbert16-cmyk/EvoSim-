...
pub fn corner_normal(shape: &Shape, vertex: usize) -> Option<(f64, f64)> {
    ...
    let normals = |a: (f64, f64), b: (f64, f64)| {
        ...
        let dy = b.1 - a.1;
        let len = dx.hypot(dy);
        if len <= f64::EPSILON { None }
        else if ccw { Some((dy / len, -dx / len)) }
        else { Some((-dy / len, dx / len)) }
    };
    ...
    let ny = ny0 + ny1;
    let len = nx.hypot(ny);
    if len <= f64::EPSILON { None } else { Some((nx / len, ny / len)) }
}
...
mod tests {
    ...
    fn l_shape_preserves_its_concave_boundary() {
        ...
        assert_eq!(v.len(), 6);
        assert_eq!(v[0], (-1.0, -2.0)); assert_eq!(v[1], (1.0, -2.0)); assert_eq!(v[2], (1.0, 2.0));
        assert_eq!(v[3], (0.0, 2.0)); assert_eq!(v[4], (0.0, 0.0)); assert_eq!(v[5], (-1.0, 0.0));
    }
    #[test]
