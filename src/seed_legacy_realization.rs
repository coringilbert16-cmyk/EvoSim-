//! Isolated legacy constructor used only to compile the historical seed into
//! an explicit atomic blueprint. It is not part of live construction.
...
fn placement_candidates_for_new(neighbor: &StructuralUnit, neighbor_endpoint: ConnectionEndpoint, resource: &BaseResource, desired_x: f64, desired_y: f64, catalog: &[BaseResource]) -> Option<Vec<Placement>> {
    ...
    for endpoint in endpoint_prototypes(&temp, catalog) {
        match endpoint {
            ConnectionEndpoint::Corner { point_index } | ConnectionEndpoint::LineEndpoint { point_index } => {
                ...
                out.push(Placement { x: target.x - (p.x * c - p.y * s), y: target.y - (p.x * s + p.y * c), rotation_radians: rotation });
            }
            ConnectionEndpoint::Boundary { .. } | ConnectionEndpoint::Fluid { .. } => {
                let radius = resource.shape.form.bounding_radius();
                let dx = target.x - desired_x; let dy = target.y - desired_y; let distance = dx.hypot(dy);
                let (ux, uy) = if distance > CONTACT_EPSILON { (dx / distance, dy / distance) } else { (1.0, 0.0) };
                out.push(Placement { x: target.x - ux * radius, y: target.y - uy * radius, rotation_radians: 0.0 });
            }
        }
    }
    Some(out)
...
fn connect_units(structure: &mut OrganismStructure, a: usize, b: usize, catalog: &[BaseResource]) -> Option<f64> {
    ...
    let id_a = structure.physical_id(a)?; let id_b = structure.physical_id(b)?;
    let pa = structure.units.get(a)?.properties(catalog)?; let pb = structure.units.get(b)?.properties(catalog)?;
