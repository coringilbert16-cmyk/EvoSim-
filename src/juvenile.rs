...
pub(crate) fn realize_initial_with_reserve(
    ...
    let (_, _, required_initial_energy) = form_declared_bonds(base.clone(), blueprint, catalog, TRIAL_ENERGY)?;
    ...
    validate_realized_juvenile(&structure, catalog, &blueprint.core_elements, JuvenileViabilityRequirements::default())?;
    if remaining + EPS < reserve_energy { return Err(format!("juvenile initialization could not preserve its reserve: remaining={remaining}")); }
    Ok((structure, ledger, remaining))
}

fn realize_declared_units(blueprint: &StructuralBlueprint, catalog: &[BaseResource]) -> Result<OrganismStructure, String> {
    let mut structure = OrganismStructure::new();
    for element in &blueprint.elements {
        let mut unit = StructuralUnit::from_material(element.material.clone(), crate::structure::Placement { x: element.placement.x, y: element.placement.y, rotation_radians: element.placement.rotation_radians }).ok_or_else(|| "juvenile blueprint contains invalid material".to_string())?;
...
fn form_declared_bonds(
    ...
    for connection in &blueprint.connections {
        combine_specific_pair(&mut structure, connection.element_a, connection.element_b, catalog, 0.0, &mut cache, &mut ledger, &mut energy).ok_or_else(|| format!("juvenile blueprint bond could not be realized: {}-{}", connection.element_a, connection.element_b))?;
    }
...
mod tests {
    ...
    fn juvenile_initialization_is_a_valid_physical_realization() {
        ...
        let (s, _, e) = realize_initial_with_reserve(&g.juvenile_blueprint, &c, g.juvenile_energy_reserve).unwrap();
    ...
    fn nonpositive_reserve_is_rejected() {
        let g = initial_genome();
        assert!(realize_initial_with_reserve(&g.juvenile_blueprint, &default_catalog(), 0.0).is_err());
    }
}
