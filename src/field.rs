...
impl ActiveMaterialField {
    ...
    pub fn deposit_structured(
        &mut self,
        material: Material,
        placements: Vec<crate::structure::Placement>,
    ) -> bool {
        ...
        let Some((x, y)) = placements.first().map(|p| (p.x, p.y)) else {
            return false;
        };
        let Some(index) = self.index_for_position(x, y) else {
            return false;
        ...
        let Some(instance) = FieldMaterial::new(id, material, placements) else {
            return false;
        };
    ...
    /// Return stable field-material identities whose actual constituent
    /// boundaries touch/intersect the organism body. Cell membership is only
    /// an index; it is never used as proof of contact.
    pub fn contacting_materials(
        ...
        usize,
        usize,
        Vec<crate::boundary_contact::BoundaryContact>,
    )> {
        if tolerance < 0.0 {
            return Vec::new();
        }
        ...
        for (cell_index, cell) in self.cells.iter().enumerate() {
            for (material_index, field_material) in cell.materials.iter().enumerate() {
                let Some(instance) = field_material.physical_instance(catalog) else {
                    continue;
                };
                let boundary = crate::boundary_contact::boundary_contacts(
                    body, &instance, tolerance,
                );
                if !boundary.is_empty() {
                    contacts.push((cell_index, material_index, boundary));
                }
            }
        }
        contacts
    ...
    pub fn take_for_acquisition_by_id(
        &mut self,
        id: u64,
        maximum_transfer_amount: f64,
    ) -> Option<Material> {
        if !maximum_transfer_amount.is_finite() || maximum_transfer_amount < 1.0 {
            return None;
        }
        for cell in &mut self.cells {
            let Some(material_index) = cell.materials.iter().position(|material| material.id == id)
            else {
                continue;
            };
            let field_material = &mut cell.materials[material_index];
            if field_material.material.has_internal_structure() {
                if field_material.material.total_amount()
                    > maximum_transfer_amount + MATERIAL_EPSILON
                {
                    return None;
                }
    ...
    pub fn take_at_index(
        ...
        self.cells[index]
            .materials
            .retain(|material| !material.material.is_empty());
        Some(taken)
    }
    ...
    pub fn take_for_acquisition(&mut self, index: usize) -> Option<Material> {
        ...
        let material_index = cell
            ...
            .iter()
            .position(|m| {
                !m.material.has_internal_structure()
                    && !m.material.is_empty()
                    && m.material.is_valid()
            })?;
        let part_index = cell.materials[material_index]
            ...
            .position(|(_, amount)| {
                amount.is_finite()
                    && *amount >= 1.0
                    && amount.fract().abs() <= MATERIAL_EPSILON
            })?;
        let name = cell.materials[material_index].material.parts[part_index]
            .0
            .clone();
    ...
    pub fn diffuse_step(&mut self, fraction: f64) {
        ...
        for i in 0..n {
            ...
            for material_index in 0..material_count {
                if self.cells[i].materials[material_index]
                    .material
                    .has_internal_structure()
                {
                    continue;
                }
                let total = self.cells[i].materials[material_index]
                    .material
                    .total_amount();
                ...
                let Some(piece) = take_whole_unstructured(
                    &mut self.cells[i].materials[material_index].material,
                    outflow,
                ) else {
                    continue;
                };
                let Some(source_position) = source.placements.first().copied() else {
                    continue;
                };
                ...
                for (k, &neighbor_index) in neighbors.iter().enumerate() {
                    ...
                    if count != outflow {
                        let Some(taken) =
                            take_whole_unstructured(&mut piece_part, count)
                        else {
                            continue;
                        };
                        piece_part = taken;
                    ...
                    if let Some(instance) = FieldMaterial::new(id, piece_part, placements) {
                        moves.push((neighbor_index, instance));
                    }
                }
            }
            self.cells[i]
                .materials
                .retain(|m| !m.material.is_empty());
        }
        for (neighbor_index, instance) in moves {
