...
pub struct FieldCell {
    /// Ecological material instances occupying this field cell.
...
impl ActiveMaterialField {
    ...
    pub fn cells_within_radius(&self, x: f64, y: f64, radius: f64) -> Vec<usize> {
        if !x.is_finite() || !y.is_finite() || !radius.is_finite() || radius < 0.0 || self.cells.is_empty() || self.width_cells == 0 || self.height_cells == 0 {
            return Vec::new();
        }
    ...
    /// Deposit at an actual world-space location. Unstructured material is
    /// represented as a spatial parcel at that location; structured material
    /// must use `deposit_structured` because its constituents need explicit
    /// placements.
    ...
    /// Compatibility helper for callers that only know a field cell. This is
    /// deliberately limited to unstructured material; geometry-sensitive code
    /// must use a world-space position or explicit constituent placements.
    ...
    pub fn deposit_structured(&mut self, material: Material, placements: Vec<crate::structure::Placement>) -> bool {
        if material.is_empty() || !material.is_valid() || !material.has_internal_structure() || placements.len() != material.parts.len() {
            return false;
        }
    ...
    /// Remove one consumable free material unit from a field cell. This remains
    /// a transitional inventory API; geometry-aware acquisition should select
    /// a concrete FieldMaterial before calling it.
    pub fn take_for_acquisition(&mut self, index: usize) -> Option<Material> {
        let cell = self.cells.get_mut(index)?;
        let material_index = cell.materials.iter().position(|field_material| {
            !field_material.material.has_internal_structure()
                && !field_material.material.is_empty()
                && field_material.material.is_valid()
        })?;
        let part_index = cell.materials[material_index].material.parts.iter().position(|(_, amount)| {
    ...
    pub fn diffuse_step(&mut self, fraction: f64) {
        ...
        let mut moves: Vec<(usize, usize, FieldMaterial)> = Vec::new();
        for i in 0..n {
            ...
            for material_index in 0..material_count {
                ...
                let source_position = source.placements.first().copied();
                let Some(source_position) = source_position else { continue; };
                ...
                for (k, &neighbor_index) in neighbors.iter().enumerate() {
                    ...
                    if count != outflow {
                        let Some(taken) = take_whole_unstructured(&mut piece_part, count) else { continue; };
                        piece_part = taken;
                    }
                    let (target_x, target_y) = self.cell_center(neighbor_index);
                    let dx = target_x - source_position.x;
                    let dy = target_y - source_position.y;
                    let id = self.allocate_id();
                    let placements = piece_part.parts.iter().map(|p| crate::structure::Placement {
                        x: source_position.x + dx,
                        y: source_position.y + dy,
                        rotation_radians: source_position.rotation_radians,
                    }).collect();
                    if let Some(instance) = FieldMaterial::new(id, piece_part, placements) {
                        moves.push((i, neighbor_index, instance));
                    }
                }
            }
        }
        for (_, neighbor_index, instance) in moves {
            self.cells[neighbor_index].materials.push(instance);
        }
    }
