...
impl FieldCell {
    ...
    pub fn total_amount(&self) -> f64 {
        self.materials.iter().map(|m| m.material.total_amount()).sum()
    }
...
impl ActiveMaterialField {
    ...
    fn allocate_id(&mut self) -> u64 {
        ...
        self.next_material_id = self.next_material_id.saturating_add(1);
        id
    }
    ...
    pub fn deposit(&mut self, x: f64, y: f64, material: Material) -> bool {
        ...
        self.deposit_unstructured_at(index, material, x, y);
        true
    }
    ...
    pub fn deposit_at_index(&mut self, index: usize, material: Material) {
        ...
        let (x, y) = self.cell_center(index);
        self.deposit_unstructured_at(index, material, x, y);
    }

    pub fn deposit_structured(&mut self, material: Material, placements: Vec<crate::structure::Placement>) -> bool {
        // A structured material is one rigid physical object. Its internal
        // parts and bonds define its shape; exactly one placement anchors that
        // whole object in world space.
        if material.is_empty() || !material.is_valid() || !material.has_internal_structure() || placements.len() != 1 {
            return false;
        }
        let Some((x, y)) = placements.first().map(|p| (p.x, p.y)) else { return false; };
        let Some(index) = self.index_for_position(x, y) else { return false; };
        let id = self.allocate_id();
        let Some(instance) = FieldMaterial::new(id, material, placements) else { return false; };
    ...
    fn deposit_unstructured_at(&mut self, index: usize, material: Material, x: f64, y: f64) {
        ...
        let placements: Vec<_> = material.parts.iter().map(|_| crate::structure::Placement { x, y, rotation_radians: 0.0 }).collect();
        if let Some(existing) = self.cells[index].materials.iter_mut().find(|existing| {
            !existing.material.has_internal_structure()
                ...
                && existing.placements.iter().zip(&placements).all(|(a, b)| a.x == b.x && a.y == b.y && a.rotation_radians == b.rotation_radians)
                && existing.material.parts.len() == material.parts.len()
                && existing.material.parts.iter().zip(&material.parts).all(|((a, _), (b, _))| a == b)
        }) {
            let mut merged_parts = existing.material.parts.clone();
            for ((_, existing_amount), (_, incoming_amount)) in merged_parts.iter_mut().zip(&material.parts) { *existing_amount += *incoming_amount; }
            if merged_parts.iter().all(|(_, amount)| amount.is_finite()) { existing.material.parts = merged_parts; return; }
        }
        let id = self.allocate_id();
        if let Some(instance) = FieldMaterial::new(id, material, placements) { self.cells[index].materials.push(instance); }
    }

    pub fn contacting_materials(&self, body: &OrganismBodyGeometry, catalog: &[crate::resources::BaseResource], tolerance: f64) -> Vec<(usize, usize, Vec<crate::boundary_contact::BoundaryContact>)> {
    ...
    pub fn take_material_by_id(&mut self, id: u64) -> Option<FieldMaterial> {
        for cell in &mut self.cells {
            if let Some(index) = cell.materials.iter().position(|material| material.id == id) { return Some(cell.materials.swap_remove(index)); }
        }
        None
    }
    ...
    pub fn take_at_index(&mut self, index: usize, material_index: usize, amount: f64) -> Option<Material> {
        ...
        self.cells[index].materials.retain(|material| !material.material.is_empty());
        Some(taken)
    }
    ...
    pub fn take_for_acquisition(&mut self, index: usize) -> Option<Material> {
        ...
        cell.materials.retain(|m| !m.material.is_empty());
        Some(material)
    }
    ...
    pub fn diffuse_step(&mut self, fraction: f64) {
        ...
        let mut moves: Vec<(usize, FieldMaterial)> = Vec::new();
        for i in 0..n {
            ...
            for material_index in 0..material_count {
                ...
                let total = self.cells[i].materials[material_index].material.total_amount();
                if total <= MATERIAL_EPSILON { continue; }
                let outflow = (total * fraction).floor() as usize;
                if outflow < 1 { continue; }
                ...
                let base_share = outflow / neighbors.len();
                let remainder = outflow % neighbors.len();
                for (k, &neighbor_index) in neighbors.iter().enumerate() {
                    let share = base_share + usize::from(k < remainder);
                    if share == 0 { continue; }
                    let share_material = if share == outflow { piece.clone() } else { piece.take(share).unwrap_or_else(|| Material::free_base("", 0.0)) };
                    if share_material.is_empty() { continue; }
                    let (x, y) = self.cell_center(neighbor_index);
                    let placements = share_material.parts.iter().map(|_| crate::structure::Placement { x, y, rotation_radians: source_position.rotation_radians }).collect();
                    let id = self.allocate_id();
                    if let Some(instance) = FieldMaterial::new(id, share_material, placements) { moves.push((neighbor_index, instance)); }
                }
            }
        }
        for (index, material) in moves { self.cells[index].materials.push(material); }
        for cell in &mut self.cells { cell.materials.retain(|m| !m.material.is_empty()); }
    }
    ...
    pub fn total_material(&self) -> Vec<(String, f64)> {
        ...
        for cell in &self.cells {
            for (name, amount) in cell.total_material() {
                if let Some(existing) = totals.iter_mut().find(|(n, _)| n == &name) { existing.1 += amount; } else { totals.push((name, amount)); }
            }
        }
        totals
    }
