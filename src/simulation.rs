...
impl Simulation {
    ...
    fn mature_structural_mass(organism: &Organism, environment: &Environment) -> f64 {
        organism.genome.mature_construction_target().ok().map(|target| target.structural_mass(&environment.catalog)).unwrap_or(0.0)
    }
    ...
    fn update_development_stage(organism: &mut Organism, environment: &Environment) {
        match organism.development_stage {
            DevelopmentStage::Offspring => {
                if organism.reproductive_construction.is_none() { organism.development_stage = DevelopmentStage::Juvenile }
            }
            DevelopmentStage::Juvenile => {
                if Self::growth_fraction(organism, environment) >= ADULTHOOD_GROWTH_FRACTION { organism.development_stage = DevelopmentStage::Adult }
            }
    ...
    fn acquisition_targets(organism: &Organism, environment: &Environment) -> Vec<usize> {
        ...
        if cell.physical_materials.iter().any(|material| material.is_realized() && !material.material.is_empty())
            || cell.materials.iter().any(|material| !material.is_empty() && material.is_valid())
    ...
    fn action_eligibility(organism: &Organism, environment: &Environment) -> ActionEligibility {
        ...
        ActionEligibility {
            ...
            can_combine: organism.active_transformation_id.is_none() && (can_build_from_storage || can_join_existing_structure),
    ...
    fn acquire_target(organism: &mut Organism, environment: &mut Environment, field_index: usize) -> bool {
        ...
        // A physically realized object is transferred intact. No composition-only
        // fallback is allowed to consume an existing physical object.
        if let Some(physical) = environment.field.take_physical_for_acquisition(field_index) {
            ...
            if organism.stored_material.store_physical(material, placements, &environment.catalog) {
                return true;
            }
    ...
    fn recycle_dead_organism(environment: &mut Environment, organism: &mut Organism, ledger: &mut EnergyLedger) -> Option<crate::decomposition::DecomposingBody> {
        ...
        if let Some(construction) = &organism.reproductive_construction {
            for material in construction.committed_material.materials.iter().cloned() { environment.field.deposit(position.x, position.y, material); }
        }
        ...
        for index in 0..self.decomposing_bodies.len() {
            ...
            if step.net_energy > 0.0 {
                if let Some(organism_index) = crate::decomposition::harvestable_decomposition_energy(&self.organisms, &self.decomposing_bodies[index].position) {
                    ...
                    let (body, organism) = { let body = &mut self.decomposing_bodies[index].energy_budget; let organism = &mut self.organisms[organism_index].usable_energy; (body, organism) };
            ...
            if let Some(materials) = step.released_material {
                ...
                for material in materials { self.environment.field.deposit(position.x, position.y, material); }
    ...
    pub(crate) fn step(&mut self) {
        ...
        let mut reproduction_requests = Vec::new();
        {
            ...
            for organism in organisms {
                ...
                match selected.action {
                    ...
                    ActionKind::Combine => {
                        let combined = crate::combine_runtime::try_combine(organism, environment, &mut compatibility_cache, &mut self.energy_ledger).is_some();
                        ...
                        if matches!(organism.development_stage, DevelopmentStage::Adult) { reproduction_requests.push(organism.id.clone()); }
                    }
                    ActionKind::Break => {
                        if let Some(transformation) = Self::try_start_transformation(organism, &environment.catalog, &mut self.next_transformation_id, &selected) { self.active_transformations.push(transformation); }
                    }
                    ActionKind::Acquire => {
                        let success = selected.context_key.as_deref().and_then(|key| key.strip_prefix("target:")).and_then(|index| index.parse::<usize>().ok()).map(|field_index| Self::acquire_target(organism, environment, field_index)).unwrap_or(false);
        ...
        for id in reproduction_requests {
            if let Some(organism) = self.organisms.iter_mut().find(|o| o.id == id) {
                let _ = crate::reproduction::begin_reproduction(organism, &mut self.rng, &catalog, &mut self.energy_ledger);
            }
        }
    ...
    pub(crate) fn apply_energy_capacity(organism: &mut Organism, environment: &Environment, ledger: &mut EnergyLedger) -> bool {
