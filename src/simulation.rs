...
impl Simulation {
    ...
    pub(crate) fn create_initial_organism() -> Organism {
        ...
        let hypothetical = genome.structural_blueprint.realize(&catalog).expect("initial structural blueprint must be realizable");
        let mut bootstrap_energy = 1.0e12_f64;
        let structure = crate::construction_authority::commit_candidate_structure(&crate::structure::OrganismStructure::new(), &hypothetical, &catalog, &mut bootstrap_energy, 0.0).expect("initial structural blueprint must be physically constructible through COMBINE");
        let mature_mass = genome.structural_blueprint.structural_mass(&catalog).max(f64::EPSILON);
        let realized_mass: f64 = structure.units.iter().map(|unit| unit.material.mass(&catalog)).sum();
    ...
    pub(crate) fn step(&mut self){
        ...
        let environment_snapshot=self.environment.clone();let decision_parameters=self.decision_parameters;for organism in &mut self.organisms{organism.age+=1;Self::update_development_stage(organism,&environment_snapshot);organism.apply_aging();organism.apply_maintenance(&environment_snapshot.catalog,&mut self.energy_ledger);Self::update_resource_perception(organism,&environment_snapshot);Self::update_memory_from_sources(organism,&environment_snapshot);Self::update_reproductive_readiness(organism,&environment_snapshot,decision_parameters);}
        ...
        let catalog=self.environment.catalog.clone();for id in reproduction_requests{if let Some(organism)=self.organisms.iter_mut().find(|o|o.id==id){let _=crate::reproduction::begin_reproduction(organism,&mut self.rng,&catalog,crate::combine_runtime::water_field_amount(&self.environment,organism));}}
        let mut offspring=Vec::new();let mut next_organism_id=self.next_organism_id;for organism in &mut self.organisms{if organism.reproductive_construction.is_some(){let outcome={let water=crate::combine_runtime::water_field_amount(&self.environment,organism);let construction=organism.reproductive_construction.as_mut().expect("construction checked above");crate::reproduction::advance_construction(&mut organism.stored_material,construction,&catalog,&mut organism.usable_energy,water)};match outcome{crate::reproduction::ConstructionAdvance::Progress{remaining,blueprint_index,stress,structure_indices}=>{organism.stored_material=remaining;let construction=organism.reproductive_construction.as_mut().expect("construction remains active after progress");construction.realized_elements.push(blueprint_index);construction.realized_constituent_groups.push(structure_indices);organism.add_transaction_stress(stress);}crate::reproduction::ConstructionAdvance::WaitingForMaterial=>{}crate::reproduction::ConstructionAdvance::WaitingForEnergy=>{}crate::reproduction::ConstructionAdvance::DevelopmentalFailure=>{let _=crate::reproduction::fail_reproduction(organism);}}if organism.reproductive_construction.as_ref().map(|construction|construction.realized_elements.len()==construction.target_elements.len()).unwrap_or(false){let child_id=next_organism_id.to_string();if let Some(child)=crate::reproduction::finish_reproduction(organism,child_id){next_organism_id+=1;offspring.push(child);}}}}self.next_organism_id=next_organism_id;self.organisms.extend(offspring);
