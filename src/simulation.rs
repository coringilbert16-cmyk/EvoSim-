...
impl Simulation {
    ...
    pub(crate) fn step_environment(&mut self) {
        apply_vents(&mut self.environment.field, &self.environment.catalog, &mut self.environment.vents, &mut self.rng);
    ...
    fn update_development_stage(organism: &mut Organism, environment: &Environment) {
        match organism.development_stage {
            DevelopmentStage::Offspring => { if organism.reproductive_construction.is_none() { organism.development_stage = DevelopmentStage::Juvenile } }
            DevelopmentStage::Juvenile => { if Self::growth_fraction(organism, environment) >= ADULTHOOD_GROWTH_FRACTION { organism.development_stage = DevelopmentStage::Adult } }
    ...
    fn current_needs(organism: &Organism, environment: &Environment, parameters: DecisionParameters) -> CurrentNeeds {
        ...
        let _ = environment;
        CurrentNeeds { survival, reproduction: if matches!(organism.development_stage, DevelopmentStage::Adult) { 1.0 } else { 0.0 } }
    }
    ...
    fn acquisition_targets(organism: &Organism, environment: &Environment) -> Vec<usize> {
        ...
        if cell.physical_materials.iter().any(|material| material.is_realized() && !material.material.is_empty()) || cell.materials.iter().any(|material| !material.is_empty() && material.is_valid()) { vec![field_index] } else { Vec::new() }
    }
    ...
    fn action_eligibility(organism: &Organism, environment: &Environment) -> ActionEligibility {
        let can_build_from_storage = !organism.structure.units.is_empty() && organism.stored_material.count_unstructured() > 0;
        let can_join_existing_structure = organism.structure.units.len() >= 2;
        ActionEligibility { can_move: organism.active_transformation_id.is_none(), can_acquire: organism.active_transformation_id.is_none() && !Self::acquisition_targets(organism, environment).is_empty(), can_combine: organism.active_transformation_id.is_none() && (can_build_from_storage || can_join_existing_structure), can_break: organism.active_transformation_id.is_none() && !organism.structure.bonds.is_empty(), can_expel: false }
    }
    ...
    fn decision_candidates(organism: &Organism, environment: &Environment, needs: CurrentNeeds, eligibility: ActionEligibility) -> Vec<ActionCandidate> {
        ...
        if relevant(ActionKind::Acquire) { candidates.extend(Self::acquisition_targets(organism, environment).into_iter().map(|field_index| ActionCandidate { action: ActionKind::Acquire, context_key: Some(Self::acquisition_context_key(field_index)) })); }
        if relevant(ActionKind::Expel) { candidates.push(ActionCandidate { action: ActionKind::Expel, context_key: None }); }
        candidates
    }

    pub(crate) fn acquire_target(organism: &mut Organism, environment: &mut Environment, field_index: usize) -> bool {
        ...
        if let Some(physical) = environment.field.take_physical_for_acquisition(field_index) {
            ...
            if organism.stored_material.store_physical(material, placements, &environment.catalog) { return true; }
            let _ = environment.field.deposit_physical_at_index(field_index, physical);
            return false;
        }
    ...
    fn recycle_dead_organism(environment: &mut Environment, organism: &mut Organism, ledger: &mut EnergyLedger) -> Option<crate::decomposition::DecomposingBody> {
        let position = organism.occupied_cells.first().cloned()?;
        for material in organism.stored_material.materials.iter().cloned() { environment.field.deposit(position.x, position.y, material); }
        if let Some(construction) = &organism.reproductive_construction { for material in construction.committed_material.materials.iter().cloned() { environment.field.deposit(position.x, position.y, material); } }
        let mut body = crate::decomposition::DecomposingBody::new(organism.structure.clone(), 0.0, position)?;
        let energy = organism.usable_energy;
        if !ledger.transfer(&mut organism.usable_energy, &mut body.energy_budget, energy) { return None; }
        Some(body)
    }
    ...
    fn process_decomposing_bodies(&mut self) {
        ...
        for index in 0..self.decomposing_bodies.len() {
            let Some(step) = crate::decomposition::resolve_one_bond_with_ledger(&mut self.decomposing_bodies[index], &self.environment, &mut self.energy_ledger) else { continue; };
            if step.net_energy > 0.0 { if let Some(organism_index) = crate::decomposition::harvestable_decomposition_energy(&self.organisms, &self.decomposing_bodies[index].position) { let amount = step.net_energy.min(self.decomposing_bodies[index].energy_budget); let (body, organism) = { let body = &mut self.decomposing_bodies[index].energy_budget; let organism = &mut self.organisms[organism_index].usable_energy; (body, organism) }; let _ = self.energy_ledger.transfer(body, organism, amount); } }
    ...
    pub(crate) fn step(&mut self) {
        ...
        let mut reproduction_requests = Vec::new();
        {
            ...
            for organism in organisms {
                ...
                match selected.action {
                    ActionKind::Move => { let moved = Self::update_movement(organism, environment); crate::decision_runtime::record_outcome(&mut organism.decision_history, &selected, if moved { crate::decision::OutcomeKind::Neutral } else { crate::decision::OutcomeKind::Harmful }); }
                    ActionKind::Combine => { let combined = crate::combine_runtime::try_combine(organism, environment, &mut compatibility_cache, &mut self.energy_ledger).is_some(); crate::decision_runtime::record_outcome(&mut organism.decision_history, &selected, if combined { crate::decision::OutcomeKind::Neutral } else { crate::decision::OutcomeKind::Harmful }); if matches!(organism.development_stage, DevelopmentStage::Adult) { reproduction_requests.push(organism.id.clone()); } }
                    ActionKind::Break => { if let Some(transformation) = Self::try_start_transformation(organism, &environment.catalog, &mut self.next_transformation_id, &selected) { self.active_transformations.push(transformation); } }
                    ActionKind::Acquire => { let success = selected.context_key.as_deref().and_then(|key| key.strip_prefix("target:")).and_then(|index| index.parse::<usize>().ok()).map(|field_index| Self::acquire_target(organism, environment, field_index)).unwrap_or(false); crate::decision_runtime::record_outcome(&mut organism.decision_history, &selected, if success { crate::decision::OutcomeKind::Neutral } else { crate::decision::OutcomeKind::Harmful }); }
                    ActionKind::Expel => {}
                }
            }
        }
        ...
        for organism in &mut self.organisms {
            if organism.reproductive_construction.is_some() {
                if let Some(construction) = organism.reproductive_construction.as_mut() { if let Some(stress) = crate::reproduction::advance_construction(&mut organism.stored_material, construction, &catalog, &mut self.energy_ledger, &mut organism.usable_energy) { organism.add_transaction_stress(stress); } }
                if organism.reproductive_construction.as_ref().map(|construction| construction.realized_elements.len() == construction.target_elements.len()).unwrap_or(false) { let child_id = next_organism_id.to_string(); if let Some(child) = crate::reproduction::finish_reproduction(organism, child_id, &catalog, &mut self.energy_ledger) { next_organism_id += 1; offspring.push(child); } }
            }
        }
        ...
        for mut organism in self.organisms.drain(..) { let dead = Self::apply_energy_capacity(&mut organism, &self.environment, &mut self.energy_ledger); if dead { if let Some(body) = Self::recycle_dead_organism(&mut self.environment, &mut organism, &mut self.energy_ledger) { self.decomposing_bodies.push(body); } } else { survivors.push(organism); } }
        ...
        self.active_transformations.retain(|t| live_ids.contains(&t.organism_id));
    ...
    pub(crate) fn total_material_in_system(&self) -> f64 {
        ...
        for organism in &self.organisms { total += organism.stored_material.total_amount(); if let Some(construction) = &organism.reproductive_construction { total += construction.committed_material.total_amount(); } total += organism.structure.units.iter().map(|unit| unit.material.total_amount()).sum::<f64>(); }
        for body in &self.decomposing_bodies { total += body.structure.units.iter().map(|unit| unit.material.total_amount()).sum::<f64>(); }
        total
    }
}
