    ...
    pub(crate) fn step(&mut self) {
        ...
        }
        {
            ...
            for index in 0..organisms.len() {
                ...
                // Movement is a separate decision channel. It can be selected
                // independently of processing, including choosing not to move.
                let movement_candidates = vec![
                    ActionCandidate {
                        action: ActionKind::NoTransaction,
                        context_key: None,
                    },
                ...
                let movement_context = DecisionContext {
                    needs,
                    eligibility: ActionEligibility {
                        can_move: true,
                        ..Default::default()
                ...
                if let Some(movement_selected) = crate::decision_runtime::select_action(
                    movement_context,
                    &organisms[index].decision_history,
                    &movement_candidates,
                    &mut self.rng,
                ) {
                    if movement_selected.action == ActionKind::Move {
                        let movement_before =
