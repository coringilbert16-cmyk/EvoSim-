...
impl Point {
    ...
    fn dot(self, o: Self) -> f64 { self.x * o.x + self.y * o.y }
...
impl GenomeCavity {
    pub fn qualifies(&self) -> bool { self.area > self.minimum_area + EPS }
}
...
pub fn minimum_genome_cavity_area(catalog: &[BaseResource]) -> Result<f64, String> {
    let carbon = catalog.iter().find(|r| r.name == "Carbon")
...
pub fn analyze_genome_cavity(
    ...
    for start in 0..edges.len() {
        ...
        for _ in 0..=edges.len() {
            ...
            face.push(current);
            let e = edges[current];
            let incoming = points[e.to].sub(points[e.from]);
            ...
            for &candidate in &outgoing[e.to] {
                ...
                let n = edges[candidate];
                let direction = points[n.to].sub(points[n.from]);
        ...
        let boundary_units = face.iter().filter_map(|&i| {
            ...
            polygons.iter().find_map(|(unit, p)| segment_in_polygon_boundary(a, b, p).then_some(*unit))
        }).collect();
...
fn boundary_bonds_are_sealed(structure: &OrganismStructure, polygons: &[(usize, Vec<Point>)]) -> bool {
    ...
    for i in 0..polygons.len() {
        for j in (i + 1)..polygons.len() {
            let touching = polygon_edges_touch(&polygons[i].1, &polygons[j].1);
            if touching {
...
fn polygon_edges_touch(a: &[Point], b: &[Point]) -> bool {
    (0..a.len()).any(|i| {
        ...
        (0..b.len()).any(|j| {
            ...
            (a0.sub(b0).norm() <= NODE_TOLERANCE && a1.sub(b1).norm() <= NODE_TOLERANCE)
                || (a0.sub(b1).norm() <= NODE_TOLERANCE && a1.sub(b0).norm() <= NODE_TOLERANCE)
                || (a0.sub(b0).norm() <= NODE_TOLERANCE && a1.sub(b0).norm() <= NODE_TOLERANCE)
                || (a0.sub(b1).norm() <= NODE_TOLERANCE && a1.sub(b1).norm() <= NODE_TOLERANCE)
        })
    })
}
...
fn segment_in_polygon_boundary(a: Point, b: Point, polygon: &[Point]) -> bool {
    (0..polygon.len()).any(|i| {
        ...
        (p.sub(a).norm() <= NODE_TOLERANCE && q.sub(b).norm() <= NODE_TOLERANCE) || (p.sub(b).norm() <= NODE_TOLERANCE && q.sub(a).norm() <= NODE_TOLERANCE)
    })
}
...
mod tests {
    ...
    fn seed_core_exceeds_three_joined_carbon_reference() {
        ...
        let cavity = analyze_genome_cavity(&structure, &catalog, &genome.structural_blueprint.core_elements).unwrap().expect("seed core must be sealed");
        assert!(cavity.qualifies(), "cavity={cavity:?}");
    }
    #[test]
    fn unbonded_core_is_not_a_genome() {
        let catalog = default_catalog(); let genome = initial_genome();
        let mut structure = genome.structural_blueprint.realize(&catalog).unwrap(); structure.bonds.clear();
        assert!(analyze_genome_cavity(&structure, &catalog, &genome.structural_blueprint.core_elements).unwrap().is_none());
    }
}
