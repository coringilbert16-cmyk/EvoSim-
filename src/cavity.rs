...
pub fn minimum_genome_cavity_area(catalog: &[BaseResource]) -> Result<f64, String> {
    ...
    if !minimum.is_finite() || minimum <= 0.0 { return Err("Carbon reference cavity area is invalid".into()); }
    Ok(minimum)
}
...
pub fn analyze_genome_cavity(
    ...
    for &unit_index in core_units {
        ...
        if vertices.len() < 3 { return Err(format!("genome core unit {unit_index} has no closed boundary")); }
    ...
    for (index, edge) in edges.iter().enumerate() { outgoing[edge.from].push(index); }
    ...
    for start in 0..edges.len() {
        ...
        for _ in 0..=edges.len() {
            ...
            for &candidate in &outgoing[edge.to] {
                if candidate == reverse_edge(current) || (visited[candidate] && candidate != start) { continue; }
        ...
        let signed_area = face.iter()
            .map(|&edge_index| points[edges[edge_index].from].cross(points[edges[edge_index].to]))
            .sum::<f64>() * 0.5;
        if signed_area <= EPS { continue; }
        let sample_edge = edges[face[0]];
        let a = points[sample_edge.from]; let b = points[sample_edge.to];
        let tangent = b.sub(a); let length = tangent.norm();
        if length <= EPS { continue; }
        let midpoint = a.add(b).scale(0.5);
        let sample = midpoint.add(Point { x: -tangent.y / length, y: tangent.x / length }.scale(NODE_TOLERANCE * 10.0));
        if polygons.iter().any(|(_, polygon)| point_in_polygon(sample, polygon)) { continue; }
        let boundary_units = face.iter().filter_map(|&edge_index| {
            let from = points[edges[edge_index].from]; let to = points[edges[edge_index].to];
            polygons.iter().find_map(|(unit_index, polygon)| polygon_contains_segment(polygon, from, to).then_some(*unit_index))
        }).collect::<Vec<_>>();
        let candidate = GenomeCavity { area: signed_area, boundary_units, minimum_area };
        if best.as_ref().map_or(true, |current| candidate.area > current.area) { best = Some(candidate); }
    }
    Ok(best)
}

fn reverse_edge(index: usize) -> usize { index ^ 1 }
fn intern_point(point: Point, points: &mut Vec<Point>, point_index: &mut HashMap<(i64, i64), usize>) -> usize {
    let key = ((point.x / NODE_TOLERANCE).round() as i64, (point.y / NODE_TOLERANCE).round() as i64);
    if let Some(&index) = point_index.get(&key) { if points[index].sub(point).norm() <= NODE_TOLERANCE { return index; } }
    let index = points.len(); points.push(point); point_index.insert(key, index); index
}
fn transformed_polygon(form: &Form, placement: crate::structure::Placement) -> Option<Vec<Point>> {
    ...
    Some(vertices.into_iter().map(|(x, y)| {
        ...
        Point { x: placement.x + x * c - y * s, y: placement.y + x * s + y * c }
    }).collect())
}
...
mod tests {
    ...
    fn three_carbon_reference_is_derived_from_catalog_geometry() {
        let catalog = default_catalog(); let carbon = catalog.iter().find(|r| r.name == "Carbon").unwrap();
        let expected = 3.0 * form_area(&carbon.shape.form).expect("Carbon must have 2D area");
    ...
    fn initial_core_encloses_more_than_three_joined_carbons() {
        ...
        let cavity = analyze_genome_cavity(&structure, &catalog, &genome.structural_blueprint.core_elements).unwrap().expect("seed core must enclose a physical cavity");
    ...
    fn opening_the_core_fails_cavity_qualification() {
        ...
        let cavity = analyze_genome_cavity(&structure, &catalog, &genome.structural_blueprint.core_elements).unwrap();
