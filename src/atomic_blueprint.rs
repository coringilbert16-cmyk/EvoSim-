...
impl AtomicBlueprint {
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (i, bond) in self.bonds.iter().enumerate() {
            ...
            if bond.required_bonds != 1 {
                return Err("atomic blueprint currently permits exactly one physical bond per prescribed relationship".into());
            }
    ...
    pub fn realize(&self, catalog: &[BaseResource]) -> Result<OrganismStructure, String> {
        ...
        for prescribed in &self.bonds {
            ...
            if !strength.is_finite() || !(0.0..=1.0).contains(&strength) {
                return Err("prescribed bond has invalid physical strength".into());
            }
            ...
            let bond = Bond {
                endpoint_a: BondEndpoint::new(id_a, prescribed.endpoint_a),
                endpoint_b: BondEndpoint::new(id_b, prescribed.endpoint_b),
                strength,
                bond_energy: 0.0,
            };
    ...
    /// One-time deterministic compiler for the existing seed. It uses the
    /// legacy construction path only to recover the already-established seed
    /// topology; the returned AtomicBlueprint contains every atom transform
    /// and every physical endpoint explicitly.
    #[cfg(test)]
    pub(crate) fn compile_seed_from_legacy(
        legacy: &StructuralBlueprint,
        catalog: &[BaseResource],
    ) -> Result<Self, String> {
        legacy.validate()?;
        let mut structure = OrganismStructure::new();
        let mut groups = HashMap::<usize, Vec<usize>>::new();
        for (index, element) in legacy.elements.iter().enumerate() {
        ...
        for connection in &legacy.connections {
            crate::structural_blueprint::realize_connection_groups(
                &mut structure,
                &groups,
                *connection,
                catalog,
        ...
        for (element_index, element) in legacy.elements.iter().enumerate() {
            let (sin_frame, cos_frame) = element.placement.rotation_radians.sin_cos();
            for &unit_index in groups.get(&element_index).ok_or_else(|| "seed compiler lost an element group".to_string())? {
                ...
                let dx = unit.placement.x - element.placement.x;
                let dy = unit.placement.y - element.placement.y;
                let x = dx * cos_frame + dy * sin_frame;
                let y = -dx * sin_frame + dy * cos_frame;
                unit_to_atom[unit_index] = atoms.len();
                atoms.push(BlueprintAtom {
                    resource: unit.resource_name().ok_or_else(|| "seed compiler encountered a non-atomic unit".to_string())?.to_string(),
                    transform: BlueprintTransform {
                        x,
                        y,
                        rotation_radians: unit.placement.rotation_radians - element.placement.rotation_radians,
                    },
                });
            }
        }

        let id_to_unit = structure
            .units
            .iter()
            .enumerate()
            .map(|(index, unit)| (unit.physical_id, index))
            .collect::<HashMap<_, _>>();
        ...
        for bond in &structure.bonds {
            ...
            bonds.push(BlueprintBond {
                atom_a: unit_to_atom[unit_a],
                endpoint_a: bond.endpoint_a.location,
                atom_b: unit_to_atom[unit_b],
        ...
        for &element in &legacy.core_elements {
            core_atoms.extend(groups.get(&element).ok_or_else(|| "seed compiler lost a core group".to_string())?.iter().map(|&unit| unit_to_atom[unit]));
        }
        let result = Self {
...
mod tests {
    ...
    #[test]
    fn seed_topology_compiles_to_exact_atomic_graph() {
        let legacy = crate::genome::initial_genome().structural_blueprint;
        let catalog = crate::resources::default_catalog();
        let blueprint = AtomicBlueprint::compile_seed_from_legacy(&legacy, &catalog).unwrap();
        assert_eq!(blueprint.atoms.len(), 183);
        assert_eq!(blueprint.core_atoms.len(), 57);
        assert_eq!(blueprint.bonds.len(), 278);
        let structure = blueprint.realize(&catalog).unwrap();
        assert_eq!(structure.units.len(), 183);
        assert_eq!(structure.bonds.len(), 278);
    }
}
