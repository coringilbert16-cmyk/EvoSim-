...
impl AtomicBlueprint {
    ...
    pub(crate) fn compile_seed_from_legacy(legacy: &StructuralBlueprint, catalog: &[BaseResource]) -> Result<Self, String> {
        ...
        for (index, element) in legacy.elements.iter().enumerate() { let ids = crate::construction_realization::realize_material(&mut structure, element, catalog).map_err(|error| format!("seed element {index} ({}) failed: {error}", element.material.parts.iter().map(|(name, _)| name.as_str()).collect::<Vec<_>>().join("+")))?; groups.insert(index, ids); }
        ...
        for (element_index, _element) in legacy.elements.iter().enumerate() { for &unit_index in groups.get(&element_index).ok_or_else(|| "seed compiler lost an element group".to_string())? { let unit = structure.units.get(unit_index).ok_or_else(|| "seed compiler lost a unit".to_string())?; unit_to_atom[unit_index] = atoms.len(); atoms.push(BlueprintAtom { resource: unit.resource_name().ok_or_else(|| "seed compiler encountered a non-atomic unit".to_string())?.to_string(), transform: BlueprintTransform { x: unit.placement.x, y: unit.placement.y, rotation_radians: unit.placement.rotation_radians } }); } }
