...
impl AtomicBlueprint {
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (i, bond) in self.bonds.iter().enumerate() {
            ...
            if self.bonds[..i].iter().map(|previous| previous.canonical()).any(|previous| previous == canonical) { return Err("atomic blueprint contains duplicate bonds".into()); }
        }
        Ok(())
    }
    ...
    pub fn realize(&self, catalog: &[BaseResource]) -> Result<OrganismStructure, String> {
        ...
        for atom in &self.atoms {
            let placement = Placement { x: self.anchor.x + atom.transform.x * cos_anchor - atom.transform.y * sin_anchor, y: self.anchor.y + atom.transform.x * sin_anchor + atom.transform.y * cos_anchor, rotation_radians: self.anchor.rotation_radians + atom.transform.rotation_radians };
            let mut unit = StructuralUnit::new(atom.resource.clone(), placement);
            if !unit.realize_default_geometry(catalog) { return Err(format!("atomic blueprint references invalid resource {}", atom.resource)); }
            ids.push(structure.add_unit(unit));
        }
        for prescribed in &self.bonds {
            let a = ids[prescribed.atom_a]; let b = ids[prescribed.atom_b];
            ...
            if (point_a.x - point_b.x).hypot(point_a.y - point_b.y) > CONTACT_EPSILON { return Err(format!("prescribed bond between atoms {} and {} is not physically realized", prescribed.atom_a, prescribed.atom_b)); }
            ...
            if crate::contact::try_add_bond(&mut structure, bond, catalog).is_err() { return Err(format!("prescribed bond between atoms {} and {} is physically invalid", prescribed.atom_a, prescribed.atom_b)); }
        }
        Ok(structure)
    }
    ...
    pub(crate) fn compile_seed_from_legacy(legacy: &StructuralBlueprint, catalog: &[BaseResource]) -> Result<Self, String> {
        ...
        for (element_index, element) in legacy.elements.iter().enumerate() {
            ...
            for &unit_index in groups.get(&element_index).ok_or_else(|| "seed compiler lost an element group".to_string())? {
                ...
                let dx = unit.placement.x - element.placement.x; let dy = unit.placement.y - element.placement.y;
                unit_to_atom[unit_index] = atoms.len();
                atoms.push(BlueprintAtom { resource: unit.resource_name().ok_or_else(|| "seed compiler encountered a non-atomic unit".to_string())?.to_string(), transform: BlueprintTransform { x: dx * cos_frame + dy * sin_frame, y: -dx * sin_frame + dy * cos_frame, rotation_radians: unit.placement.rotation_radians - element.placement.rotation_radians } });
            }
        }
