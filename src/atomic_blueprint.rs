...
impl AtomicBlueprint {
    ...
    pub(crate) fn compile_seed_from_legacy(legacy: &StructuralBlueprint, catalog: &[BaseResource]) -> Result<Self, String> {
        ...
        for (index, element) in legacy.elements.iter().enumerate() { let ids = crate::seed_legacy_realization::realize_material(&mut structure, element, catalog).map_err(|error| format!("seed element {index} ({}) failed: {error}", element.material.parts.iter().map(|(name, _)| name.as_str()).collect::<Vec<_>>().join("+")))?; groups.insert(index, ids); }
