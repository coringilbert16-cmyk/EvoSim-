...
pub fn apply_settling(field: &mut ActiveMaterialField, reservoir: &mut DeepReservoir, fraction: f64) {
    ...
    for field_index in 0..field.cells.len() {
        ...
        if bonded_total > MATERIAL_EPSILON { let outflow = bonded_total * fraction; if outflow > MATERIAL_EPSILON { if let Some(taken) = field.cells[field_index].bonded.take(outflow) { for (name, amount) in taken.parts { reservoir.cells[reservoir_index].add(true, &name, amount); } } } }
        let unbonded_total = field.cells[field_index].unbonded.total_amount();
        if unbonded_total > MATERIAL_EPSILON { let outflow = unbonded_total * fraction; if outflow > MATERIAL_EPSILON { if let Some(taken) = field.cells[field_index].unbonded.take(outflow) { for (name, amount) in taken.parts { reservoir.cells[reservoir_index].add(false, &name, amount); } } } }
    }
}
