...
impl CorePairKind {
    ...
    pub fn carbon(self) -> &'static str {
        "Carbon"
    }
    pub fn partner(self) -> &'static str {
        match self {
            Self::Cm => "Methane",
            Self::Ch => "Hydrogen",
            Self::Cs => "Sulfur",
...
pub fn build_f_core(catalog: &[BaseResource]) -> Option<CoreGeometry> {
    ...
    let partner_radii = [
        resource_bounding_radius(catalog, "Methane")?,
        resource_bounding_radius(catalog, "Hydrogen")?,
        resource_bounding_radius(catalog, "Sulfur")?,
    ];
    let pair_outer_envelopes = partner_radii.map(|partner| {
        let separation = carbon_radius + partner;
        separation / 2.0 + partner
    });
    let pair_inner_envelopes = partner_radii.map(|partner| {
        let separation = carbon_radius + partner;
        separation / 2.0 + carbon_radius
    });
    ...
    for index in 0..6 {
        ...
        units.push(CoreUnitPlacement {
            kind,
            center_x,
            center_y,
            radial_angle_radians: angle,
            carbon: CoreConstituentPlacement {
                resource_name: kind.carbon(),
                x: center_x - separation / 2.0 * radial_x,
                y: center_y - separation / 2.0 * radial_y,
                rotation_radians: angle,
            },
            partner: CoreConstituentPlacement {
                resource_name: kind.partner(),
                x: center_x + separation / 2.0 * radial_x,
                y: center_y + separation / 2.0 * radial_y,
                rotation_radians: angle,
            },
        ...
        angle -= 2.0
            * ((pair_outer_envelopes[current] + pair_outer_envelopes[next]) / (2.0 * ring_radius))
                .asin();
    }
    Some(CoreGeometry {
        units,
...
fn solve_ring_radius(envelopes: &[f64; 3], sequence: &[usize; 6]) -> Option<f64> {
    let max_sum = (0..6)
        .map(|i| envelopes[sequence[i]] + envelopes[sequence[(i + 1) % 6]])
    ...
    for _ in 0..100 {
        ...
        let angle_sum = (0..6)
            .map(|i| {
                let sum = envelopes[sequence[i]] + envelopes[sequence[(i + 1) % 6]];
                2.0 * (sum / (2.0 * radius)).asin()
            })
            .sum::<f64>();
        if angle_sum > std::f64::consts::TAU {
            low = radius;
        } else {
            high = radius;
        }
...
mod tests {
    ...
    #[test]
    fn f_core_has_six_units_in_locked_sequence() {
        let g = build_f_core(&default_catalog()).unwrap();
        assert_eq!(g.units.len(), 6);
        assert_eq!(
            g.units.iter().map(|u| u.kind).collect::<Vec<_>>(),
            CorePairKind::F_SEQUENCE.to_vec()
        );
    }
    #[test]
    fn f_core_has_two_of_each_pathway() {
        let g = build_f_core(&default_catalog()).unwrap();
        let mut c = [0; 3];
        for u in &g.units {
            match u.kind {
                CorePairKind::Cm => c[0] += 1,
                CorePairKind::Ch => c[1] += 1,
                CorePairKind::Cs => c[2] += 1,
            }
        }
        assert_eq!(c, [2, 2, 2]);
    }
    ...
    fn f_core_is_closed() {
        let g = build_f_core(&default_catalog()).unwrap();
        let c = resource_bounding_radius(&default_catalog(), "Carbon").unwrap();
        let p = [
            resource_bounding_radius(&default_catalog(), "Methane").unwrap(),
            resource_bounding_radius(&default_catalog(), "Hydrogen").unwrap(),
            resource_bounding_radius(&default_catalog(), "Sulfur").unwrap(),
        ];
        let e = p.map(|r| (c + r) / 2.0 + r);
        let i = [0, 1, 2, 0, 1, 2];
        for n in 0..6 {
            let a = g.units[n];
            let b = g.units[(n + 1) % 6];
            let d = ((a.center_x - b.center_x).powi(2) + (a.center_y - b.center_y).powi(2)).sqrt();
    ...
    fn core_is_hollow() {
        let g = build_f_core(&default_catalog()).unwrap();
        assert!(g.cavity_radius > 0.0);
        assert!(g.outer_radius > g.ring_radius);
    }
}
