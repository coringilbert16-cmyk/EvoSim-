...
impl Simulation {
    ...
    pub(crate) fn resolve_transformation(
        ...
        let Some(target_bond) = transformation.bond else {
            panic!("BREAK resolution invoked with no target bond: transformation={:?}", transformation.decision_context_key);
        };
        ...
        let Some(matching_index) = matching_index else {
            panic!(
                "BREAK resolution target missing: target={target_bond:?}, bonds={:?}, context={:?}",
