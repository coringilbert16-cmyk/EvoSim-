...
fn need_pressure(action:ActionKind,needs:CurrentNeeds)->f64{action.relevant_needs().iter().map(|need|needs.pressure(*need)).fold(0.0_f64,|best,p|best.max(p))}
fn history_adjustment(history:&DecisionHistory,candidate:&ActionCandidate)->f64{match history.outcome(candidate.action,candidate.context_key.as_deref()){Some(OutcomeKind::Beneficial)=>HISTORY_INFLUENCE,Some(OutcomeKind::Harmful)=>-HISTORY_INFLUENCE,Some(OutcomeKind::Neutral)|None=>0.0}}
pub fn select_action(context:DecisionContext,history:&DecisionHistory,candidates:&[ActionCandidate])->Option<ActionCandidate>{let mut best:Option<(f64,ActionCandidate)>=None;for candidate in candidates{if approve(context,candidate.action)!=DecisionResult::Approve{continue}let score=need_pressure(candidate.action,context.needs)+history_adjustment(history,candidate);if best.as_ref().map_or(true,|(s,_)|score>*s){best=Some((score,candidate.clone()));}}best.map(|(_,candidate)|candidate)}
