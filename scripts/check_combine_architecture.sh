...
# New external bonds may only be admitted through COMBINE. Pre-existing
# internal bonds carried by an already-physical stored material are restored
...
while IFS= read -r file; do
  ...
  if awk '/#\[cfg\(test\)\]/{exit} /\bBond[[:space:]]*\{/ {print; found=1} END{exit found ? 0 : 1}' "$file"; then
    echo "ERROR: production Bond construction found outside COMBINE or material restoration in $file"
    fail=1
  fi
