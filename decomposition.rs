...
///
/// Biological identity ends at death, but the physical structure persists so
/// its organism-level bonds can be dismantled progressively through the same
/// BREAK energy model used by living organisms.
#[derive(Clone, Debug)]
...
pub(crate) fn resolve_one_bond(
    ...
    let a = body.structure.units.get(target.unit_a)?.properties(&environment.catalog)?;
    let b = body.structure.units.get(target.unit_b)?.properties(&environment.catalog)?;
    ...
    let released_material = if body.is_finished() {
        Some(
            body.structure
                .units
                .iter()
                .map(|unit| unit.material.material().clone())
                .collect(),
        )
    } else {
        None
...
pub(crate) fn harvestable_decomposition_energy(
    ...
    organisms
        ...
        .filter_map(|(index, organism)| {
            ...
            let distance = (organism_position.x - position.x)
                .hypot(organism_position.y - position.y);
            Some((index, distance))
        })
