...
impl Simulation {
    ...
    // ========================================================
    // ACQUISITION  (physical-contact based)
    // ========================================================
    //
    // Locked model: PHYSICAL ACCESS -> GENOMIC PREFERENCE ->
    // ACQUISITION -> MATERIAL TRANSFER.
    //
    //   PHYSICAL ACCESS: only cells within PROCESSING_REACH - the
    //   existing "how far can this organism physically act" constant,
    //   reused here as the organism's physical contact envelope
    //   radius rather than inventing a second reach quantity - are
    //   eligible at all.
    //
    //   GENOMIC PREFERENCE: among physically accessible UNBONDED
    //   observations, the organism prefers whichever has the highest
    //   `desirability`. This REUSES the existing genome-affinity-
    //   driven desirability already computed this same tick during
    //   perception (mass/potential_energy/reactivity/cohesion
    //   affinities) rather than inventing a new preference trait -
    //   it already IS a general, evolvable acquisition-preference
    //   signal. Bonded material is deliberately excluded: this
    //   acquisition path only ever feeds stored_unbonded (see
    //   Organism::store_unbonded_material's TODO on bonded material).
    //
    //   ACQUISITION AMOUNT: bounded by physical contact area, using
    //   the existing nominal-unit-area invariant. The organism's
    //   envelope circle and the target cell's own circular footprint
    //   (radius = half the cell size - the same circular
    //   approximation ActiveMaterialField::cells_within_radius
    //   already uses for cells) overlap in some area; dividing that
    //   area by NOMINAL_UNIT_AREA gives how many resource units are
    //   physically contacted. This is an explicit, isolated
    //   approximation (see math::circle_overlap_area) - the field is
    //   a discretized grid, not continuous particle geometry, so this
    //   is not pretended to be exact; only this one calculation would
    //   need to change if a more precise model replaces it later.
    //
    //   TRANSFER: exactly the computed (and availability-capped)
    //   amount moves from the field cell to organism.stored_unbonded.
    //   No energy, no bonds, no StructuralUnits are created here.
    // ========================================================

    fn try_acquire_material(organism: &mut Organism, field: &mut ActiveMaterialField) {
        let (px, py) = {
            let p = &organism.occupied_cells[0];
            (p.x, p.y)
        };
        ...
        let target = organism
            ...
            .sensed_resources
            .iter()
            .filter(|r| !r.bonded && r.distance <= PROCESSING_REACH)
            .max_by(|a, b| {
                a.desirability
        ...
        let (cell_x, cell_y) = field.cell_center(target.field_index);
        let dx = cell_x - px;
        let dy = cell_y - py;
        let distance = (dx * dx + dy * dy).sqrt();

        let cell_radius = field.cell_size / 2.0;
        let overlap_area = math::circle_overlap_area(PROCESSING_REACH, cell_radius, distance);
        let max_by_contact_area = overlap_area / resources::NOMINAL_UNIT_AREA;

        if max_by_contact_area <= 0.0 {
            return;
        }

        if let Some(taken) = field.take_at_index(target.field_index, false, max_by_contact_area) {
            organism.store_unbonded_material(taken);
        }
    }
    ...
    fn step(&mut self) -> Snapshot {
        ...
        for organism in &mut self.organisms {
            Self::try_acquire_material(organism, &mut self.environment.field);
        }
...
mod integration_tests {
    ...
    #[test]
    fn inaccessible_material_cannot_be_acquired() {
        ...
        // Place unbonded material far outside PROCESSING_REACH.
        sim.environment.field.deposit(
            px + 500.0,
            py,
            Material { parts: vec![("Carbon".into(), 20.0)], bonded: false },
        );

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        assert_eq!(sim.organisms[0].stored_unbonded.total_amount(), 0.0);
    }

    #[test]
    fn accessible_unbonded_material_can_be_acquired() {
        let mut sim = Simulation::new(1, 10.0);
        let (px, py) = {
            let p = &sim.organisms[0].occupied_cells[0];
            (p.x, p.y)
        ...
        sim.environment.field.deposit(px, py, Material { parts: vec![("Carbon".into(), 20.0)], bonded: false });

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        assert!(sim.organisms[0].stored_unbonded.total_amount() > 0.0);
    }

    #[test]
    fn acquisition_cannot_exceed_available_material() {
        let mut sim = Simulation::new(1, 10.0);
        let (px, py) = {
            let p = &sim.organisms[0].occupied_cells[0];
            (p.x, p.y)
        ...
        // Deposit a tiny amount - far less than contact area would allow.
        sim.environment.field.deposit(px, py, Material { parts: vec![("Carbon".into(), 0.05)], bonded: false });

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        assert!(
            sim.organisms[0].stored_unbonded.total_amount() <= 0.05 + 1e-9,
            "acquisition must never exceed what was actually present"
        );
    }

    #[test]
    fn acquisition_is_bounded_by_physical_contact_area() {
        let mut sim = Simulation::new(1, 10.0);
        let (px, py) = {
            let p = &sim.organisms[0].occupied_cells[0];
            (p.x, p.y)
        ...
        // Deposit far more than contact area could possibly allow.
        sim.environment.field.deposit(px, py, Material { parts: vec![("Carbon".into(), 100_000.0)], bonded: false });

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        let cell_radius = sim.environment.field.cell_size / 2.0;
        let max_possible_area = std::f64::consts::PI * (PROCESSING_REACH.min(cell_radius)).powi(2).max(
            std::f64::consts::PI * cell_radius.powi(2),
        );
        let max_possible_amount = max_possible_area / resources::NOMINAL_UNIT_AREA;

        assert!(
            sim.organisms[0].stored_unbonded.total_amount() <= max_possible_amount + 1e-6,
            "acquired amount must be bounded by contact area, not by the huge deposit"
        );
        assert!(sim.organisms[0].stored_unbonded.total_amount() < 100_000.0);
    }

    #[test]
    fn acquired_material_enters_stored_unbonded_and_field_decreases_by_exactly_that_amount() {
        let mut sim = Simulation::new(1, 10.0);
        let (px, py) = {
            let p = &sim.organisms[0].occupied_cells[0];
            (p.x, p.y)
        ...
        let field_before = sim.environment.field.total_amount();

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        let acquired = sim.organisms[0].stored_unbonded.total_amount();
        let field_after = sim.environment.field.total_amount();

        assert!(acquired > 0.0);
        assert!((field_before - field_after - acquired).abs() < 1e-9);
    }

    #[test]
    fn acquisition_conserves_total_material_over_many_ticks() {
        let mut sim = Simulation::new(1, 10.0);
    ...
    #[test]
    fn genomic_preference_selects_among_multiple_accessible_resources() {
        // Give the organism a strong potential_energy affinity so
        // desirability clearly favors the higher-potential-energy
        // resource (Methane, pe=20) over the lower one (Nitrogen, pe=0.5).
        let mut sim = Simulation::new(1, 10.0);
        for t in &mut sim.organisms[0].genome.traits {
            if t.name == "potential_energy_affinity" {
                t.value = 1.0;
            }
        }

        // Two adjacent field cells, both within PROCESSING_REACH of
        // the organism placed at their midpoint - deliberately two
        // SEPARATE cells (not the same position) so this actually
        // tests choosing between two distinct observations, rather
        // than one cell's stack that would just merge both resources
        // into a single blended observation.
        let (cell_x, cell_y) = sim.environment.field.cell_center(0);
        let cell_size = sim.environment.field.cell_size;
        let neighbor_index = sim.environment.field.index_for_position(cell_x + cell_size, cell_y).unwrap();

        sim.organisms[0].occupied_cells[0].x = cell_x + cell_size / 2.0;
        sim.organisms[0].occupied_cells[0].y = cell_y;

        sim.environment.field.deposit_at_index(0, Material { parts: vec![("Nitrogen".into(), 20.0)], bonded: false });
        sim.environment.field.deposit_at_index(neighbor_index, Material { parts: vec![("Methane".into(), 20.0)], bonded: false });

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        let acquired_names: Vec<String> =
            sim.organisms[0].stored_unbonded.parts.iter().map(|(n, _)| n.clone()).collect();
        assert_eq!(acquired_names, vec!["Methane".to_string()], "should prefer the more desirable resource");
    }

    #[test]
    fn genomic_preference_cannot_select_inaccessible_material() {
        let mut sim = Simulation::new(1, 10.0);
        for t in &mut sim.organisms[0].genome.traits {
            if t.name == "potential_energy_affinity" {
                t.value = 1.0;
        ...
        // Preferred (high potential energy) resource is out of reach;
        // less-preferred resource is accessible.
        sim.environment.field.deposit(px + 500.0, py, Material { parts: vec![("Methane".into(), 20.0)], bonded: false });
        sim.environment.field.deposit(px, py, Material { parts: vec![("Nitrogen".into(), 20.0)], bonded: false });
        ...
        let acquired_names: Vec<String> =
            sim.organisms[0].stored_unbonded.parts.iter().map(|(n, _)| n.clone()).collect();
        assert_eq!(acquired_names, vec!["Nitrogen".to_string()], "physical access must override preference");
    }

    #[test]
    fn acquisition_never_creates_bonds_or_structural_units() {
        let mut sim = Simulation::new(1, 10.0);
        let (px, py) = {
            let p = &sim.organisms[0].occupied_cells[0];
            (p.x, p.y)
    ...
    #[test]
    fn acquisition_ignores_bonded_material() {
        let mut sim = Simulation::new(1, 10.0);
        let (px, py) = {
            let p = &sim.organisms[0].occupied_cells[0];
            (p.x, p.y)
        };
        sim.environment.field.deposit(px, py, Material { parts: vec![("Carbon".into(), 20.0)], bonded: true });

        Simulation::update_resource_perception(&mut sim.organisms[0], &sim.environment.clone());
        Simulation::try_acquire_material(&mut sim.organisms[0], &mut sim.environment.field);

        assert_eq!(sim.organisms[0].stored_unbonded.total_amount(), 0.0, "bonded material must not be acquired here");
    }
