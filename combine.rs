...
mod tests {
    ...
    fn candidate(load_a: f64, load_b: f64) -> ConnectionPairCandidate {
        ConnectionPairCandidate { point_a: 0, point_b: 0, distance: 0.0, facing: 1.0, load_a, load_b, available_a: true, available_b: true }
    }
    ...
    fn bonded_status_does_not_change_recipe_identity() {
        ...
        let bonded = Material { parts: unbonded.parts.clone(), bonded: true };
        assert_eq!(MaterialRecipeKey::from_material(&unbonded), MaterialRecipeKey::from_material(&bonded));
        assert_eq!(MaterialRecipeKey::from_inputs(&[unbonded]), MaterialRecipeKey::from_inputs(&[bonded]));
    }
    #[test]
    ...
    fn intrinsic_bond_strength_is_cohesion_only() {
        ...
        let altered = ResourceProperties { mass: 100.0, potential_energy: -500.0, reactivity: 0.0, cohesion: 0.2 };
    ...
    fn formation_uses_load_and_surplus() {
        ...
        assert!((formation_surplus(free, free.threshold) - 0.0).abs() < 1e-12);
        assert!(formation_surplus(free, free.threshold + 1.0) > 0.0);
    ...
    #[test]
    fn occupied_region_remains_eligible_for_additional_bonds() {
        ...
        assert!(crate::contact::try_add_bond(&mut structure, bond(a, point, b, other), &catalog).is_ok());
        let second = eligible_candidates(&structure, a, c, &catalog, &mut cache);
        assert!(!second.is_empty());
        assert!(second.iter().all(|candidate| candidate.available_a && candidate.available_b));
    }
}
