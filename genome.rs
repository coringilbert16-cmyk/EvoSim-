...
/// Seed body plan:
/// - rings 0..2: 19-unit Carbon F-Core, strong but not the six-unit minimum;
/// - ring 3: 18-unit Carbon+Water hydrated lattice, softer and permeable;
/// - ring 4: 24-unit Carbon+Nitrogen+Water hydrated membrane, slightly
///   firmer than the lattice while retaining permeability.
///
/// Carbon hexagons are arranged on a corner-contact triangular lattice. The
/// exact corner indices make every inherited structural bond physically
/// realizable rather than relying on approximate overlap.
fn default_structural_blueprint()->StructuralBlueprint{
 let scaffold_radius=0.438_691_f64;
 ...
 for q in -4_i32..=4_i32{
  for axial_r in -4_i32..=4_i32{
   ...
   let x=scaffold_radius*(2.0*q as f64+axial_r as f64);
   let y=scaffold_radius*3.0_f64.sqrt()*axial_r as f64;
   let index=elements.len();
   elements.push(BlueprintElement{material,placement:Placement{x,y,rotation_radians:0.0}});
 ...
 for q in -4_i32..=4_i32{
  for axial_r in -4_i32..=4_i32{
   ...
   for (dq,dr,point_a,point_b) in [(1_i32,0_i32,0_usize,3_usize),(0,1,1,4),(-1,1,2,5)]{
    let Some(&b)=index_by_axial.get(&(q+dq,axial_r+dr))else{continue};
    connections.push(BlueprintConnection{element_a:a,point_a,element_b:b,point_b});
   }
  }
 }
...
mod tests{
 ...
 #[test]fn seed_blueprint_has_strong_core_hydrated_lattice_and_hydrated_membrane(){let g=initial_genome();let b=&g.structural_blueprint;assert_eq!(b.elements.len(),61);assert!(b.validate().is_ok());assert!(b.connections.len()>b.elements.len());assert_eq!(b.elements[0].material.parts[0].0,"Carbon");let hydrated=b.elements.iter().filter(|e|e.material.parts.iter().any(|(n,_)|n=="Water")).count();assert_eq!(hydrated,42);assert!(b.realize(&default_catalog()).is_ok())}
