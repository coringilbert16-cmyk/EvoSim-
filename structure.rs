...
impl StructuralUnit {
    pub fn new(resource_name: impl Into<String>, placement: Placement) -> Self {
        Self { resource_name: resource_name.into(), placement }
    }
...
pub struct Bond {
    ...
    pub unit_b: usize,
    pub point_b: usize,
    /// Structural bond strength, fixed at formation.
    pub strength: f64,
    /// Energy stored in the bonded structural state. BREAK consumes/releases
    /// this stored value; it is never reconstructed from raw resource energy.
    #[serde(default)]
    pub bond_energy: f64,
}
...
mod tests {
    ...
    fn unit_properties_and_connection_sites_are_catalog_derived() {
        ...
        let i = unit(&mut structure, "Carbon", 0.0, 0.0);
        assert_eq!(structure.units[i].properties(&catalog).unwrap().cohesion, 0.95);
        match structure.units[i].connection_sites(&catalog).unwrap() {
    ...
    fn invalid_bond_energy_is_rejected() {
        let valid = bond(0, 0, 1, 0, 0.5, 1.0);
        assert!(valid.is_valid(2, |_| Some(1)));
    ...
    fn break_bond_returns_the_stored_energy_with_the_bond() {
        ...
        let a = unit(&mut s, "Carbon", 0.0, 0.0);
        let b = unit(&mut s, "Methane", 1.0, 0.0);
        let index = s.add_bond(bond(a, 0, b, 0, 0.8, 7.25));
        let removed = s.break_bond(index).unwrap();
    ...
    fn formation_threshold_is_symmetric_and_diminishing_with_load() {
        ...
        assert!(loaded > base && more_loaded > loaded);
        assert!(formation_threshold(0.9, 0.1, 3.0, 1.0).abs_diff(formation_threshold(0.1, 0.9, 1.0, 3.0)) < 1e-12);
    }
}