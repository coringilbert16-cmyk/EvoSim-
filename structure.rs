...
impl OrganismStructure {
    ...
    pub fn available_connection_sites(&self, _catalog: &[crate::resources::BaseResource]) -> Vec<ConnectionSiteRef> { self.units.iter().enumerate().flat_map(|(unit_index, unit)| (0..unit.geometry.connection_regions.len()).map(move |point_index| ConnectionSiteRef { unit_index, point_index })).collect() }
    ...
    pub fn component_connection_sites(&self, component: &[usize], catalog: &[crate::resources::BaseResource]) -> Vec<ConnectionSiteRef> { let component_set: std::collections::HashSet<usize> = component.iter().copied().collect(); self.available_connection_sites(catalog).into_iter().filter(|site| component_set.contains(&site.unit_index)).collect() }
    pub fn connection_load(&self, unit: usize, point: usize, catalog: &[crate::resources::BaseResource]) -> f64 { self.bonds.iter().filter(|b| b.touches(unit, point)).filter_map(|bond| { let props_a = self.units.get(bond.unit_a)?.properties(catalog)?; let props_b = self.units.get(bond.unit_b)?.properties(catalog)?; Some(crate::combine::bond_strength(*props_a, *props_b)) }).sum() }
...
mod tests {
    ...
    #[test] fn invalid_bond_energy_is_rejected() { assert!(bond(0, 0, 1, 0, 1.0).is_valid(2, |_| Some(1))); assert!(!bond(0, 0, 1, 0, -1.0).is_valid(2, |_| Some(1))); assert!(!bond(0, 0, 1, 0, f64::NAN).is_valid(2, |_| Some(1))); }
    #[test] fn connection_load_uses_intrinsic_strength() { let catalog = default_catalog(); let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); s.add_bond(bond(a, 0, b, 0, 9.0)); let expected = crate::combine::bond_strength(*s.units[a].properties(&catalog).unwrap(), *s.units[b].properties(&catalog).unwrap()); assert!((s.connection_load(a, 0, &catalog) - expected).abs() < 1e-12); assert_eq!(s.connection_count(a, 0), 1); }
    #[test] fn break_bond_returns_energy() { let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); let i = s.add_bond(bond(a, 0, b, 0, 7.25)); assert_eq!(s.break_bond(i).unwrap().bond_energy, 7.25); assert!(s.bonds.is_empty()); }
    #[test] fn break_matching_bond_uses_endpoint_identity() { let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); let stored = bond(a, 0, b, 0, 7.5); let snapshot = bond(b, 0, a, 0, 7.25); s.add_bond(stored); assert_eq!(s.break_matching_bond(snapshot), Some(stored)); }
