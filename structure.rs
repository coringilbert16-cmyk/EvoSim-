...
impl Deref for UnitProperties {
    type Target = ResourceProperties;
    fn deref(&self) -> &Self::Target { &self.0 }
}

/// A physical instance of one inherited blueprint element.
/// Material and geometry are authoritative runtime state; resource name is
/// intentionally not stored as a second identity.
...
impl StructuralUnit {
    /// Compatibility constructor for existing single-resource callers. It
    /// creates the complete material-bearing physical unit from immutable
    /// catalog data. Inherited organisms should use `from_blueprint`.
    pub fn new(resource_name: impl Into<String>, placement: Placement) -> Self {
        ...
        let shape = catalog
            ...
            .unwrap_or(crate::resources::Shape {
                form: crate::resources::Form::Circle { radius: 0.1 },
            });
        Self {
    ...
    pub fn from_blueprint(material: StructuralMaterial, geometry: BlueprintGeometry, placement: Placement) -> Self {
        Self { material, geometry, placement }
    }

    /// Properties are derived from the unit's material. No cached resource
    /// properties are stored on the physical unit.
    ...
    pub fn connection_sites(&self, _catalog: &[crate::resources::BaseResource]) -> Option<ConnectionSites> {
        Some(self.geometry.envelope.connection_sites())
    }
}
...
impl OrganismStructure {
    ...
    pub fn connection_site(&self, site: ConnectionSiteRef, _catalog: &[crate::resources::BaseResource]) -> Option<ConnectionPoint> {
        self.units.get(site.unit_index)?.geometry.connection_regions.get(site.point_index).map(|region| region.point)
    }

    /// Connection regions have no numerical bond capacity. All regions remain
    /// candidates; geometry/contact decides whether another bond can coexist.
    pub fn available_connection_sites(&self, _catalog: &[crate::resources::BaseResource]) -> Vec<ConnectionSiteRef> {
        self.units.iter().enumerate().flat_map(|(unit_index, unit)| {
            (0..unit.geometry.connection_regions.len()).map(move |point_index| ConnectionSiteRef { unit_index, point_index })
        }).collect()
    }
    ...
    pub fn connected_components(&self) -> Vec<Vec<usize>> {
        ...
        for bond in &self.bonds {
            if bond.unit_a >= self.units.len() || bond.unit_b >= self.units.len() { continue; }
        ...
        for start in 0..self.units.len() {
            ...
            while let Some(unit) = stack.pop() {
                ...
                for &neighbor in &adjacency[unit] {
                    if !visited[neighbor] { visited[neighbor] = true; stack.push(neighbor); }
                }
            }
    ...
    pub fn connection_load(&self, unit: usize, point: usize, catalog: &[crate::resources::BaseResource]) -> f64 {
        self.bonds.iter().filter(|b| b.touches(unit, point)).filter_map(|bond| {
    ...
    pub fn break_matching_bond(&mut self, target: Bond) -> Option<Bond> {
        let index = self.bonds.iter().position(|bond| bond.has_same_identity(&target))?;
        self.break_bond(index)
    }
    pub fn disconnect_point(&mut self, unit: usize, point: usize) -> Vec<Bond> {
        let mut removed = Vec::new(); let mut i = 0;
        while i < self.bonds.len() { if self.bonds[i].touches(unit, point) { removed.push(self.bonds.remove(i)); } else { i += 1; } }
        removed
    }
    pub fn loaded_points(&self) -> Vec<(usize, usize)> {
        let mut pairs = Vec::new();
        for bond in &self.bonds { for pair in [(bond.unit_a, bond.point_a), (bond.unit_b, bond.point_b)] { if !pairs.contains(&pair) { pairs.push(pair); } } }
        pairs
    }
}

pub fn formation_threshold(cohesion_a: f64, cohesion_b: f64, load_a: f64, load_b: f64) -> f64 {
    let load_a = load_a.max(0.0); let load_b = load_b.max(0.0);
...
mod tests {
    ...
    #[test] fn unit_properties_and_geometry_are_derived() {
        let catalog = default_catalog(); let mut s = OrganismStructure::new(); let i = unit(&mut s, "Carbon", 0.0, 0.0);
        assert_eq!(s.units[i].properties(&catalog).unwrap().cohesion, 0.95); assert_eq!(s.units[i].geometry.connection_regions.len(), 6);
    }
    #[test] fn connection_regions_are_not_occupancy_limited() {
        let catalog = default_catalog(); let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0);
        s.add_bond(bond(a, 0, b, 0, 0.5, 2.0)); let sites = s.available_connection_sites(&catalog);
        assert!(sites.contains(&ConnectionSiteRef { unit_index: a, point_index: 0 })); assert!(sites.contains(&ConnectionSiteRef { unit_index: b, point_index: 0 }));
    }
    #[test] fn connected_components_form_from_bond_graph() {
        let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); let c = unit(&mut s, "Carbon", 2.0, 0.0); let _d = unit(&mut s, "Methane", 10.0, 0.0);
        s.add_bond(bond(a, 0, b, 0, 0.5, 2.0)); s.add_bond(bond(b, 1, c, 0, 0.5, 3.0)); assert_eq!(s.connected_components(), vec![vec![0, 1, 2], vec![3]]);
    }
    #[test] fn breaking_a_bond_splits_components() {
        let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); let c = unit(&mut s, "Carbon", 2.0, 0.0);
        let first = bond(a, 0, b, 0, 0.5, 2.0); let second = bond(b, 1, c, 0, 0.5, 3.0); s.add_bond(first); s.add_bond(second);
        assert_eq!(s.connected_components(), vec![vec![0, 1, 2]]); assert_eq!(s.break_matching_bond(second), Some(second)); assert_eq!(s.connected_components(), vec![vec![0, 1], vec![2]]);
    }
    #[test] fn bond_energy_is_separate_from_strength() {
        let original = bond(0, 0, 1, 0, 0.25, 4.5); let restored: Bond = serde_json::from_str(&serde_json::to_string(&original).unwrap()).unwrap();
        assert_eq!(restored.strength, 0.25); assert_eq!(restored.bond_energy, 4.5);
    }
    ...
    #[test] fn is_valid_bond_ignores_stored_strength() {
        let catalog = default_catalog(); let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0);
        assert!(s.is_valid_bond(&bond(a, 0, b, 0, f64::NAN, 2.0), &catalog));
    }
    #[test] fn connection_load_uses_intrinsic_strength() {
        let catalog = default_catalog(); let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); s.add_bond(bond(a, 0, b, 0, 0.0, 9.0));
        let expected = crate::combine::bond_strength(*s.units[a].properties(&catalog).unwrap(), *s.units[b].properties(&catalog).unwrap()); assert!((s.connection_load(a, 0, &catalog) - expected).abs() < 1e-12); assert_eq!(s.connection_count(a, 0), 1);
    }
    #[test] fn break_bond_returns_energy() {
        let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); let i = s.add_bond(bond(a, 0, b, 0, 0.8, 7.25));
        assert_eq!(s.break_bond(i).unwrap().bond_energy, 7.25); assert!(s.bonds.is_empty());
    }
    #[test] fn break_matching_bond_uses_endpoint_identity() {
        let mut s = OrganismStructure::new(); let a = unit(&mut s, "Carbon", 0.0, 0.0); let b = unit(&mut s, "Methane", 1.0, 0.0); let stored = bond(a, 0, b, 0, 0.7999999999, 7.5); let snapshot = bond(b, 0, a, 0, 0.8, 7.25); s.add_bond(stored);
        assert_eq!(s.break_matching_bond(snapshot), Some(stored));
    }
    #[test] fn formation_threshold_is_symmetric_and_increases_with_load() {
