...
///
/// Material and geometry are both authoritative runtime state. No
/// `resource_name` field is retained, preventing a second resource identity
/// from diverging from composite material or inherited geometry.
...
impl StructuralUnit {
    /// Compatibility constructor for existing callers that instantiate a
    /// single base-resource unit. New inherited structures should construct
    /// units from their blueprint element directly.
    ...
    pub fn properties(&self, catalog: &[crate::resources::BaseResource]) -> Option<ResourceProperties> {
    ...
    /// Geometry is now intrinsic to the physical unit rather than looked up
    /// from a resource name. The catalog argument remains only for API
...
impl Bond {
    ...
    pub fn is_valid(&self, unit_count: usize, connection_point_count: impl Fn(usize) -> Option<usize>) -> bool {
        if self.unit_a >= unit_count || self.unit_b >= unit_count || self.unit_a == self.unit_b {
            return false;
        }
...
impl OrganismStructure {
    ...
    pub fn is_valid_bond(&self, bond: &Bond, catalog: &[crate::resources::BaseResource]) -> bool {
        if !bond.is_valid(self.units.len(), |i| {
            self.units.get(i).and_then(|u| u.geometry.connection_regions.len().into())
        }) {
            return false;
        }
    ...
    /// Return every connection region. Regions have no numerical bond
    /// capacity; occupancy is intentionally not filtered here. Physical
    /// geometry/contact determines whether additional bonds can coexist.
    ...
    /// Connection load is derived exclusively from immutable resource
    /// properties. Stored Bond::strength is never authoritative.
    ...
    pub fn disconnect_point(&mut self, unit: usize, point: usize) -> Vec<Bond> {
        ...
        while i < self.bonds.len() {
            if self.bonds[i].touches(unit, point) { removed.push(self.bonds.remove(i)); } else { i += 1; }
        }
        removed
    }
...
mod tests {
    ...
    #[test]
    fn unit_properties_and_geometry_are_material_and_unit_derived() {
        ...
        assert_eq!(s.units[i].geometry.connection_regions.len(), 6);
    }

    #[test]
    fn available_connection_regions_are_not_occupancy_limited() {
        let catalog = default_catalog();
        let mut s = OrganismStructure::new();
    ...
    #[test]
    fn breaking_a_bond_splits_components() {
        let mut s = OrganismStructure::new();
        let a = unit(&mut s, "Carbon", 0.0, 0.0);
        let b = unit(&mut s, "Methane", 1.0, 0.0);
        let c = unit(&mut s, "Carbon", 2.0, 0.0);
    ...
    #[test]
    fn bond_energy_is_separate_from_strength() {
        let original = bond(0, 0, 1, 0, 0.25, 4.5);
        let restored: Bond = serde_json::from_str(&serde_json::to_string(&original).unwrap()).unwrap();
    ...
    #[test]
    fn is_valid_bond_ignores_stored_strength() {
        let catalog = default_catalog();
        let mut s = OrganismStructure::new();
        let a = unit(&mut s, "Carbon", 0.0, 0.0);
        let b = unit(&mut s, "Methane", 1.0, 0.0);
    ...
    #[test]
    fn connection_load_uses_intrinsic_strength() {
        let catalog = default_catalog();
        let mut s = OrganismStructure::new();
        let a = unit(&mut s, "Carbon", 0.0, 0.0);
        let b = unit(&mut s, "Methane", 1.0, 0.0);
    ...
    #[test]
    fn break_bond_returns_energy() {
        let mut s = OrganismStructure::new();
        let a = unit(&mut s, "Carbon", 0.0, 0.0);
        let b = unit(&mut s, "Methane", 1.0, 0.0);
        let i = s.add_bond(bond(a, 0, b, 0, 0.8, 7.25));
        assert_eq!(s.break_bond(i).unwrap().bond_energy, 7.25);
        assert!(s.bonds.is_empty());
    }

    #[test]
    fn break_matching_bond_uses_endpoint_identity() {
        let mut s = OrganismStructure::new();
        let a = unit(&mut s, "Carbon", 0.0, 0.0);
        let b = unit(&mut s, "Methane", 1.0, 0.0);
        let stored = bond(a, 0, b, 0, 0.7999999999, 7.5);
        let snapshot = bond(b, 0, a, 0, 0.8, 7.25);
        s.add_bond(stored);
        assert_eq!(s.break_matching_bond(snapshot), Some(stored));
    }

    #[test]
    fn formation_threshold_is_symmetric_and_increases_with_load() {
